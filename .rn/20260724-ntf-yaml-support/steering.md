...
### #48: 図の作成（PlantUML 21枚）・既存画像の処置・README「図の作成方法」（user 承認 2026-08-30）
...
- **Next**: `#48` Step 1 から。ただし着手前に user の判断が要るものが1件ある（下記 Notes の (1)）
- **Notes**: ブランチ `ntf-yaml-support`、作業ツリーはクリーン。**変更したら push する**。指示書は `ntf-doc-48-figures.md`。**4観点レビューは回さない**。次の3点は着手前調査で見つけた指示書の不備で、いずれも未処置:
  - **(1) user 判断待ち（これだけが着手の前提）** — `setup/common.rst:219` は「ベースディレクトリの配下は\ **次の図のとおりで**\ 、リクエストIDごとに1つのファイルを置く。」であり、その「次の図」が `:221` の `send_sync_test_data_structure.png` を指す。§4 はこの画像を削除、§3 は代わりの図3を `:170` に置くため、削除すると `:219` の指示語が指す先を失う。§6 の (a)〜(g) にこの行の逐語が無く、§9 は本文の言い換えを禁じているので直せない。候補は3つ: (A) 末尾を「ベースディレクトリの配下には、リクエストIDごとに1つのファイルを置く。」に詰める（§6 (c) と同種。推奨）／(B) 「上の図のとおりで」に変える／(C) 図3と当該画像の削除だけ保留し残り20枚を進める。競合する他の指示語が無いことは実測済み（`grep -rn '次の図\|上の図\|下図' ja/development_tools/testing_framework --include=*.rst` の結果、(a)〜(e) で処置済みの5件を除くと残るのはこの1件だけ）
  - **(2) 判断不要・報告のみ** — §4 の見出し「削除する15件」は表の実数と合わない。表が挙げるのは13ファイル（`send_sync_test_data_structure.png`／`modification_detected.png`・`copy_from_backup.png`／`send_sync_base.png`・`hanrei.png`／`mail_overview.jpg`・`htmlDumpDir.png`／`send_sync_online_base.png`・`send_sync_online_mock.png`・`send_sync_test_data_no.png`・`send_sync_response_count_change.png`／`requestDumpToolAbstract.png`・`image.xlsx`）。現物も 28件 ＝ 削除13 ＋ キャプチャ13 ＋ 上書き2（`select_sequence.png`・`update_sequence.png`）で整合する。§8 の完了条件1（55件 ＝ `.puml` 21 ＋ `.png` 34）は13件前提で正しく、直すべきは §4 の見出しの数字と §8 の完了条件2 の「削除15件」の語だけ
  - **(3) 判断不要・記録のみ** — 図8（`web/request_test_components`）の「見せるもの」は `Nablarch Application Framework` を含むが、本文欄の `implementation/request_unit_test/web.rst:13`-`:44` にこの語は無い（同ページ全体でも0件。`grep -n 'Nablarch Application Framework' implementation/request_unit_test/web.rst` が0件）。同じ事実は `about/index.rst:106`（テスト対象クラスは本番と同じく\ NAF\ を介して動作する）と `implementation/request_unit_test/rest.rst:17`（内蔵サーバ上で動作する\ NAF\ が\ Action\ を呼び出す）にあり、解説書としては既出であるため、図8 に含めたうえで出典を `checks/task-48.md` に `about/index.rst:106`・`rest.rst:17` として記録する方針。異論があれば着手時に指示すること
