...
- **Status**: paused
- **Date**: 2026-08-26
- **Last completed**: `#36` Step 3 の論点12（`TODO(NTF-MOD-01-3)` を削除）と業務サンプルからの参照の是正。その前が論点11（グループIDに使える文字。本文変更なし）・論点7（識別子の大文字化。本文変更なし）。その前が論点6（空行スキップ。論点3 を含む。`5301d6e`）。**詳細は `#36` エントリの Step 3 の表と各コミットメッセージにある。State には繰り返さない**
- **Next**: `#36` Step 3 の続き。**残るのは論点13（0件テーブルが次ブロックを食う。本体未着手。Step 4 の入力）だけで、このPRの解説書側の残作業は png 26枚（user 担当）のみ。** 残作業の唯一の一覧は `~/work/cowork/nablarch/ntf-doc-renewal/01-現在地.md` の「このPRの残作業」にある。**本文に新しい記述が入るときは、書く前に実際に走らせて出力を確かめること。** `#33` (b)（png 26枚の禁止語点検。user 担当・このPRで実施）・(e-1)（見送り確定）と `#34` は変わらず残る
