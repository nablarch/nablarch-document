...
- [ ] `design.md:84`（対比表の取引単体テスト行の実行方法・備考）・`:86`（「取引単体テストが自動テストではないことを明示する。」）を、処理方式によって実施方法が異なることが分かる内容に改める。文言は CC が起案する — **`5e87f6e` で一度書いたが M-1 で差し戻し。書き直す**
- [ ] `about/index.rst:73`・`:75` に `design.md` の是正内容をそのまま反映する — **同上**
- [ ] `about/index.rst:77-79` の `important` を差し替える。**削除はしない**（対比表だけでは処理方式による違いが伝わらない）— **同上。ただし `important` を外して表直後の地の文にする案がクラフト観点から出ており、(1) の判断に含まれる**
- [x] `style.md:45` を、順序の規約と必須・任意の区別の両方を含む記述に改める（`design.md` は変更しない。第3部の規約 `style.md:46-51` も変更しない）— `5e87f6e`。4観点とも OK（`design.md:180`・`:185`・`:189`・`:194`・`:783` と一致、実ページ8件の実態とも一致）
- [x] `style.md:56-57` の `design.md` 参照が行番号で指しているものを**節見出しで指す形に改める**（実測: 第2部ページアウトラインは `design.md:76-88` ではなく `### ページのアウトライン`＝現 `:173-190`、第3部は `:132-141` ではなく現 `:242-` にある。生きている文書を行番号で指すと必ず陳腐化する。未解決事項10 と同根）— `5e87f6e`。実物は `:55-56` だった。新しい節見出し参照は実在し一意（`design.md:147` ＞ `:173`、`:213` ＞ `:242`）
- [x] 4観点のレビューを、それぞれ別のサブエージェントで実施する（QA / 設計 / クラフト / 検証。公開本文が変わるため回す）— ラウンド1 実施。**4観点とも FAIL**（`must` 1件に集約・`should` 13件）。是正後にラウンド2（是正差分限定）が要る
- [x] `checks/task-24.md`（新規）にゲート1〜12 の実行結果を記録し、`reviews/page-about_index.md` に是正内容と上記の実測を追記する — ゲート1〜12 全件 PASS。4観点の判定は `checks/task-24.md`「4観点レビュー ラウンド1」に記録（`db5a84a`）
- [x] commit & push — `5e87f6e`（本文）・`db5a84a`（レビュー記録）
- [ ] **user review**
...
**進捗（2026-08-14）**: 本文コミット `5e87f6e`（push 済み）。ゲート1〜12 は全件 PASS で `checks/task-24.md` に記録済み（Docker フルビルドは検証観点が独立に再実行して `build succeeded, 1 warning.`・新規0件、`sphinx.mo` 復元済み）。**4観点レビュー ラウンド1 は4観点とも FAIL。** 重複除去後の `must` は1件（M-1）に集約され、`should` は13件。詳細は `checks/task-24.md`「4観点レビュー ラウンド1」。

**ユーザー判断待ち（M-1。是正を回さずに上げた。`#22` 回答 §7「範囲を自分で広げない」に従う）**: `about/index.rst:73`・`:79` の「それ以外の処理方式はJUnitで自動実行」が、出典を持たない処理方式まで断定で覆っている。コーディネータが `mapping.csv` から機械的に作った全件表では、**自動と言い切れるのは6処理方式のうち2〜3件**である（HTTPメッセージングは手動の出典のみ、テーブルをキューとして使ったメッセージングは出典0件、MOMによるメッセージングは混在）。根には `send_sync.rst`・`http_send_sync.rst` の帰属の未決着がある — 出典自身の構成（旧 `05_UnitTestGuide/index.rst:16-27` の `toctree` が両ファイルを **\*ウェブアプリケーション\*** 節に置く。本文も両者 `:7` が「〜を伴うウェブアプリケーションで」）と `mapping.csv`（両者を MOM／HTTPメッセージングのページに割当）が食い違う。**`mapping.csv` は本タスクの変更禁止ファイル**であり、かつレビュー役が示した前提「手動なのはウェブアプリケーションだけであり、他はすべて自動である」自体の見直しになるため、独断で動かない。

**`should` 13件は M-1 の結論が出てから一括で処理する**（本文の文言が確定しないと、備考セルの係り受け・`実行方法`／`実施方法` の揺れ・`important` の要否・`:widths:` が決まらないため）。うち `glossary.md:168`（`取引単体テスト` の意味欄が「手動操作で確認するテスト」のまま）と `style.md` の残り10件の行番号参照・撤回済み方針（`design.md:139`）への依拠は、**いずれも本タスクの変更禁止ファイルまたはゲート5の範囲外**であり、`#24` に含めるか別タスクにするかの判断も要る。
...
- **Status**: paused
- **Date**: 2026-08-14
- **Last completed**: #22（user review 承認済み・DONE）。#24 は本文を一度書き（`5e87f6e`）ゲート1〜12 を全件 PASS したが、4観点レビュー ラウンド1 が**4観点とも FAIL**。判定は `checks/task-24.md`「4観点レビュー ラウンド1」に記録済み（`db5a84a`）
- **Next**: **#24 のユーザー判断3件の回答を受ける。** 回答後、`must` 1件（M-1）の是正と `should` 13件を同じラウンドで一括処理し、是正差分限定の検証ラウンド2 を回す
- **Notes**: ブランチ `ntf-yaml-support`（`db5a84a` まで push 済み・作業ツリーはクリーン・未追跡0件）。**ユーザー判断待ち3件**（2026-08-14 に提示済み・未回答）: (1) 公開本文の書き方 — 「それ以外の処理方式」という包括表現をやめて処理方式を名指しする案を推奨（`important` を外して表直後の地の文にする案を含む）、(2) `send_sync.rst`・`http_send_sync.rst` の帰属（`mapping.csv` 6行。出典自身の `toctree` はウェブアプリケーション節、`mapping.csv` は MOM／HTTPメッセージング）— `#25` として別タスクにする案を推奨、(3) `glossary.md:168` と `style.md` の残り（行番号参照10件・`design.md:139` で撤回済みの集約方針への依拠・`:45` と `:77` の自己矛盾）を `#24` に含めるか — 2件とも含める案を推奨。**回答があるまで本文を書き直さない**（`#22` 回答 §7「範囲を自分で広げない」）。不変条件: `mapping.csv` 595行 / 12,986 / 11,983、Docker フルビルドの既知警告は `db_double_submit.rst:108` の1件のみ。**#23 は指示書 `指示/task-23.md` の受領待ちで着手しない。**
