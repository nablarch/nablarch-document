...
- [ ] 30. **フェーズB B-3**。差分限定の2観点（範囲統制／新しい欠陥。後者に**承認された逐語案そのものへの反例検証**を含める）を**それぞれ別のサブエージェント**で回す。各プロンプトに常設の3点（実測で裏付ける／付属の検証スクリプトを正解にせず独立に組む／敵対的に見る）を入れる。**各観点の生出力を要約せずそのまま** `reviews/page-testdata_notation.md` の是正5 の節へ貼る。新しいファイルは作らない。`must` が出たら是正して再度2観点、上限2ラウンド（同 B-3）。**ラウンド1 実施済み。生出力は記録に貼付済み**（`reviews/page-testdata_notation.md` §「`#35`-是正5 フェーズB B-3 差分限定レビュー ラウンド1（2026-08-24）」）。**範囲統制は0件、新しい欠陥は `must` 1・`should` 2・`nice` 3。`must` は自分でも `git show e21bf67:` に当たって成立を確認した。是正が `:883` の user 承認済み逐語に触れるため、処置方針を user に諮って停止中。3案の逐語と `should`・`nice` 5件の判定は `reviews/page-testdata_notation.md` §「B-3 ラウンド1 の処置方針」に記録済み**
