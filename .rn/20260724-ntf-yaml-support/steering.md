...
- [ ] 30. **フェーズB B-3**。差分限定の2観点（範囲統制／新しい欠陥。後者に**承認された逐語案そのものへの反例検証**を含める）を**それぞれ別のサブエージェント**で回す。各プロンプトに常設の3点（実測で裏付ける／付属の検証スクリプトを正解にせず独立に組む／敵対的に見る）を入れる。**各観点の生出力を要約せずそのまま** `reviews/page-testdata_notation.md` の是正5 の節へ貼る。新しいファイルは作らない。`must` が出たら是正して再度2観点、上限2ラウンド（同 B-3）。**ラウンド1 実施済み。生出力は記録に貼付済み**（`reviews/page-testdata_notation.md` §「`#35`-是正5 フェーズB B-3 差分限定レビュー ラウンド1（2026-08-24）」）。**範囲統制は0件、新しい欠陥は `must` 1・`should` 2・`nice` 3。`must` は自分でも `git show e21bf67:` に当たって成立を確認した。是正が `:883` の user 承認済み逐語に触れるため、処置方針を user に諮って停止中**
- [ ] 31. **フェーズB B-4**。`reviews/page-testdata_notation.md` の是正5 の節に、フェーズA の実測結果と確定した逐語（A-1〜A-5）・B-3 の生出力とラウンドごとの指摘件数と観点を記録する。申し送り39・40・41 を処置済み／クローズにする。§「是正2・是正3 の指摘件数と観点」の「`nice` 3（**未処置**）」を処置済みへ更新する。是正3 の `nice` 3件目の記録の誤りと、`mapping/glossary.md` §5.10 の同型の誤り（申し送り42）を書く。`checks/task-35.md` と `steering.md` を反映後の状態にする。**記録に本ラウンドのコミットハッシュを書かない**（同 B-4）。**フェーズA の実測結果・逐語案・申し送り42・43 と、B-3 ラウンド1 の生出力・指摘件数は記録済み。残り（申し送り39〜41 の処置済み化、是正2・是正3 の `nice` 3 の更新、是正3 の `nice` 3件目の記録の誤り、`checks/task-35.md`・`steering.md` の反映）は Step 30 の決着後**
- [ ] 32. **フェーズB B-5**。Docker フルビルドを1回通し警告0を確認し、直後に `git checkout -- locales/ja/LC_MESSAGES/sphinx.mo` を実行する。`locales/` を `.gitignore` に加えない。`ca.crt`・`Dockerfile.ca` を作業ツリーに残さない。**B-1〜B-5 をまとめた1コミットをプッシュする。`--amend` と force push は行わない**（同 B-5）。**未着手。作業指示は「`956c723` に続けて」と書いているが、`956c723` の後に `/rn:dn` の `667f7e0` が既に push 済みであるため、実際には最新コミットに続けることになる**
...
**Notes（`#35`-是正5 第2ラウンド。フェーズA 完了時点）**:

- **Steps 1〜28 は check off 済み**（是正1〜是正4 と、是正5 の §1 検証・フェーズA）。**Steps 29〜32（フェーズB）は user の承認待ちで未着手。** `ja/` は1文字も変更していない（`git diff --stat -- ja/` が空）
- **A-1 は user 判断で確定**——`:1545` は前半の『行』付き4語を据え置き、後半の句だけを「フィールド名称の数を超える位置の値は読み込まない」へ差し替える
- **A-2（申し送り40）・A-3（申し送り39）・A-4（`:883`）は逐語案を出した。A-5（申し送り41）は「変更不要（別概念）」と判定した。** 4件の実測結果・逐語案・`file:line` は `reviews/page-testdata_notation.md` §「`#35`-是正5 フェーズA（実測と逐語案。`.rst` は1文字も変更していない、2026-08-24）」にある
- **本ラウンドの実測は、ソースの読解ではなく実行で行った。** `nablarch-testing@e21bf67` と `nablarch-testing-yaml@190cc9a` を `git worktree` ＋ `mvn -o -DskipTests compile` でビルドし、POI で生成した `.xls` と手書きの `.yaml` を各パーサへ直接通して `DataFileFragment` の `names`・`values` を観測した。手順は同節 §「実測の方法（A-2・A-3・A-4 共通）」
- **user の判断が要る点が2つある**——(1) A-4 の案に足した「\ Excel\ 形式で行の全セルを空にした場合は、その行自体が読み飛ばされる」の1文を入れるか（`:1534` はテーブル・`LIST_MAP` に限定した書き方であり、ファイルデータについては同ページに記述が無い）。(2) A-3 の案に同種の1文を第3文として足すか（作業指示は「1〜2文」と定めているため案には含めていない）
- **申し送り42・43 を新たに起票した**（`reviews/page-testdata_notation.md` §「申し送り（続き2）」）。42 は `mapping/glossary.md` §5.10 の `フィールド長行` の意味欄「固定長ファイルのみで使うもの」の誤り（完了条件48 が求めるもの）。43 は同 §5.10 の `レコード種別行` が実装上そういう行として存在しない件。**用語集は変更禁止のため、どちらも未変更**
- **申し送り41 はクローズしてよい**（A-5）。`mapping/glossary.md` §5.10 は「行」の名称、`testdata_notation.rst:854`-`:858`／`:866`-`:879` は「要素」の名称であり、別概念である。決め手は `:855`「レコード種別 + フィールド名称」（1行に2要素）・用語表に `パディング`（処理）が含まれること・`testdata_examples.rst:1082` が1文で「レコード種別行」（行）と「レコード種別」「フィールド名称」（要素）を書き分けていること
- **是正3 の `nice` 3件目の記録は不正確**——「『フィールド長行』は固定長ファイルにのみ存在する（`:1076`）」。`:1076` が言うのは可変長ファイルに無いことだけで、メッセージには有る（`:1158`・`:1196`）。フェーズB（B-4）で記録を更新する
- **`#35` は3コミットに分かれている**——`9aa06d7`（是正1・是正2 の本体）／`1d88729`（是正3 の本体。`wip:` コミット）／是正4 の1コミット（＋Evidence 追記の `149277f`）。**加えて `/rn:dn` の `667f7e0` が `956c723` の後に push 済みである。** 作業指示 B-g の「`956c723` に続けて1コミット」はこの時点で成り立たないため、フェーズB のコミットは最新コミットに続ける。`--amend` と force push は一度も行っていない
- **現在の `:1545`（是正4 で確定。フェーズB で後半の句を差し替える）**: `    - ディレクティブ行・フィールド名称行・データ型行・フィールド長行の行末の空セルを取り除く（\ Excel\ 形式のみ）。フィールド名称が宣言されていない値は読み込まない`
...
- **Next**: **B-3 ラウンド1 の `must` 1件の処置方針について user の判断を待つ。** 判断が出たら Step 30 の残り（案1 なら是正→ラウンド2）→ Step 31（B-4 の記録）→ Step 32（B-5 のビルドと1コミット）と続ける。`must` は `implementation/testdata_notation.rst:883` の「データ行の**セル数**（Excel形式）…がフィールド数より少ない場合」が1つずれている件（Excel のデータ行は先頭に空のレコード種別セルを持ち、`DataFileParser.java:186` の `addValue(tail(line))` と `:262`-`:267` の `tail` が必ず落とす）。**この句は現行 `:883` からの持ち越しで本差分が書いた文言ではないが、同じ差分が `:1160` に「ラベル列を除いたセル数」と正しく書いたため、ページ内に食い違いが生まれた。** 是正すると user 承認済みの A-4 逐語に触れるため停止した。提示した3案・差し替え案の逐語・`should` 2件と `nice` 3件の判定は、直前のチャット報告にある
- **Notes**: ブランチ `ntf-yaml-support`。**作業ツリーには `ja/` の2ファイルと `reviews/page-testdata_notation.md` の変更が未コミットで残る**（本 `/rn:dn` の `wip:` コミットに含める）。差分は A-1〜A-4 の4 hunk のみで、範囲統制レビューが1文字一致を機械照合済み。**A-3 の挿入で A-1 の行は `:1545` → `:1547` にずれた**——完了条件44（B-a）の `sed -n '1545p'` は `sed -n '1547p'` へ読み替える。**フェーズB の1コミットは最新コミットに続ける**（B-g の「`956c723` に続けて」は既に成り立たない。`--amend` と force push は行わない）。**実測用の `git worktree` 2つ（`nablarch-testing@e21bf67`・`nablarch-testing-yaml@190cc9a`）は user 指示により B-5 の後始末まで残す。** 実体はセッションのスクラッチパッド配下で、本リポジトリの作業ツリーには何も置いていない。詳細な申し送りと未確認点は `#35` の Notes 節を参照