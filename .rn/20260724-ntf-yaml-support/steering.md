...
- **Status**: paused
- **Date**: 2026-08-25
- **Last completed**: `#36` Step 3 のうち3論点（論点5・9・10）。**論点9** マーカーカラムが掛かるデータタイプの範囲を是正（`faebf90`。5箇所）。**論点5** 不成立＝回帰していないと判定し、残っていた1箇所だけ同ページ内の記述と揃えた（`8fe964b`）。**論点10** `#23` で既に書かれており対応不要と判定。**詳細は `#36` エントリの Step 3 の表と各コミットメッセージにある。State には繰り返さない**
- **Next**: `#36` Step 3 の続き。**残るのは論点2・3・6・7・11（仕様の判断が要る）と論点12・13（TODO 管理）。** 次の1件は**論点2（`"-"` の副作用）**で、実装の裏付けと着手手順は `~/work/cowork/nablarch/ntf-doc-renewal/01-現在地.md` の「次の1件 ── 論点2」にある。**本文に新しい記述が入るため、書く前に実際に走らせて出力を確かめること**（`#36` ラウンド1 の重大指摘①と同じ形を踏まないため）。**着手前に `#36` エントリの「確定した基準」と Step 3 の「進め方の追加」を読む**（後者は「あるべき姿が自明なものは確認を仰がず直す」）。**`01-現在地.md` の表の行番号は `d86bb59` 時点のものでずれているため、使う前に実物を開くこと。** `#33` (b)（png 26枚の禁止語点検。user 実施）・(e-1)（見送り確定）と `#34` は変わらず残る
- **Notes**: ブランチ `ntf-yaml-support`、最新コミット `a69761a`、作業ツリーはクリーン、未追跡パスなし。**`origin` は `e2aa3e2` で、`faebf90`・`8fe964b`・`a69761a` の3コミットは未 push**（push は user の明示指示があるまで行わない）。番号付きタスクは 1〜32・35 が Closed/DONE、`#33`・`#34` が未決、`#36` が進行中。`main` へのマージと `.rn/` の扱いは user の明示指示待ち（`Rules` 末尾）。**`nablarch-testing-yaml` の `feature/ntf-yaml` は参照ピン `190cc9a` から71コミット進んで `0db2221` になっており、YAML の空行判定とカラム名決定をモジュール側が先に変更している**（論点6・論点3 の領域）。SSoT 方針では解説書が正なので、解説書で決めた仕様にモジュールを合わせる。ステップ4の申し送りに記録済み
