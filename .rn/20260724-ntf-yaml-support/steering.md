...
**母集合ラウンド1レビュー（対象 `5276d1e`）の指摘を受けた修正（ラウンド2）**:

QA/設計/クラフト/検証の4観点とも NG。トリアージの詳細は `checks/task-03.md`「母集合ラウンド1の指摘トリアージ」を参照。Valid 4件を以下のとおり修正する。

- [ ] `term-candidates.csv` に「本文を含む出現ファイル数」「総出現数」の列を追加する。全331候補（採用済みの候補も含む）について、見出しだけでなく本文出現も含めて機械的に数え直す。誤りの原因は「見出し出現ファイル数」を代理指標にしたこと自体であり、単一ファイル判定で不採用にした候補に限定せず、採用側に回した候補にも同じ基準の誤りが混入している可能性があるため、対象を限定しない
- [ ] 採用・不採用の判定をこの実測値に基づいて再構成する。再監査によって判定が変わった候補（採用→不採用、不採用→採用の両方向）の件数を self-check の Evidence に記載する
- [ ] `verify_glossary.py` に、採用・不採用の判定がこの実測値と矛盾していないことを検査する項目を追加する
- [ ] §5.15.5 ほかの不採用理由を、紋切り型の使い回しではなく、実測値（本文を含む出現ファイル数・総出現数・識別子か否か）から機械的に導ける具体的な理由に書き直す
- [ ] §5.4/§5.5 ほか、揺れ表記の代表file:line根拠が皆無の行（11行・27箇所）に代表1件のfile:lineを補充する
- [ ] `エンティティバリデーション`・`Excel形式`の「意味」欄にある旧表記（`バッチ処理`・`ブック`）を正表記に統一する
- [ ] self-check（`checks/task-03.md`）を更新する
- [ ] commit & push
- [ ] 4観点のレビュー（ラウンド2。ラウンド1の指摘発生源にまたがるため QA/設計/クラフト/検証を再実行）— **プロンプトは Rules「レビューを依頼するサブエージェント…」の3点を必ず含める**
- [ ] **user review** — 承認を受けるまで #4 に進まない
...
- `term-candidates.csv` に「本文を含む出現ファイル数」「総出現数」の列がある
- 採用・不採用の判定が、この実測値と矛盾していないことを `verify_glossary.py` が検査する
- 再監査によって判定が変わった候補の件数が Evidence に記載されている（採用→不採用、不採用→採用の両方向）
