...
- **Steps 1〜22 は全件 check off 済み。完了条件1〜39 は自己判定で全件充足。** 残るは user review のみ。是正ラウンドは上限3に到達しており、是正4 ではレビューを回していない（作業指示の冒頭。`:1545` の逐語の反例検証はディレクター側が実測で実施済み。転記した §5 の5点は、こちらでも `git show e21bf67:<path>` に当たり直して全件一致を確認した）
- **`#35` は3コミットに分かれた**——`9aa06d7`（是正1・是正2 の本体）／`1d88729`（是正3 の本体。`wip:` コミット）／是正4 の1コミット。`--amend` と force push は一度も行っていない。あいだに `/rn:up` の bookkeeping コミット（`3132688`・`13bd603`）が入る
- **確定した `:1545`**: `    - ディレクティブ行・フィールド名称行・データ型行・フィールド長行の行末の空セルを取り除く（\ Excel\ 形式のみ）。フィールド名称が宣言されていない値は読み込まない`。`:1544`・`:1546`・`:1547`・`tools/testdata_converter.rst:71`・`mapping/glossary.md` は未変更
- **`#35` 着地後に1タスクへまとめるもの（3件）**——申し送り39（`tools/testdata_converter.rst:71` の飛び先にメッセージのデータ行の記述が無い）／申し送り40（同 `:71` が「フィールド名称行」だけを挙げ、`:1545` の4行と食い違う）／`:883` の Excel 側の括弧書き（「全フィールドを省略した行（Excel形式では先頭セルが空の行）」が `PoiXlsReader.java:140`-`:147` の `isBlankLine` で読み飛ばされるため成り立たない疑い。申し送り候補、未起票）。いずれも `reviews/page-testdata_notation.md` §「申し送り（続き2）」と本項が出典
- **是正3 の `nice` 3件（未処置）**——同期応答メッセージ送信のラベル列と `:1545` 後半の句の字面の衝突／ページ内の行の呼称の不統一（「データ型行」はページ内に定義が無い）／「フィールド長行」は固定長ファイルにのみ存在。作業指示の範囲外のため処置していない。内容と出典（`1d88729` の `State`）は `reviews/page-testdata_notation.md` §「是正2・是正3 の指摘件数と観点（Steps 11・16 の積み残しをここへ移す）」にある。**未確認**——是正3 のレビューの観点別の生出力は保全されておらず、どの観点がどの指摘を出したかは記録に無い
