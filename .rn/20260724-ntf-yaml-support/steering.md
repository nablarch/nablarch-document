...
- [ ] 2a. 本文の書き直しを伴う2件。**user の判断待ち**（下の「未決」を参照）
- [ ] 3. 2ページ完了後、`01-現在地.md` の残り10論点を突き合わせ、基準で消えるものを落とす
...
**レビューは回していない。** 削除が主で、公開本文に新しい記述が入らないため（`nablarch/CLAUDE.md` 3-1
の「既存のものの是正」に当たる）。**判断の根拠は各コミットメッセージにあり、user が差分を見る前提で
進めている。**

**未決**:

- **Step 1・Step 2 のコミットは push していない。** `origin/ntf-yaml-support` は `d86bb59` のまま。
  push の可否は user の判断を待っている
- `implementation/testdata_notation.rst` の「データ行を書かない場合でも、カラム名の行は省略できない。
  識別子行の次の行がカラム名の行として読み込まれるため、カラム名の行を書かないと、その次に現れた行が
  カラム名の行になる。」の後半は帰結だが、論点13（0件テーブルが次ブロックを食う）が未決のため触っていない
- **Step 2a-1: `implementation/testdata_examples.rst`「可変長ファイルのディレクティブを指定する >
  YAML形式の場合」の `record-separator` の記述。** 現行は「`"\r\n"` と記述すると実際の制御文字になり、
  値が除去されて区切りが無くなる。**この場合はエラーにならない**ため、改行コードは `CRLF` のように
  シンボルで指定する」。帰結を述べているが、`"\r\n"` は利用者が正しく書こうとして選ぶ書き方であり、
  かつ失敗が表に出ない。**落とすと文が成立せず書き直しになるため、判断を仰いで止めている**
- **Step 2a-2: 同ページ「null・空文字・改行など特殊な値を記述する」節冒頭の「なお、空文字・改行の
  記述例は、この節では示していない。」** `implementation/testdata_notation.rst:1370`・`:1420`・`:1470`
  の3箇所が「実際の記述例は…参照」としてこの節を指しており、参照が空振りしている。
  **この文を落とすだけでは空振りが見えなくなるだけで直らない。例を足すのは新しい本文が入るため、
  判断を仰いで止めている**
...
- **Status**: paused
- **Date**: 2026-08-25
- **Last completed**: `#36` Step 1（`implementation/testdata_notation.rst` の基準によるノイズ除去。10コミット、58行削除・21行追加）。ディレクターが直接コミットしており、CC への指示書はない。**修正の根拠は各コミットメッセージにある**
- **Next**: `#36` Step 2（`implementation/testdata_examples.rst` を同じ基準で見直す。2,264行、未着手）。**着手前に `#36` エントリの「確定した基準」と「`#35`・`#33` の決定を上書きした箇所」を必ず読むこと。** 過去の記録（`#35` 是正1〜6、`#33` (a)）と現在の本文が食い違って見えるのは、基準の確定によって意図的に上書きしたためである。`#33` (b)（png 26枚の禁止語点検）・(e-1) と `#34` は変わらず残る
- **Notes**: ブランチ `ntf-yaml-support`、作業ツリーはクリーン、未追跡パスなし。**`origin` とは同期していない。`#36` の10コミットが未 push で、`origin/ntf-yaml-support` は `d86bb59` のまま**（push の可否は user 判断待ち）。番号付きタスクは 1〜32・35 が Closed/DONE、`#33`・`#34` が未決、`#36` が進行中。`main` へのマージと `.rn/` の扱いは user の明示指示待ち（`Rules` 末尾）
