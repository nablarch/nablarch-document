...
- **Status**: paused
- **Date**: 2026-08-05
- **Last completed**: #8 フィードバック対応・2ラウンド目（2026-08-05、`/rn:gm`）。1ラウンド目（6点、コミット
  `82ca8ce`/`a386505`）の後、「全体像と特徴を別見出しのまま`:ref:`で往復させる構成がおかしい、既存ありきの
  修正では」という指摘を受け、FW解説書`nablarch/big_picture.rst`の実際の構成を確認したうえで「全体像」と
  「特徴」を1つのH2に統合し、各特徴を能力の提示＋具体的メリットの一体記述に書き直し、同一ページ内の
  `:ref:`往復（アーキテクチャ・テストデータ・全体像へのバックリンク）をすべて削除。JUnit4アノテーション
  互換性の説明・コード例は「JUnitの書き方を活かせる」特徴点の裏付けとして再配置。design.md §2改訂・
  `about/index.rst`反映、Dockerビルド確認済み（`build succeeded`、想定内warning1件のみ）
