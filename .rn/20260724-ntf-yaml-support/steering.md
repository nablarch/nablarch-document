...
- [ ] 24. **§2**。`implementation/testdata_notation.rst:1545` を §1 の逐語に差し替える。`:1544`・`:1546`・`:1547`・`tools/testdata_converter.rst:71`・`mapping/glossary.md` は変更しない。`:866`-`:875` の用語表と `:1055`-`:1063` の書式表も変更しない（是正5 §2・完了条件2〜3）。**未着手**（Step 23 の反例のため）
- [ ] 25. **§3**。差分限定の2観点（範囲統制／新しい欠陥。後者に**逐語案そのものへの反例検証**を含める）を**それぞれ別のサブエージェント**で回す。各プロンプトに常設の3点（実測で裏付ける／付属の検証スクリプトを正解にせず独立に組む／敵対的に見る）を入れる。**各観点の生出力を要約せずそのまま** `reviews/page-testdata_notation.md` の新設節へ貼る。新しいファイルは作らない。`must` が出たら是正して再度2観点、上限2ラウンド（是正5 §3・完了条件4）。**未着手**
- [ ] 26. **§4**。`reviews/page-testdata_notation.md` に是正5 の節を新設し、§1 の4点の検証結果・確定した `:1545` の逐語・§3 の生出力とラウンドごとの指摘件数と観点を記録する。§「是正2・是正3 の指摘件数と観点」の「`nice` 3（**未処置**）」を処置済みへ更新する。是正3 の `nice` 3件目の記録「『フィールド長行』は固定長ファイルにのみ存在する（`:1076`）」が不正確であること（メッセージにも存在する。`:1158`・`:1196`）も書く。申し送り41 を §「申し送り（続き2）」に起票する。`checks/task-35.md` と `steering.md` を反映後の状態にする。**記録に本ラウンドのコミットハッシュを書かない**（是正5 §4・完了条件5〜6）。**§1 の検証結果と申し送り41 は実施済み。残りは Step 24〜25 の後**
- [ ] 27. **§5**。Docker フルビルドを1回通し警告0を確認し、直後に `git checkout -- locales/ja/LC_MESSAGES/sphinx.mo` を実行する。`locales/` を `.gitignore` に加えない。`ca.crt`・`Dockerfile.ca` を作業ツリーに残さない。**完了条件40〜46 をまとめた1コミットを `149277f` に続けてプッシュする。`--amend` と force push は行わない**（是正5 §5・完了条件7〜8）。**未着手**
...
**Completion criteria（`#35-是正5` 指示書「完了条件」1〜8 の逐語）**。**上の 32 は是正5 §1 の文面へ読み替える**（是正4 §1 が定めた `:1545` の逐語を是正5 §1 が差し替える。`:1544`・`:1546`・`:1547` は是正2 のまま）:

40. §1 の4点の検証結果が記録にある。反例が出た場合は §2 以降に進まず報告していること
41. `:1545` が §1 の逐語と1文字一致（`sed -n '1545p'` の出力を記録に貼る）
42. `git diff -U0` の `ja/` 側のハンクが `@@ -1545 +1545 @@` の1つだけ。`:1547`・`tools/testdata_converter.rst:71`・`mapping/glossary.md`・`:866`-`:875`・`:1055`-`:1063` が未変更
43. §3 の2観点を別サブエージェントで回し、生出力が要約なしで記録にある。ラウンドごとの指摘件数と観点が記録にある
44. 是正3 の `nice` 3件が処置済みとして記録が更新されている
45. 申し送り41 が起票済み
46. Docker フルビルド成功・警告0、`sphinx.mo` が未変更、`ca.crt`・`Dockerfile.ca` が無い
47. 40〜46 をまとめた1コミットを `149277f` に続けてプッシュ済み。`--amend` と force push は行わない

**Notes（`#35`-是正5 §1 の反例検出時点）**:

- **Steps 1〜22 は全件 check off 済み（是正1〜是正4）。完了条件1〜39 は自己判定で全件充足。** 是正5（Steps 23〜27・完了条件40〜47）は **Step 23（§1 の逐語検証）で反例が出たため、§2 以降に進んでいない**。`ja/` は1文字も変更していない
- **§1 (1) の反例**——作業指示 §1 (1) は「`:1055` の「ディレクティブ行」だけが『行』付きである」とするが、`implementation/testdata_notation.rst` には `:1545` を除いても『行』付きが6件ある（`ディレクティブ行` `:1010`・`:1055`／`フィールド長行` `:1076`／`フィールド名称行` `:1267`・`:1269`・`:1271`）。同ページ `:1267`-`:1271` は**メッセージの節で「フィールド名称行」を3回使っている**。よって `:1545` から『行』を落とすと、ページ内の他の記述から離れる。詳細は `reviews/page-testdata_notation.md` §「`#35`-是正5 §1 の逐語検証（着手前。`.rst` は未変更）」
- **§1 (2)(3)(4) は反例なし。** (3) は `:874`「各フィールドのバイト長（固定長ファイルのみ存在）」と `:1158`・`:1196`（メッセージにフィールド長が有る）を実物で確認済み。(4) は `nablarch-testing@e21bf67` の5箇所すべてを `git show` で確認し、指示書の逐語と一致した
- **是正3 の `nice` 3件目の記録は不正確**——「『フィールド長行』は固定長ファイルにのみ存在する（`:1076`）」。`:1076` が言うのは可変長ファイルに無いことだけで、メッセージには有る（`:1158`・`:1196`）。作業指示 §4 もこの訂正を求めている
- **申し送り41 は起票済み**（`reviews/page-testdata_notation.md` §「申し送り（続き2）」）。ただし内容は §1 (1) の実測に合わせてあり、作業指示の文面（「`:1055` だけが『行』付き」）そのままではない
- **`#35` は3コミットに分かれた**——`9aa06d7`（是正1・是正2 の本体）／`1d88729`（是正3 の本体。`wip:` コミット）／是正4 の1コミット（＋Evidence 追記の `149277f`）。`--amend` と force push は一度も行っていない。あいだに `/rn:up` の bookkeeping コミットが入る
- **現在の `:1545`（是正4 で確定）**: `    - ディレクティブ行・フィールド名称行・データ型行・フィールド長行の行末の空セルを取り除く（\ Excel\ 形式のみ）。フィールド名称が宣言されていない値は読み込まない`。`:1544`・`:1546`・`:1547`・`tools/testdata_converter.rst:71`・`mapping/glossary.md` は未変更
- **`#35` 着地後に1タスクへまとめるもの（4件）**——申し送り39（`tools/testdata_converter.rst:71` の飛び先にメッセージのデータ行の記述が無い）／申し送り40（同 `:71` が「フィールド名称行」だけを挙げ、`:1545` の4行と食い違う）／申し送り41（ページ内の行の呼称の不統一）／`:883` の Excel 側の括弧書き（`PoiXlsReader.java:140`-`:147` の `isBlankLine` で読み飛ばされるため成り立たない疑い。申し送り候補、未起票）。いずれも `reviews/page-testdata_notation.md` §「申し送り（続き2）」と本項が出典
...
- **Status**: paused
- **Date**: 2026-08-24
- **Last completed**: `#35`-是正5 の Step 23（§1 の逐語検証）。**(1) で反例が出たため §2 以降は未着手**。`ja/` は1文字も変更していない。Steps 24〜27（§2〜§5）と完了条件41〜43・46〜47 は未着手、44 は一部
- **Next**: **`#35`-是正5 の逐語の扱いについて user の判断を待つ。** 選択肢は3つ——(1) `:1545` は是正4 の『行』付きのまま据え置き、語法の統一を申し送り41 の1タスクへ送る（こちらの推奨。`:1267`-`:1271` と揃った状態を保てる）／(2) 後半の句だけ差し替える（「フィールド名称が宣言されていない値は読み込まない」→「フィールド名称の数を超える位置の値は読み込まない」。§1 (4) で実装一致を確認済み）／(3) 反例を承知のうえで指示どおり4語から『行』を落とす。判断が出たら Steps 24〜27 を続ける。反例の中身と (2)(3)(4) の検証結果は `reviews/page-testdata_notation.md` §「`#35`-是正5 §1 の逐語検証（着手前。`.rst` は未変更、2026-08-24）」にある
- **Notes**: ブランチ `ntf-yaml-support`（`956c723` まで push 済み）。作業ツリーはクリーンで、未追跡の残置ファイルは無い。`#35` の是正1〜是正4 は Steps 1〜22 全件 check off 済みで user review 待ち。**user へ上げた申し送り事項2件**——(1) 作業指示 §1 (1) の前提「`:1055` の『ディレクティブ行』だけが『行』付き」は成り立たない（`:1545` を除き同ページに6件）。(2) `mapping/glossary.md` §5.10 の `フィールド長行` の意味欄「固定長ファイルのみで使うもの」も、是正3 の `nice` 3件目と同じ誤りを含む（§2 が変更禁止のため未変更）。**未確認**——同欄の採用根拠 `S:input/ntf-doc-terms.md:178` には当たっていない。**参照コミット**: `nablarch-testing@e21bf67`・`nablarch-testing-converter@e977824`・`nablarch-testing-yaml@190cc9a`
