...
- **Step 4 の `nablarch-testing` では、`src/main` を変更しない**（user 判断 2026-08-26）。解説書と実装が食い違い、**解説書側が正しいと判断した場合も**実装を直しに行かず、根拠を添えて報告して止める。理由は、`nablarch-testing` が既にリリース済みであり、実装を動かすと利用者の後方互換が壊れること。**何を直すかは報告を受けた user が決める。** §4-2（解説書との全件突合）では `src/test` も変更しない（不一致が疑われる現行挙動を特性テストで固定すると、誤っている疑いのある挙動を正解として確定させることになる）。§4-1 の特性テストと §4-3 のカバレッジのテスト追加は対象外。指示書 `ntf-step4-01-nablarch-testing.md` に反映済み。**他の4モジュールへ同じ禁止を及ぼすかは未決**（`-yaml` の `YamlSection.isBlankRow`、`-converter` の中間モデルの `[ ]` は、いずれも user が是正すると判断済みの項目であり、この禁止とは別扱い）
