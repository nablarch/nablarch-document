...
- [ ] `about/index.rst:77-79` の `.. important::` を外し、指示書 §2-2 の3文の地の文に差し替える（対比表の直後・`.. _testing_framework_about-test_type_names:` の前、字下げ0、1段落1行）
- [ ] `design.md:84`（実行方法セル・備考セル）を `about/index.rst:73`・`:75` と逐語一致させる（`APサーバ` を使わず `アプリケーションサーバ`）— `:84` は4観点とも OK
- [ ] `design.md:86` を指示書 §2-3 の7点で書き直す。`## 走査した事実` / `## 決定と理由` / `## 申し送り` に段落を分け、grep で得た事実とリンクをたどって得た事実を分けて書く。末尾に §3-2 の申し送り1文を置く
- [ ] `style.md:96-102`（拡張例の省略の根拠）から `:98`・`:99`（「なし。」と書く実例）を削り、採らない旨を1文で残す（§4-2）
- [ ] `style.md:189` の S-04 表 L2 行の用途欄を「ページの大セクション。構成は `S-02` による」に改める（`-` の記号対応と根拠欄 `:198` は変えない。§4-2）
...
- [ ] `reviews/page-about_index.md` の `## #24` 節を整理し、公開本文の最終形と出典の実測結果だけを残す（S-11）— `82dbe16` で `:318` 以降への純追加61行。**走査記録の是正（R2-M4）と `design.md` との逐語重複の解消（R2-S2・ゲートN）が残る**
...
- [ ] `checks/task-24.md` にラウンド3の節を追記する（ラウンド1・2の記述は書き換えない）— ゲート1〜12 の再実行とゲートA〜N の実行結果
- [x] 4観点のレビューを、それぞれ別のサブエージェントでもう一巡回す（**指示書はサブエージェントに渡さない**）— ラウンド2 実施。**4観点とも FAIL**。重複除去後の `must` 5件は `checks/task-24.md`「4観点レビュー ラウンド2」
- [ ] 4観点のレビューをラウンド3 でもう一巡回す（**`ntf-doc-24.md` はサブエージェントに渡さない**）
- [ ] commit & push — ラウンド2 は `82dbe16`（push 済み）。**ラウンド3 の `must` が0件になったら、判断を仰がずに commit & push してよい**（指示書 §5-4）
- [ ] **user review**
