...
**Purpose**: ページを作らないタスク。`#22` の user review で回答された判断待ち2件を、承認済み資産の是正として1タスクにまとめて行う。(a) 取引単体テストの実行方法は処理方式によって異なるという事実に `design.md` と `about/index.rst` を合わせる。(b) `style.md:45` に第2部セクションの必須・任意の区別を書き足す。取引単体テストは第2部に残り2ページ・第3部に6ページあり、そのすべてが同じ矛盾を踏むため先送りしない。指示は `ntf-doc-24.md`（ラウンド3 まで反映した唯一の指示書。ラウンド2 に渡した版の文言は破棄）。

**ラウンド1 の到達点（`5e87f6e`・`db5a84a`）**: ゲート1〜12 全件 PASS。ただし4観点レビューは**4観点とも FAIL**（`must` 1件＝M-1 に集約・`should` 13件）。M-1 は「それ以外の処理方式はJUnitで自動実行」が出典を持たない2処理方式まで断定で覆っていたこと。判定は `checks/task-24.md`「4観点レビュー ラウンド1」。

**ラウンド3 の指示（2026-08-14 受領、`ntf-doc-24.md`）**: 判断待ち5件に回答。(M1) 「JUnitでの自動実行はできず」は**削る**（出典8ファイルに不可能性の記述0件）。(—) `important` は**外して地の文に降ろす**（M1 を削ると S-06 の要件を満たさないため。ラウンド1 の `S-3` 起案が正しかった）。(M2) **MOM を名指しから外す**（`about/index.rst:92` の MOM の定義が受信と送信の両方を覆い、処理方式名の粒度では真偽が定まらない）。(M3) 「自動実行する」→**「取引単体テストをJUnitで自動実行できる」**に戻す。(4) 指示書 §2-1 の「実施方法をひとことも書いていない」は**レビュー役の誤り**（両ファイルは実施方法を書いており、書いていないのは手動・自動の区別だけ）。(5) `style.md` 範囲外2件は**2件とも `#24` に含める**。**公開本文で名指しするのは3処理方式のみ**（ウェブアプリケーション＝手動 `index.rst:7-8`／RESTfulウェブサービス＝自動 `rest.rst:7`／Nablarchバッチアプリケーション＝自動 `batch.rst:5-6`）。`MOMによるメッセージング`・`HTTPメッセージング`・`テーブルをキューとして使ったメッセージング` の3つは**公開本文で何も述べない**。

**ラウンド4 の指示（2026-08-14 受領、`ntf-doc-24.md` §7）**: **ラウンド3 の成果は承認された**（レビュー役が `4961a86` を自分でクローンして全ゲートを追試し全件一致。報告1「`design.md` §3-2 の申し送りの前提が事実に反していた」も CC の指摘が正しいと認められた）。残る判断待ち3件に回答し、**3行だけ直して `#24` を閉じる**。(1) 公開本文の文言の改善案5件は**2件だけ採る**（第2文に目的語「取引単体テストを」を補う／「テスト対象のアプリケーション」→「テスト対象」）。**デプロイの重複解消・第2文を「行える」に揃える・名指ししない3処理方式への誘導文の3件は採らない**（特に「行える」は出典の断定（`index.rst:7-8`）を可能性に弱めるため書いてはならない）。(2) `about/index.rst:81` の「このうち」は**直す**（許可行を `:81` まで広げる）。(3) 申し送り3件のうち `style.md:5` の「7. トンマナ」→「8. トンマナ」だけ `#24` で直し、残り2件は `#pre-last` へ送る。**4観点レビューは回さない**（§7-7）。

**Steps**（ラウンド4。`- [x]` はラウンド1〜3で確定済みのもの）:

- [x] `about/index.rst:77` の**第2文のみ**を差し替える（目的語の補い＋「アプリケーション」3回の解消）。第1文・第3文は変更しない — `66fe4c9`
- [x] `about/index.rst:81` の「このうち」を「3種類のテストのうち、」に改める（先行詞を L54 の3種類のテストに固定する）。ラベル `testing_framework_about-test_type_names` の名称・位置は変えない — `66fe4c9`
- [x] `style.md:5` の `design.md`「7. トンマナ」を「8. トンマナ」に改める（実物は `design.md:411` が `## 8. トンマナ`。`style.md:569` は既に正しい）。「8観点」は変えない — `66fe4c9`
- [x] `reviews/page-about_index.md` の `## #24` 節に最終形の表と `:81` の変更理由を追記する — `66fe4c9`。変更行は `:335` 以降で節見出し `:322` の内側
- [x] `checks/task-24.md` にラウンド4の節を追記する（ラウンド1〜3の記述は書き換えない）— `66fe4c9`。変更全件表・ゲートO〜U・ゲートU の全件5件・ラウンド3 ゲート1〜12/A〜N の再実行結果を記録
- [x] 4観点のレビューは**回さない**（指示書 §7-7 が明示。ラウンド3 で収束済み、変更は局所的な語句の差し替え）
- [x] commit & push — `66fe4c9`。push 済み

**Steps**（ラウンド3。`- [x]` はラウンド1・2で確定済みのもの）:

- [x] `about/index.rst:58`・`:73`・`:75` — `82dbe16` で差し替え済み。4観点とも OK（`:widths: 9,26,16,49` は表示幅の実測比 `20/55/35/106` → `9.3/25.5/16.2/49.1` と一致）。**ラウンド3 では変更しない**（指示書 §2-2）
- [x] `about/index.rst:77-79` の `.. important::` を外し、指示書 §2-2 の3文の地の文に差し替える — `443dccc`。ゲートA・B・J・K PASS
- [x] `design.md:84`（実行方法セル・備考セル）を `about/index.rst:73`・`:75` と逐語一致させる — ラウンド2 で確定済み。ラウンド3 でゲートC を再実行して PASS（変更なし）
- [x] `design.md:86` を指示書 §2-3 の7点で書き直す（`443dccc`）。`## 走査した事実` / `## 決定と理由` / `## 申し送り` に段落を分け、grep で得た事実とリンクをたどって得た事実を分けて書く。末尾に §3-2 の申し送り1文を置く
- [x] `style.md:96-102`（拡張例の省略の根拠）から `:98`・`:99`（「なし。」と書く実例）を削り、採らない旨を1文で残す（§4-2）— `443dccc`
- [x] `style.md:189` の S-04 表 L2 行の用途欄を「ページの大セクション。構成は `S-02` による」に改める（§4-2）— `443dccc`
- [x] `style.md` の残り10件の `design.md:NN` 行番号参照を節見出し参照に置き換える（S-7）。`:77` を `:45-46` に合わせる（S-8）。`:53-54`・`:62-67`・`:80-81` を `design.md` の「モジュール一覧は第1部に置かない」に合わせる（S-9）。`:69-73` の「本文に『なし。』とだけ書く」を削る（S-10）— `82dbe16`。4観点とも `must` 0件。置き換え先の節見出しは `design.md:118`・`:173`（親 `:147`）・`:242`（親 `:213`）で実在し一意
- [x] `glossary.md:168` の**意味欄のみ**を、実施方法を含まない定義に改める（S-6）— `82dbe16`。差分は1行のみ、§5.15 の差分0件。兄弟項目との非対称の指摘は指示書 §4 S-6 が裁定済みのため Invalid
- [x] `reviews/page-about_index.md` の `## #24` 節を整理し、公開本文の最終形と出典の実測結果だけを残す（S-11）— `443dccc`。走査記録を grep 由来／リンク由来に分け、理由づけを `design.md` への参照にした。20-gram 一致率 28%→7.8%
- [x] `style.md:45` を、順序の規約と必須・任意の区別の両方を含む記述に改める — `5e87f6e`。4観点とも OK
- [x] `style.md:56-57` の `design.md` 参照を節見出しで指す形に改める — `5e87f6e`。実物は `:55-56` だった。参照先は実在し一意（`design.md:147`・`:213`）
- [x] `checks/task-24.md` にラウンド2の節を追記する（ラウンド1の記述は書き換えない）— ゲート1〜12 の再実行・ゲートA〜I・S-4 の `:widths:` 算出根拠・S-5 後半を対応不要とした理由・4観点ラウンド2の判定を記録済み
- [x] `checks/task-24.md` にラウンド3の節を追記する — `7ddc30f`。ゲート1〜12・A〜N の実行結果と4観点の triage を記録
- [x] 4観点のレビューを、それぞれ別のサブエージェントでもう一巡回す（**指示書はサブエージェントに渡さない**）— ラウンド2 実施。**4観点とも FAIL**。重複除去後の `must` 5件は `checks/task-24.md`「4観点レビュー ラウンド2」
- [x] 4観点のレビューをラウンド3 でもう一巡回す（指示書は渡していない）— QA=FAIL／設計=FAIL／クラフト=PASS／検証=FAIL。重複除去後の `must` 2件は是正済み（`checks/task-24.md`「4観点レビュー ラウンド3」）
- [x] commit & push — ラウンド3 は `443dccc`（本文）・`7ddc30f`（記録）。push 済み
- [ ] **user review**

**守ること**:

- 処理方式の名称は `glossary.md` §5.2 の正表記を使う（出典の呼称「バッチ処理」「メッセージ受信処理」等をそのまま持ち込まない）
- 「それ以外」「すべての処理方式」のような閉じた表現を使わない。**名指しするのは3処理方式だけ**で、残り3処理方式（`MOMによるメッセージング`・`HTTPメッセージング`・`テーブルをキューとして使ったメッセージング`）には触れない
- 差し替えた段落に `できず`・`できない`・`不可` を書かない（出典に不可能性の記述が0件のため）
- `about/index.rst:83-94` の内訳表（リクエスト単体テストの6処理方式）は変更しない（`design.md:90` の確定事項）
- 取引単体テストの処理方式ごとの一覧表を新設しない（`design.md:90` と衝突する）
- **変更してよいファイル**: `ja/.../about/index.rst`（`:58`・`:73`・`:75`・`:77-79` のみ）／`design.md`（`:84`・`:86` のみ。`:86` は段落を分けるため行数が増えてよい）／`mapping/style.md`（`:45-46`・`:53-54`・`:62-67`・`:69-73`・`:77`〜`:81`・**`:96-102`・`:189`**）／`mapping/glossary.md`（`:168` の意味欄のみ）／`reviews/page-about_index.md`（`## #24` 節のみ）／`checks/task-24.md`
- **変更してはならないファイル**: `mapping.csv`／`mapping/_batch/`／`volume.md`／`vocabulary.md`／`glossary.md` の `:168` 以外の全行（特に §5.15 の証拠一覧）／`ja/conf.py`／`about/index.rst` 以外の `ja/` 配下すべて／`steering.md`（コーディネータのみが更新する）／`reviews/` の `#24` 節より前の全行

**Completion criteria**: ラウンド1 のゲート1〜12 を再実行して全件 PASS、加えて `ntf-doc-24.md` §5-3 のゲートA〜N と §7-6 のゲートO〜U が全件 PASS で `checks/task-24.md` に記録されていること。要点は次のとおり。

- **ラウンド4 のゲートO〜U**: `ja/` の変更行が `about/index.rst` の2行だけ（O）／`design.md`・`mapping.csv`・`glossary.md`・`steering.md` の差分が空（S）／`style.md` の差分が `:5` の1行だけ（T）／`about/index.rst` に `このうち` が0件（Q）／ラベル `:79` の直後の段落が `3種類のテストのうち、` で始まる（R）／`style.md` が見出しで指す `design.md` の節が全件実在し一意（U、5件）。**ゲートP のみ指示の文言のままでは満たせない** — 「段落に `アプリケーション` 2回」は段落単位では3回（第3文の `Nablarchバッチアプリケーション`、変更禁止）、第2文単位では2回で指示どおり。第2文単位で PASS とし本文は変更していない（`checks/task-24.md`「指示との食い違い1件」）

- `git status --porcelain` の**全件**表で、上記「変更してよいファイル」以外が0件（ゲート1。`commit & push` の直前にもう一度実行する＝ゲート12・ゲートI）
- 出典8件を `git show 2e501ad:<path>` で開いて `file:line` と記述を突合し、`ntf-doc-24.md` §2-1 の表と一致（ゲート2。一致しないものがあれば**本文を書かずに報告する**）
- `about/index.rst` に「それ以外」「すべての処理方式」が0件（ゲートA）。差し替えた段落が名指しする処理方式が `glossary.md` §5.2 の正表記と逐語一致し、`MOMによるメッセージング`・`HTTPメッセージング`・`テーブルをキューとして使ったメッセージング` が現れないこと（ゲートB）
- 差し替えた段落が地の文であること（直前に `.. important::` が無く字下げ0）。`about/index.rst` の `.. important::` が Jakarta Batch・マルチスレッドの2件だけ（ゲートJ）。段落に `できず`・`できない`・`不可` が0件（ゲートK）
- `design.md:86` に書いた走査コマンドをそのまま実行してヒット行が一致し、リンクをたどって得た事実が grep のヒットとして書かれていないこと（ゲートL）
- `style.md:189` 相当行の用途欄に `モジュール一覧` が現れず、「根拠（拡張例の省略）」に `なし。` を書く実例が残っていないこと（ゲートM）
- `design.md:86` と `reviews/page-about_index.md` の `#24` 節に同一の文の逐語重複が無いこと（ゲートN）
- `design.md:84` と `about/index.rst:73`・`:75` が逐語一致し `APサーバ` が0件（ゲートC）。両ファイルの `実施方法` が0件で `実行方法` に統一（出典の引用箇所を除く。ゲートD）
- `style.md` に `design.md:[0-9]` が0件で、置き換え先の節見出しが `design.md` に実在し一意（ゲートE）
- `glossary.md` の差分が `:168` の1行のみで §5.15 の差分が0件（ゲートF）。`reviews/page-about_index.md` の変更行が `## #24` 節の内側だけ（ゲートG）
- `about/index.rst` の内訳表（導入文 `:81`／表本体 `:83-92`）が反映前後で `diff` 0件
- `verify_mapping.py` が `exit 0` で 595行 / 12,986 / 11,983 が不変、禁止ファイル群の差分0件
- Docker フルビルド（`-a`）が `build succeeded, 1 warning.`・既知の `db_double_submit.rst:108` のみ・新規0件。**直後に `git checkout -- locales/ja/LC_MESSAGES/sphinx.mo`**（ゲートH）

**`verify_glossary.py` はゲートに入れない**（`#22` と同じ扱い。`#pre-last` で一括是正する）。

**判断が要る場合**: `ntf-doc-24.md` §2-1 の実測と手元の実物が食い違った場合、および §2-2 の文言が `design.md` の他の確定事項（特に `:88`・`:90`・`:92`・`:94`）と両立しない場合は、**本文を書かずに報告する。範囲を自分で広げない。** 指示書が回答済みの5件を蒸し返す指摘は、回答の根拠（§2-1・§2-2）と食い違う**新しい一次情報を伴う場合にだけ**上げる。
...
- **Status**: paused
- **Date**: 2026-08-14
- **Last completed**: `#24` ラウンド4 の3行の是正（`66fe4c9`、push 済み）。ゲート1〜12・A〜N・O〜U を再実行し、ゲートP を除き全件 PASS
- **Next**: `#24` の **user review**（ラウンド4 の成果への `/rn:ty` / `/rn:gm` 判定）。承認で `#24` は閉じる
- **Notes**: 判定待ちに添えた報告1件 — ゲートP は指示の文言（「段落に `アプリケーション` 2回」）のままでは満たせない。段落単位では3回で、3件目は変更禁止の第3文の `Nablarchバッチアプリケーション`。第2文単位では2回で指示どおりのため第2文単位で PASS とし、本文は変更していない。承認後の次タスクは `#23` で、指示書 `指示/task-23.md` は未受領（受領するまで着手しない）
