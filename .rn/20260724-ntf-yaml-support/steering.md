...
### #116: 別セッションが承認なしに push した `de940451` を revert する（user 判断 2026-09-08「revert して」） — 済（直接コミット `9429bbda`、push 済み）

- 由来: 比較走行のセッション（ntf-excel-vs-yaml-2a）が、user の確認を取らずに `testdata_examples.rst`・`request_unit_test/web.rst`・`testdata_notation.rst` の 3 ページへ +9/−7 行を push した（`de940451`）。台帳エントリなし。
- 却下の理由: 「`routes.xml` の `:action` に当たる Action のメソッド名がそのままリクエスト ID になる」は同じ nablarch-example-web に反例がある（`AuthenticationActionRequestTest/indexNormal.yaml:15` は REQUEST_ID `login`・GET で `routes.xml` は `<get path="/action/login" to="Authentication#index"/>`）。NTF の `REQUEST_ID` は baseUri に連結する URI の末尾（nablarch-testing `d309471` `AbstractHttpRequestTestTemplate.java:433`）で、その説明は `web.rst:112` に既にある。「命名規約を採るプロジェクトの例」は HttpMethodBinding の `"do"`＋リソース名の規則（nablarch-fw-web 2.3.0 `HttpMethodBinding.java:100-107`）でプロジェクトの規約ではない。
- user 方針: リクエスト ID の決め方は PJ 次第で解説書には書かない。比較走行の指示書側で明示する。`testdata_notation.rst` の持ち越しの 1 文（実装と矛盾なし）も今回は入れない。
- 結果: 本文は `4e076fba` と同一。レビューは回さない（本文差分なし）。
...
- **Status**: paused
- **Date**: 2026-09-08
- **Last completed**: `#116` 承認なしに push された `de940451` の revert（`9429bbda`）。その前は `#115` 記法ページの「データ（1件以上）」の訂正とスキーマ監査で見つかった規則の追記（`54bc76a5`）。それ以前は各タスクの見出し（`#57`〜`#114`）に記録。
- **Next**: 解説書側の作業は残っていない（user のページ別レビューは 2026-09-07 に全ページ完了）。残る user 判断は 2 件で、user が話を出すまでこちらから出さない: ①HTML 配布方法（推奨 zip。`03-検証スクリプト.md` §13）②PR #728 のマージとリリース（`#29`「マージ直前にまとめて処置」と一体。他者の未マージ PR 9 本の扱いを含む）。モジュール側は rest task #3 の user review 待ち（`nablarch-testing-rest@cbad873`）。
- **Notes**: ブランチ `ntf-yaml-support`、`origin` に push 済み。本文は `4e076fba` と同一。解説書への反映は user 承認と台帳エントリを前提とし、他セッションからの直接 push は受けない（`#116`）。モジュール側の一次記録の所在は台帳 `#52`〜`#54`・`#115` の各「根拠」にある。
