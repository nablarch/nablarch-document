...
- [ ] 2c. **レビュー。未実施。次のセッションはここから始める**（user 指示 2026-08-25
  「再開後にレビュー回しましょう」）。公開本文に新しい記述が入ったため、`nablarch/CLAUDE.md` 3-1
  により回す対象になった（Step 1・Step 2 の削除だけの範囲では回していなかった）。
  回し方は下の「Step 2c の回し方」を読むこと
...
| C 規約 | `mapping/style.md` の11観点、`mapping/glossary.md` の正表記。新設した3つの節見出しの語と、既存の節見出しのトンマナが揃っているか |
...
- **Step 2c のレビューが未実施。** `fcf51c5` で公開本文に新しい記述（記載例3節）が入ったため、
  `nablarch/CLAUDE.md` 3-1 により回す対象になる
...
- **Status**: paused
- **Date**: 2026-08-25
- **Last completed**: `#36` Step 2・2a・2b（`implementation/testdata_examples.rst` の基準によるノイズ除去と、特殊記法の記載例4項目の追加、その波及として `implementation/testdata_notation.rst` の改行文字2箇所の是正）。Step 1 と合わせて `#36` は計20コミット。ディレクターが直接コミットしており、CC への指示書はない。**修正の根拠は各コミットメッセージにある**
- **Next**: `#36` Step 2c（**レビュー**。user 指示 2026-08-25「再開後にレビュー回しましょう」）。`fcf51c5` で公開本文に新しい記述（記載例3節）が入ったため、`nablarch/CLAUDE.md` 3-1 により回す対象になった。**着手手順は `#36` エントリの「Step 2c の回し方」に書いてある。まずそこを読むこと。** あわせて `#36` の「確定した基準」と「`#35`・`#33` の決定を上書きした箇所」も読む。過去の記録（`#35` 是正1〜6、`#33` (a)）と現在の本文が食い違って見えるのは、基準の確定によって意図的に上書きしたためである。`#33` (b)（png 26枚の禁止語点検）・(e-1) と `#34` は変わらず残る
- **Notes**: ブランチ `ntf-yaml-support`、最新コミット `381cbdc`、作業ツリーはクリーン、未追跡パスなし。**`origin` とは同期していない。`#36` の21コミット（本 State の更新を含めると22）が未 push で、`origin/ntf-yaml-support` は `d86bb59` のまま**（push の可否は user 判断待ち）。番号付きタスクは 1〜32・35 が Closed/DONE、`#33`・`#34` が未決、`#36` が進行中。`main` へのマージと `.rn/` の扱いは user の明示指示待ち（`Rules` 末尾）
