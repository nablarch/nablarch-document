...
- [ ] `about/index.rst:73`・`:75`・`:79` を `ntf-doc-24-round2.md` §2-2 の確定文言に差し替える。`:58` の `:widths:` を新しいセル文言の実測比で決め直す（数値の根拠を `checks/task-24.md` に残す）
- [ ] `design.md:84`（実行方法セル・備考セル）を `about/index.rst:73`・`:75` と逐語一致させる（`APサーバ` を使わず `アプリケーションサーバ`）。`:86` を §2-1 の実測に差し替え、§2-3 の4点＋§3-2 の申し送り1文を含める
- [ ] `style.md` の残り10件の `design.md:NN` 行番号参照を節見出し参照に置き換える（S-7）。`:77` を `:45-46` に合わせる（S-8）。`:53-54`・`:62-67`・`:80-81` を `design.md` の「モジュール一覧は第1部に置かない」に合わせる（S-9）。`:69-73` の「本文に『なし。』とだけ書く」を削る（S-10）
- [ ] `glossary.md:168` の**意味欄のみ**を、実施方法を含まない定義に改める（S-6。揺れ表記列・別義列・採用根拠列は変更しない）
- [ ] `reviews/page-about_index.md` の `## #24` 節を整理し、公開本文の最終形と出典の実測結果だけを残す（S-11。`style.md` の監査結果は `checks/task-24.md` へ移す）
...
- [ ] `checks/task-24.md` にラウンド2の節を追記する（ラウンド1の記述は書き換えない）。ゲート1〜12 の再実行結果とゲートA〜I、S-4 の `:widths:` 算出根拠、S-5 後半を対応不要とした理由を記録する
- [ ] 4観点のレビューを、それぞれ別のサブエージェントでもう一巡回す（**`ntf-doc-24-round2.md` はサブエージェントに渡さない**）
- [ ] commit & push
- [ ] **user review**
