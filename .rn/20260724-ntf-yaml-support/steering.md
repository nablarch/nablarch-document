...
2026-08-06、再構成指示`ntf-doc-09-restructure.md`（STEP A〜G）を受領し実施。L2「テストデータの構造」新設（`mapping.csv`の`dest_section`19行を再割当）・各L3の「共通→形式別」統一・`must`2件/`note`1件の解消・`style.md`の規約不整合の解消（S-03例外、S-10順序規約、S-11新設）。4観点レビューを**3ラウンド**回し、修正を3回実施（`51f8020`→`ea4b5d3`+`65fc852`→`d8089aa`+`c94a870`→`fed3705`）。**「形式別L4配下の本文を共通部へ移す際に語彙を形式非依存に直し忘れる」型の欠陥が3ラウンド連続で再発**し、うち事実誤りが3件（`no`の説明・レコード種別の扱い×2）。最終的に形式非依存領域を機械走査する検査を導入して収束させた。詳細は`checks/task-09-restructure.md`・`reviews/page-testdata_notation.md`「ラウンド4〜7」参照。**user review未了**（判断を仰ぐ事項3件を`checks/task-09-restructure.md`末尾に記載）。
2026-08-07、user reviewで**差し戻し**（`must` 1件: レコード種別の形式差が共通部に未明記）。是正指示は`ntf-doc-09-recordtype.md`。共通部に`important`を1件追加し、あわせて手順3の横並び確認（形式別L4対を持つ8つのL3の突合）で同型の未明記の形式差を1件自発検出して`important`をもう1件追加した（テーブルデータの行内カラム値未記載時: Excelは空文字補完・YAMLは`null`）。形式別L4の既存記述は無変更（`git diff`は挿入8行・削除0行）。判断を求められた3件（`design.md` §4の上書き・内容追加2件の例外・`style.md` S-07の裁定）はいずれも承認済み。ゲート8件を実測で確認（詳細は`checks/task-09-recordtype.md`、レビュー記録は`reviews/page-testdata_notation.md`「ラウンド8」）。**user review未了**。
...
- **Status**: paused
- **Date**: 2026-08-07
- **Last completed**: #8 第1部「テスティングフレームワークとは」の作成。`#9`（テストデータの書き方）は差し戻し是正指示 `ntf-doc-09-recordtype.md` を適用し push 済み（`b7e97b3..73e84dc`）だが、user review 未了のため未チェック。
- **Next**: `#9` の user review 判定（`/rn:ty` または `/rn:gm`）を受ける。承認後は `#10`「テストデータの記載例」（見出しスキャフォルドへの本文追加）へ進む。
- **Notes**: ブランチ `work`、`origin`（`lovaizu` fork）と同期済み・PR未作成。ツリーはクリーンで未追跡パスなし。ブロッカーなし。**user review で判断を仰ぐ事項1件**（作業指示は「本文への追記1件のみ」と定めるが、手順3の横並び確認で検出した形式差1件も追加したため `important` が2件になっている。不要なら `テーブルのデータを記述する` 共通部の `important` を落とせば1件に戻る）は `checks/task-09-recordtype.md` 末尾に記載。`#10` 以降への申し送り15件は `reviews/page-testdata_notation.md` 末尾。`_build/` はユーザーがブラウザで直接レビューするため今後も削除しない。Docker ビルドは `locales/ja/LC_MESSAGES/sphinx.mo` を再生成するため、commit 前に戻すこと。
