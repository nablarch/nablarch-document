...
- **Next**: **PR #728 の差分の確認が先（user 指摘 2026-09-03「NTF 以外の変更差分が多数」）。** GitHub は PR の base を `eb1ea136`（2025-03-27）で固定しており、分岐点 `7cdfd4f5` からの 571 ファイルを表示している。現在の `origin/main` との三点比較は 468 ファイルで、NTF・`.rn/` 以外は `README.md`・`_static/custom.css`・`ja/biz_samples/04/0401_ExtendedDataFormatter.rst` の3つだけ（実測は案件フォルダ `01-現在地.md` 2026-09-03 朝②）。再開後に取り直して user に報告し、`origin/main` のマージ（user 判断）を諮る。**次に `review-map.md`（`#81`）の user レビュー待ち。** 形・粒度・「未確認」の扱いの判断を受けて直す。その後、user の刷新版38本全量読みレビューの続き（質問が来たら実物で答える）。`#29` の残り（「マージ直前にまとめて処置する」の台帳）は user のマージ判断まで着手しない
