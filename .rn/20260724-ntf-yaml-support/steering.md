...
- **Status**: paused
- **Date**: 2026-07-28
- **Last completed**: `/rn:ty`により`#5c`（差し戻し対応後）user reviewを承認として記録し、`#5d`（記録の整合とセクション境界の是正、STEP1〜8）を完了。STEP1-5: `split-plan.md`にinput-0016/0030の分割記録を追記、`checks/task-05.md`に暫定27行の一覧をグループ化、`design.md`§12未確定事項#3の確定時期のズレをchecks/task-05d.mdに記録（`fa30fef`）。STEP6: `verify_mapping.py`に`check_reference_only_sections`を追加、advisory2件（HTTPメッセージング処理方式）を実測確認（`5dcaad1`）。STEP7: `check_intro_section_split`を追加（比較対象は`ntf-doc-05d-addendum.md`のタプル表記ではなく`dest_section`単独に変更 — steering.md #5の既存ルール文言と一致させ、design.md §4記法統合方針による正当な分離ケース4件の誤検出を回避。理由はchecks/task-05d.md STEP7参照）。ERROR2件（current-0150/current-0269）を実ファイル通読のうえ機能概要→使用方法に是正、advisory4件にnote追記、是正の副作用で0件化した1セクションをPENDING_ZEROへ登録（`9510a18`）。STEP8: `#8〜`への引き継ぎ済みを確認、self-check完了・全Completion criteria機械検証OK。旧基準値（591行/11,973、design.md/mapping.csv/_batch差分なし）が#5c確定値・addendum指示と整合しないstaleな記述だったため593行/11,983・「design.mdのみ差分なし」に修正（`7198252`）。`verify_mapping.py`は593行・lines合計12,986・DROP除く11,983・エラー0件。
- **Next**: ユーザーによる`#5d`のuser review verdict待ち（サブエージェントによるレビューは実施しない）。`/rn:ty`（承認）→ `#5d`のuser reviewチェックを埋めて`#6`（未確定事項の確定とdesign.md更新）に着手。`/rn:gm`（差し戻し）→ 指摘に対応し`#5d`を再度commit&pushして再度user reviewへ。
