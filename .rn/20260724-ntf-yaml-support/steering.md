...
### #110: 〈エンティティ単体テスト〉の user レビュー FB 3 件（題に対象クラスを添える・列挙 2 箇所の箇条書き化）（2026-09-07 朝） — 済（直接コミット `3d748c59`・`287401a6`・`adb31761`、push 済み）

- FB 1「Formクラスも対象なの？エンティティ単体テストというタイトルから分からないのでタイトルに入れませんか」→ 題を「エンティティ単体テスト（Formクラス・Entityクラス）」に（`3d748c59`）。区切りは `#92`・`#100` の題に合わせて「・」。本文 `entity.rst:13` と一致。〈エンティティ単体テストの設定〉（`setup/entity_unit_test.rst`）の題は据え置き（設定項目の登録ページで、対象クラスは本ページで説明する）。
- FB 2「列挙してる部分、箇条書きにしない、読めない」（テストショットの例）→ 5 件を「見出し（入力と結果）→ 字下げした説明（確認する観点）」の箇条書きに（`287401a6`）。観点は v6 の表（`2e501ad` `01_entityUnitTestWithBeanValidation.rst:78-87`）から。
- FB 3（同、`BasicJapaneseCharacterGenerator` の文字種）→ 14 件を箇条書きに（`adb31761`）。名前と順序は実装 `nablarch-testing@dcaed44` `BasicJapaneseCharacterGenerator.java:42-55` と一致。
- 検証: docker 増分ビルド warning 0。HTML `entity.html` に新しい題 3 件、`implementation/index.html` の目次に 1 件、`<li>` 14 件と観点の説明 5 件を確認。
- 追随: `review-guide.md` の見出しと「変えたことと理由」、`design.md` のページ一覧。

### #111: 〈コンポーネント単体テスト〉の user レビュー FB 2 件（題に対象クラスを添える・JUnit 4 への誘導 tip の削除）（2026-09-07 朝） — 済（直接コミット `15ab48af`・`70e7e18c`、push 済み）

- FB 1「こちらもページタイトルからActionクラスとComponentクラスが分からない」→ 題を「コンポーネント単体テスト（Actionクラス・Componentクラス）」に（`15ab48af`。`#110` と同じ型。見出し下線を延長）。本文 `component.rst:13` と一致。
- FB 2「Junit4への補足や参照を入れるのはやめませんか？…Junit4のページあるので、冒頭以外は全ページから参照を外して」→ `component.rst:111-115` の tip（2 段落とも JUnit 4 の話）を削除（`70e7e18c`）。`#107` の 6 ページ削除後に残っていた最後の 1 件。全 rst を走査し、JUnit 4 への言及は `about/index.rst:116`・`setup/standard_usage.rst`・`setup/index.rst`・`setup/junit4.rst` だけであることを確認（実装・ツールのページは 0 件）。
- 検証: docker 増分ビルド warning 0。HTML `component.html` に新しい題 4 件、`implementation/index.html` の目次に 1 件、`junit4`・`JUnit 4` 0 件。
- 追随: `review-guide.md` の見出しと「変えたことと理由」、`design.md` のページ一覧。

### #112: リクエスト単体テスト・取引単体テストの実装ページ 12 本を実装で動かして検証し、誤り 10 件・曖昧 28 件を是正（user レビュー 2026-09-07「全ページ、書いてること正しいですか？裏取り済みですか？」×2） — 済（直接コミット `02762128`〜`b57c5678` の 33 件、push 済み）

- 由来: 12 本とも裏取りは v6・実装ソースの読み合わせ（`file:line`）までで、コード例・挙動を動かしてはいなかった（実行検証済みは `#97`・`#108` だけ）。`#108` と同じ方式で、サブエージェント opus 6 本（web／rest／batch＋db_queue／mom＋http_messaging／deal-batch＋db_queue／deal-others 4 本）に、ピン（`nablarch-testing@ae989ec`・`junit5@c06ebe8`・`rest@9ada31e`・`yaml@d50ee2b`・Example ウェブ `b5ed1fe`・Example バッチ `6981157`）の上に解説書のコード例とテストデータをそのまま置いて実行させ、期待値をずらすと落ちることまで確かめさせた。指示書と report 6 本は `review-guide-notes/verify-rut/`（ディレクター作業フォルダ）に保全。指摘はすべてディレクターがピンのソースで裏取りしてから是正した。
- 件数: 主張 346（web 84／rest 55／http+mom 43／batch+db_queue 59／deal-batch+db_queue 35／deal-others 70）。一致 303・誤り 10・曖昧 28・未確認 5（Example に題材が無い: web の `expectedSearch`・`expectedMessage`・メール送信要求、DB キューの実アプリ、rest のアーキタイプ pom）。実行で確かめた主張 217。
- 是正（重いもの）: (1) `request_unit_test/web.rst` 「実行後の HttpRequest でリクエストパラメータの書き換えを確認する」節を削除（取り出せるのは送信前のオブジェクト。v6 由来で v6 はコンパイル不能だった。`c28ad0fe`）。(2) `request_unit_test/rest.rst` `setBody` の文字列は既定設定で JSON 文字列リテラルになる（刷新で新規に書いた誤り。`19f7ea69`）。(3) `deal_unit_test/batch.rst` 例1・例2 の途中ショットの `setUpTable=default` を空欄に（全件 DELETE→INSERT で前ショットの結果が消える。`02762128`）。(4) `deal_unit_test/mom.rst` 応答不要受信のテストクラス欠落（`cc670afa`）、`log.properties` 例の `writerNames`・`availableLoggersNamesOrder` 欠落（v6 由来。`3f36462d`）。(5) `request_unit_test/mom.rst` `MQSupport` の役割と構成図（`42e05386`、PNG 再生成）、`TestDataConverter` の参照先（`ebde2c24`）。(6) `request_unit_test/http_messaging.rst` 要求電文ヘッダの値は照合されない・読み替え表にカラム名（`8fd7f1ed`）。(7) `setup/request_unit_test/web.rst` HTML リソースのコピー先（`067008e1`）。(8) `testdata_notation.rst` setUpDb は `execute` の呼び出しごと・`no` は連番に限らない・グループID併用の「両方有効」はテーブルのみ（`04544112`・`0a5e30f4`・`7ebebcda`）。ほか web 7・batch 2・deal-mom 3・deal-web 1・deal-http 1。
- 却下・据え置き: web A9 のツール tip は「Excel 形式で得られる」の 1 文だけ足し、Windows 限定は参照先（`request_data_tool.rst:27`）に任せた（ツール 2 ページの扱いは `#94` の未決と一体）。deal-others の「REST の `getBodyString`・`getBodyStream` に触れていない」「`assertStatusCode(String,int,HttpResponse)` を示していない」は誤りでないため据え置き。
- 検証: 各指摘をピンのソースで裏取り（`file:line` はコミットメッセージ）。docker 増分ビルド warning 0（途中で例2 の列数不一致 2 件を検知し `2b9b469f` で是正）。HTML で新旧の文言と、`request_test_components1.png` の md5 一致を確認。差分全量をディレクターが通読し、`#103` の型に反する括弧書き 1 件（`b99d355a`）と Advice 一覧の配置 1 件（`b57c5678`）を直した。
- 申し送り（モジュール側・範囲外）: (a) Example ウェブ `b5ed1fe`（テストデータの誤り。モジュールではない）: YAML の `requestParams` を `rows: [- {}]` と書いた 33 ファイルが、yaml モジュールの空行スキップ（`fb58781`、2026-08-24）以後は 0 行になり `Request parameter is not defined` で失敗する（実測 `ProjectActionRequestTest#showNormal` Errors: 1）。解説書どおりのマーカーカラムだけの行 `- "[no]": "1"` に直すと通る（実測 Errors: 0）。yaml モジュールと解説書（`testdata_examples.rst:757-768`）は正しい。Example ウェブの CC（task #5 レビュー待ち）に 33 ファイルの置換を出す。(b) 本体: `AbstractHttpRequestTestTemplate` の javadoc の実行順（beforeExecuteRequest → トークン）が実装と逆。(c) 本体: `nablarch.test.skip-resource-copy` の判定ディレクトリ `<htmlDumpDir>/htmlResources` を本体が作らないため、利用者が作らない限り抑止が効かない。(d) 本体: HTTP メッセージ送信の要求電文アサートは `diConfig` のリポジトリが戻った後に走るため `format` の基準パスが既定リポジトリ側に要る（設定ページ `setup/request_unit_test/http_messaging.rst` に未記載）。(e) 設定ページ: `<list name="handlerQueue">` の後置き上書きが効かない現象（機構未確認）。(f) 記法ページ `command_line` 節: 値が空のカラムもコマンドラインオプションとして Action に渡る。(g) 本体: HTTP 側の要求電文ヘッダのアサートがコメントアウト（`RequestTestingMessagingClient.java:406-411`）。
- 追随: `review-guide.md` の共通節と該当 11 ページの「変えたことと理由」。

### #113: 残っていた申し送りのうち解説書側の 6 件を閉じる（user 2026-09-07「台帳や申し送り、残ったら困る。何が残ってるの？」） — 済（直接コミット `fe650381`・`218e9f3f`・`52e49e65`・`df9a31c4`・`acf3d1ce`、push 済み）

- `#102` の残り: `request_unit_test/mom.rst` に `execute(String, boolean)`（テストショットごとの setUpDb 再投入）を書いた（`fe650381`）。
- `#112` (d): `setup/request_unit_test/http_messaging.rst` に、要求電文の検証はテスト対象の実行後（`diConfig` の設定が戻った後）に行われるため `format` の基準パスをテスト用のコンポーネント設定ファイルに登録することを書いた（`218e9f3f`。根拠 `TestShot.java:80-88`・`MainForRequestTesting.java:20-26`・`RequestTestingMessagingClient.java:392-393`@ae989ec）。
- `#112` (e): DI コンテナで実測（scratchpad `diprobe`）。import 後に同名の `<list name="handlerQueue">` を置いても要素は先の定義のまま、同名の `<component>` は後勝ち。`setup/request_unit_test/batch.rst` に「list を同名で再定義しても上書きされない。差し替えはハンドラのコンポーネント名で行う」を書いた（`52e49e65`）。
- `#112` (f): `testdata_notation.rst` のコマンドライン節に、オプションになるカラムの範囲（テスティングフレームワーク用の 5 カラム以外すべて）と空欄でも渡ることを書いた（`df9a31c4`）。
- `#87` (i): `#108` の実測（`review-guide-notes/verify-examples/report.md:175-180`・`:243`。Excel `Null`／YAML アンクォート `null` → Java null、`"null"` → 文字列、両形式一致）と、変換ツールが明示 null をアンクォートで書く（`YamlFormatWriter.java:437`、`XlsFormatReader.java:481`@9ab6648）ことで、変換ツールページの「往復で意味が保たれる」と記法ページの空文字／null の規則は整合している。解説書の変更なしで閉じる。
- `#87` (j): `html-check-config.csv` はブランクプロジェクト（`nablarch-single-module-archetype` `nablarch-web/src/test/resources/nablarch/test/http-request-test/`）に含まれ、デフォルト設定 `http-request-test.config:5` が同じパスを指す。`tools/html_check_tool.rst` に書いた（`acf3d1ce`）。
- 残る申し送りはモジュール側だけ: Example ウェブ (a)、converter／yaml (k)、本体 (b)(c)(h)(g)。`#112` に記録済み。user 判断待ち: `#94` ツール 2 ページの Eclipse 手順／HTML 配布方法／PR マージとリリース（junit5 PR #12）。
- 検証: docker 増分ビルド warning 0。HTML で 5 箇所の文言を確認。

### #114: ツール 2 ページの Eclipse 起動手順を落とす（`#94` の未決。user 判断 2026-09-07「1 で OK、ただしレビューガイドで伝えてね」） — 済（直接コミット `2418ea13`、push 済み）

- `tools/request_data_tool.rst`: 「Eclipseから起動できるように設定する」節と Eclipse からの右クリック起動（画面 4 枚）を削除し、コマンドプロンプトから `httpDump.bat` に HTML ダンプの絶対パスを渡す起動に書き換えた。絶対パスとする根拠は `downloads/request_data_tool/httpDump.bat` が `cd /d %~dp0` の後に `%1` をそのまま使うこと。
- `tools/master_data_tool.rst`: 「AntビューにAntビルドファイルを登録する」節と Ant ビューからの実行（画面 4 枚）を削除し、`ant -f master_data-build.xml [ターゲット]` の実行に書き換えた。前提事項を「EclipseとMaven」から「AntとMaven」に変えた。根拠は配布物 `master_data-build.xml` が素の Ant ビルドファイルであること（`basedir="."`、`default="マスタデータ投入"`）。`#103` で残していた `:110`「（蟻の図柄に…）」も節ごと消えた。
- 判断の理由: `#94` で決めた「IDE 固有の手順は本文に置かない」と揃える。ツール自体は Eclipse に依存しない。トレードオフ（Eclipse を使わない場合は Ant を別途入れる）は user に提示済み。
- `review-guide.md` の両ページの「変えたことと理由」に追記（user 指示）。
- 検証: docker フルビルド（`-a`。節の題を変えたため）warning 0。HTML の `request_data_tool.html`・`master_data_tool.html`・`tools/index.html` に Eclipse 0 件、`ja/` の rst に Eclipse・Antビュー 0 件、HTML が参照する画像は `tool_components.png` のみ。

### #115: 記法ページの「データ（1件以上）」の訂正と、YAML スキーマ監査で見つかった規則の追記（user 承認 2026-09-07「進めて」） — 済（直接コミット `54bc76a5`、push 済み）

- 由来: `#97` 申し送り (k)（変換ツールの `rows: []` を yaml が拒否）→ converter `ntf-step4-17` の調査（`nablarch-testing-converter@6a87afa` `checks/task-56.md`）→ ディレクターの独立検証 → user 指摘「コーパスを変換して探すのは網羅でない」→ **スキーマの拒否制約 77 箇所を母集合にした監査**（サブエージェント 3 本。証拠は作業フォルダ `review-guide-notes/audit-schema/`）。結果は OK 63／GAP 13／対応物なし 1。GAP 13 のうち直すのは 2 種類（`rows` の `minItems: 1`、ディレクティブ 7 個の integer／boolean 型）。電文 `records` の `minItems`（3）は解説書に無い書き方、送信同期 `group_id` 空文字（2）は本体の癖（`:453`「空欄なら失敗」と定めている）のため直さない。
- **上書き**: yaml `#49` A（`nablarch-testing-yaml@f3620fc`、user 確定 2026-08-31）の根拠だった「データ（1件以上）」（`:907`）は `a0d09aa1` で新規に書かれた文で、v6（`origin/main`）にも本体にも根拠が無かった。user 判断 2026-09-07 で `#49` A を取り消す（yaml 側の是正は `指示/ntf-step4-18-schema-excel-parity.md` §2、台帳 `#51`）。
- 変更（`54bc76a5`）: `:907` を「0件以上」に。ファイルの規則に「データ行を書かないと 0 行のファイルになる」（本体 `createLayout()` records=1・0 バイト。`records: []` との違い）。電文の規則に「データ行を書かない期待要求電文は電文が送信されないことの検証」（`RequestTestingMessagingClient.java:337`・`:357`・`:366`、`SendSyncSupport.java:67-69`・`:82-84`@ae989ec。本体のテストデータ 27 ファイル 44 ブロックがこの形）。YAML のディレクティブ値は整数・真偽値でも文字列でも可（`PoiXlsReader.java:123`、`DataFile.java:294-334`@ae989ec、`YamlFormatWriter.java:354-362`@878ef9a）。`:108` に数字だけの構造値のクォートを追記。`fw_header` の値の規則を追加。
- 申し送り（今回は直さない）: 変換ツールは Excel→YAML 書き出し時にスキーマ検証をしない（`YamlTestDataValidator` は読み経路のみ@878ef9a）／スキーマは電文に可変長用ディレクティブを許すが本体は固定長 11 個のみ（`MessageParser.java:54-58`@ae989ec。スキーマが緩い方向）／本体は `expectedMessage` 空欄で `[]` 付きブロックを引く（`RequestTestingSendSyncSupport.java:157`@ae989ec）。
- モジュール側の進行（2026-09-07 夜、すべて完了・ディレクターが scratchpad の clone で独立実測して承認）: yaml `#51`（`f9373ee`、B-1 反映 `a404126`。328 件緑、構造差分 8、旧スキーマで新テスト RED）→ converter `#57`（`223f954`、src は `878ef9a` と同一。732 件全緑。保留だった `#55`＝`ntf-step4-14` §3 も併せて承認、記録 `4a59436`）→ Example ウェブ task #7（`b12b525`。151 件緑）→ integration `#26`（`1fcd2f9`。546 件緑・Skipped 18、承認記録 `5d3c573`）。
- 検証: docker 増分ビルド、本文 warning 0。HTML で 6 箇所の文言を確認。記載例ページは変更なし（`#108` ハーネス再実行は不要）。
