...
- [x] 是正ラウンド2の残り — C-1〜C-8 の記録反映（`checks/task-terminology.md` §11）とゲート1〜17 の再実行
- [x] 是正ラウンド2の再レビュー（4観点）→ 設計 FAIL・検証 FAIL
- [x] 是正ラウンド3 — 上記への是正。本文8件・`glossary.md` 3件・記録10件（§12）。再レビュー（4観点）→ クラフト FAIL・設計 FAIL
- [x] 是正ラウンド4 — 再々レビューで判明した**ラウンド3の退行**の是正。本文4件・記録8件（§13）。**ラウンド上限3を超過**。理由は `checks/task-terminology.md` の Overall Verdict を参照
- [x] ゲート1〜13、追補のゲート14（`about/index.rst` の差分が L24 の1行のみ）、回答のゲート15〜17（`design.md` の `テストケース` 0件／導入文と見出しの語の一致／10種の突合表）をすべて実行結果で確認し記録する（各ラウンドで全件再実行。最終は §13-5）
- [x] commit & push（`6cc01b7`・`6e63e27`。記録ファイルは check-off 時にコーディネーターがコミット）
