...
- **Status**: paused
- **Date**: 2026-08-26
- **Last completed**: `#36` Step 3 の論点13（0件テーブルが次ブロックを食う。user 判断「現行通り」。カラム名を持たない0件テーブルの書き方を記法と記載例に追記）。その前が JUnit 5 拡張の `TestRule` 再現（`setup/junit5_extension.rst` を書き直し `TODO(NTF-MOD-03-1)` を削除。Step 4 へ2件申し送り）。その前がマスタデータ投入ツールの YAML 対応（`tools/master_data_tool.rst` を書き直し `TODO(NTF-MOD-02-4)` を削除）。その前が論点12（`TODO(NTF-MOD-01-3)` を削除）と業務サンプルからの参照の是正。その前が論点11（グループIDに使える文字。本文変更なし）・論点7（識別子の大文字化。本文変更なし）。その前が論点6（空行スキップ。論点3 を含む。`5301d6e`）。**詳細は `#36` エントリの Step 3 の表と各コミットメッセージにある。State には繰り返さない**
- **Next**: `#36` Step 3 の続き。**次は `implementation/deal_unit_test/mom.rst` の `TODO(NTF-MOD-02-3)` と `tools/testdata_converter.rst` の `TODO(NTF-MOD-01-2)` を、本文の書き直しが不要であることを実物で確かめたうえで削除する。** **`ja/` に残るこの2件は、いずれも参照点である PR ブランチでは既に対応済みである**（`NTF-MOD-02-3` = `nablarch-testing` `3c4bd2a` の `SendSyncSupport` のタイムスタンプ走査、`NTF-MOD-01-2` = `nablarch-testing-converter` `5ab13d8`）。**論点13 は決着したため、残るのは `ja/` の `TODO(NTF-MOD-01-2)`・`TODO(NTF-MOD-02-3)` の削除2件のみで、そのあとは png 26枚（user 担当）だけになる。** **Step 4 へは、図以外の残作業がすべて終わるまで進まない**（user 指示 2026-08-26）。 残作業の唯一の一覧は `~/work/cowork/nablarch/ntf-doc-renewal/01-現在地.md` の「このPRの残作業」にある。**本文に新しい記述が入るときは、書く前に実際に走らせて出力を確かめること。** `#33` (b)（png 26枚の禁止語点検。user 担当・このPRで実施）・(e-1)（見送り確定）と `#34` は変わらず残る
