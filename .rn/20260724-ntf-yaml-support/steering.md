...
- [x] フィードバック対応（2026-08-05、`/rn:gm`、4点）を反映。
      (1) 「全体像」冒頭の3文をユーザー自身が段落分割済み（空行区切り、段落内改行ではないため既存Rulesに抵触しない）で維持。
      (2) 「テストコードは定型かつ少量で済む」に外部化の事実（「テストコードとは別のファイルに宣言的に記述する」）と、
      Nablarchが対象とする基幹システムの規模（テーブル数・項目数の多さ）が外部化を欠かせないものにする、という理由づけを追加。
      旧「テストデータ」節の`current-0163`/`0176`を`dest_section`変更（テストデータ→全体像）で本特徴点に統合。
      (3) 「アーキテクチャ、アーキテクト向けだけでよいのでは」との指摘を検討し、design.md §1の読者定義
      （第1部=両読者の共通前提）・直前セクションの裏付けという位置づけ・既に図＋導入文のみという最小構成、
      の3点から維持と判断（design.md該当節に理由追記）。
      (4) 「テストデータ」節（H2見出し）を廃止。独立性（`current-0175`、マスタデータ投入ツール・マスタデータ復旧機能の
      tip含む）は、記法仕様と同じ理由（「これは何か」ではなく「どう書くか」）で第3部「テストデータの書き方」へ
      `dest_part`/`dest_page`/`dest_section`ごと再割当。マスタデータ投入ツールの非推奨懸念は、現行解説書・input資料に
      非推奨の記録がなく`mapping.csv`上も第4部の確定ページであることを確認し、非推奨ではないと判断。
      `design.md`§2・`mapping.csv`・`volume.md`・`mapping/vocabulary.md`・`verify_mapping.py`（`SECTION_TEMPLATE`から
