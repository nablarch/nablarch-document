...
- **Status**: paused
- **Date**: 2026-07-28
- **Last completed**: `/rn:gm`差し戻しを受け、`checks/task-06-proposal.md`（初版）を訂正し`checks/task-06.md`として再提示（`steering.md`/`mapping.csv`/`design.md`はまだ無変更、`#6`のStepsは未着手）。①③（稼動環境0件の案A・ページ分割/取引単体テストの構成）は独立検証で裏付け済みのため結論不変。②（テストデータの形式0件）は数値を訂正（YAML言及行は方法により90〜206行と方法依存であることを明記し、方法論上の欠陥（inputパス由来の`yaml`部分一致による水増し）を指摘した上で、結論（第2部ページ廃止）はYAML集計に依存しない独立の事実であることを明記）、「使い分け」出典なしを確認し表現を訂正。③に未処理2件を追加: (a)取引単体テストの設定（RESTfulウェブサービス）機能概要0件を含む第2部設定ページ機能概要10件/拡張例8件の一括判断（a/b案）、(b)第3部取引単体テストの小規模3ページ（HTTP33/RESTful32/ウェブ48行）の統合要否（A/B案）。`PENDING_ZERO`26件全件を8分類に仕分けて`checks/task-06.md`に記載（分類1-3は①②③の承認で解消、分類4-8はユーザー判断が必要な新規論点）。④はファイルパスの省略記法をやめ、`vocabulary.md`の全38ページを1行ずつファイルパス・行数に対応付けた表で再提示。
- **Next**: ユーザーが`checks/task-06.md`の提案を承認または修正指示したら、同ファイル末尾の「進め方」（design.md更新→mapping.csv更新→暫定表記解消→reference-only sections判断→self-check→commit&push→user review）に着手する。
- **Notes**: branch/PR: `lovaizu/nablarch-document`の`work` = PR #730（`nablarch/nablarch-document`）のhead。base commitは`c24190607fef5d76c607aa08b36d2ab2f813efe5`。作業指示: `.rn/20260724-ntf-yaml-support/ntf-doc-05b-instruction.md`、`#5d`追補: `ntf-doc-05d-addendum.md`。push権限は解決済み（`kiyobot`=`GH_TOKEN`がwrite権限を保有）。open blocker: `checks/task-06.md`（`888daf2`で確定・push済み）へのユーザー判断待ち（`/rn:ty`/`/rn:gm`ではなく通常のユーザー返信で応答される想定）。判断対象は①稼動環境0件のA案採用可否、②テストデータの形式ページ廃止・design.md §3表からの削除可否、③-未処理1（第2部設定ページの機能概要10件/拡張例8件のa/b案）、③-未処理2（第3部取引単体テストの小規模3ページ統合要否のA/B案）、④ファイル名・ディレクトリ構成（全38ページ対応表）の5点、および`PENDING_ZERO`26件の分類4-8（12件、ユーザー判断が必要な新規論点）。修正指示が来た場合は`checks/task-06.md`を更新してから再提示する。
