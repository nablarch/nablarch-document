...
- **マージ直前にまとめて処置する（台帳。以後の追加はここへ足す）** — user 指示により今は手を付けない
  ...
  - `#30` から（`/rn:ty` 2026-08-19 の user 指示で追加） — `checks/task-30.md` §「Verification Expert (fact-check)」の
    ...
    打ち消しの注記を足す
  - `.rn/` の整理（2026-09-03 user 判断「推奨で」。ディレクター報告に対する裁定） — マージ直前の1タスクで、次の6区分**以外**を `git rm` する: `design.md`・`steering.md`・`review-map.md`・`mapping/glossary.md`・`mapping/style.md`・`input/`（10 md。`design.md` §9「`input/` に資料として残す」）。削除対象（2026-09-03 実測 `git ls-files`、追跡 211 ファイル中 205）: 指示書 `ntf-*.md` 61・`checks/` 50・`reviews/` 34・`mapping/` の `mapping.csv`・`_batch/` 30・`sections-current.csv`・`sections-input.csv`・`vocabulary.md`・`volume.md`・`split-plan.md`・`scan-terms.tsv`・`term-candidates.csv`・`tools/` 13。理由: rn の方針「履歴は git と PR に置く」（本 `steering.md` §Rules の圧縮の項）。残す側で `.rn/` の外から参照されているのは `README.md:76`（`mapping/glossary.md`）のみ（実測 `grep -rn '\.rn/'`、`.rn/` 外は1件）。削除後は `steering.md` §Assumptions の指示書一覧・`design.md` §10.1 のファイル図・`review-map.md` の前書き（`reviews/page-*.md` 277 件・`checks/task-*.md` 154 件・指示書 71 件・`mapping.csv` 26 件を指す。2026-09-03 実測）が git 履歴（削除コミットの親）を指す旨に書き換える。**削除をマージ直前に置く理由**: user の38本レビューが続く間は `mapping.csv` と `mapping/tools/verify_mapping.py` を全量検証に使う
...
### #81: レビュアー向け対照表 `review-map.md` の作成（user 指示 2026-09-02） — 完了（2026-09-03 ディレクター直接コミット。user レビュー待ち）
...
**未決（user 判断待ち）**: 本書の形と粒度が用途に合うか。器だけの L2 見出し（「使用方法」「拡張例」）を「既存流用」と置いた扱いと、v6 に概要節が無いページの「機能概要」を「新規」と置いた扱いを、揃え直すか。各ページ末尾の「未確認」（記録の無い差分）を、レビュー前に潰すか、レビュアーに見せるか。
