...
- [ ] 29. **フェーズB B-1・B-2**。A-1 の逐語を `:1545` に反映し、承認された A-2〜A-5 の案を反映する。「変更不要」と判定されたもの（A-5）は反映しない（是正5 第2ラウンド B-1・B-2）。**未着手（user の承認待ち）**
- [ ] 30. **フェーズB B-3**。差分限定の2観点（範囲統制／新しい欠陥。後者に**承認された逐語案そのものへの反例検証**を含める）を**それぞれ別のサブエージェント**で回す。各プロンプトに常設の3点（実測で裏付ける／付属の検証スクリプトを正解にせず独立に組む／敵対的に見る）を入れる。**各観点の生出力を要約せずそのまま** `reviews/page-testdata_notation.md` の是正5 の節へ貼る。新しいファイルは作らない。`must` が出たら是正して再度2観点、上限2ラウンド（同 B-3）。**未着手**
- [ ] 31. **フェーズB B-4**。`reviews/page-testdata_notation.md` の是正5 の節に、フェーズA の実測結果と確定した逐語（A-1〜A-5）・B-3 の生出力とラウンドごとの指摘件数と観点を記録する。申し送り39・40・41 を処置済み／クローズにする。§「是正2・是正3 の指摘件数と観点」の「`nice` 3（**未処置**）」を処置済みへ更新する。是正3 の `nice` 3件目の記録の誤りと、`mapping/glossary.md` §5.10 の同型の誤り（申し送り42）を書く。`checks/task-35.md` と `steering.md` を反映後の状態にする。**記録に本ラウンドのコミットハッシュを書かない**（同 B-4）。**フェーズA の実測結果・逐語案と申し送り42・43 は記録済み。残りは Steps 29〜30 の後**
