...
**指示書**: なし。ディレクターが直接コミット（`9cbe94b0`）。**レビューは回さない**（整形のみ）。

**Completion criteria / 判定（2026-09-02）**: 完了。正規の場所でビルド succeeded・warning 0、HTML の important 内に `<li>` 4 件。申し送りは無い。
