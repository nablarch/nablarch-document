...
- **Next**: **レビュアー向け資料を作る（user 指示 2026-09-02）。** user のレビュー後にチームメンバー・TL がレビューする。観点は「既存から変更した部分はどこ・根拠・妥当性」「新規追加した部分はどこ・妥当性」。ページごとに、各節を **新規／既存流用／既存変更** に分け、既存変更にはその根拠を添える。粒度は利用者に影響する単位（細部は台帳番号で引く）。材料: `mapping/mapping.csv`（v6 の節 → 新ページ・disposition）、`design.md`（判断と根拠）、台帳 `#1`〜`#78`。置き場所は `.rn/20260724-ntf-yaml-support/` に1ファイル（design.md とは分ける。design.md は設計判断、こちらはレビュー用の対照表）。user「新規・既存変更の根拠を中心にレビューすれば本質的に価値ある」「メリハリ付けてレビューしたい」。その後、user の刷新版38本全量読みレビューの続き（質問が来たら実物で答える）。`#29` の残り（「マージ直前にまとめて処置する」の台帳）は user のマージ判断まで着手しない
