...
- `#24` ラウンド2 作業指示（判断待ち3件への回答）: `.rn/20260724-ntf-yaml-support/ntf-doc-24-round2.md`
...
**Purpose**: ページを作らないタスク。`#22` の user review で回答された判断待ち2件を、承認済み資産の是正として1タスクにまとめて行う。(a) 取引単体テストの実行方法は処理方式によって異なるという事実に `design.md` と `about/index.rst` を合わせる。(b) `style.md:45` に第2部セクションの必須・任意の区別を書き足す。取引単体テストは第2部に残り2ページ・第3部に6ページあり、そのすべてが同じ矛盾を踏むため先送りしない。指示は `#22` 回答（2026-08-14）§2・§3・§6・§7 と、ラウンド2 指示 `ntf-doc-24-round2.md`。
...
**ラウンド2 の指示（2026-08-14 受領、`ntf-doc-24-round2.md`）**: M-1 の差し戻しは正しく、原因はレビュー役の指示側にあると確定。判断待ち3件の回答は (1) 処理方式を名指しする・`important` は残す、(2) `mapping.csv` は変更せず別タスクも起こさない（`#6` で確定済み）、(3) 範囲外2件を含め `should` 13件のうち11件も同ラウンドで処理する。**出典が実施方法を述べているのは4処理方式のみ**（ウェブアプリケーション＝手動 `index.rst:7-8`／RESTfulウェブサービス＝自動 `rest.rst:7`／Nablarchバッチアプリケーション＝自動 `batch.rst:5-6`／MOMによるメッセージング＝自動 `real.rst:8-10`・`delayed_receive.rst:5-6`）。`HTTPメッセージング` は `http_send_sync.rst` に実施方法の記述0件、`テーブルをキューとして使ったメッセージング` は出典0件のため、**公開本文で何も述べない**。

**Steps**（ラウンド2。`- [x]` はラウンド1で確定済みのもの）:

- [ ] `about/index.rst:73`・`:75`・`:79` を `ntf-doc-24-round2.md` §2-2 の確定文言に差し替える。`:58` の `:widths:` を新しいセル文言の実測比で決め直す — `82dbe16` で差し替え済み。`:58`・`:73`・`:75` は4観点とも OK（`:widths: 9,26,16,49` は表示幅の実測比 `20/55/35/106` → `9.3/25.5/16.2/49.1` と一致）。**`:79` に `must` 3件（R2-M1〜M3）が残り、ユーザー判断待ち**
- [ ] `design.md:84`（実行方法セル・備考セル）を `about/index.rst:73`・`:75` と逐語一致させる（`APサーバ` を使わず `アプリケーションサーバ`）。`:86` を §2-1 の実測に差し替え、§2-3 の4点＋§3-2 の申し送り1文を含める — `:84` は4観点とも OK。**`:86` は R2-M4・R2-M5・R2-S1〜S3 の是正が残る**
...
- [ ] `reviews/page-about_index.md` の `## #24` 節を整理し、公開本文の最終形と出典の実測結果だけを残す（S-11）— `82dbe16` で `:318` 以降への純追加61行。**走査記録の是正（R2-M4）と `design.md` との逐語重複の解消（R2-S2）が残る**
...
- [x] 4観点のレビューを、それぞれ別のサブエージェントでもう一巡回す（**`ntf-doc-24-round2.md` はサブエージェントに渡さない**）— ラウンド2 実施。**4観点とも FAIL**。重複除去後の `must` 5件は `checks/task-24.md`「4観点レビュー ラウンド2」
- [ ] commit & push — ラウンド2 は `82dbe16`（push 済み）。ラウンド3 が残る
- [ ] **user review**

**守ること**:

- 処理方式の名称は `glossary.md` §5.2 の正表記を使う（出典の呼称「バッチ処理」「メッセージ受信処理」等をそのまま持ち込まない）
- 「それ以外」「すべての処理方式」のような閉じた表現を使わない。出典のある4処理方式だけを名指しし、残り2処理方式には触れない
- `about/index.rst:83-94` の内訳表（リクエスト単体テストの6処理方式）は変更しない（`design.md:90` の確定事項）
- 取引単体テストの処理方式ごとの一覧表を新設しない（`design.md:90` と衝突する）
- **変更してよいファイル**: `ja/.../about/index.rst`（`:58`・`:73`・`:75`・`:79` のみ）／`design.md`（`:84`・`:86` のみ）／`mapping/style.md`（`:45-46`・`:53-54`・`:62-67`・`:69-73`・`:77`〜`:81`）／`mapping/glossary.md`（`:168` の意味欄のみ）／`reviews/page-about_index.md`（`## #24` 節のみ）／`checks/task-24.md`
...
**Completion criteria**: ラウンド1 のゲート1〜12 を再実行して全件 PASS、加えて `ntf-doc-24-round2.md` §5-3 のゲートA〜I が全件 PASS で `checks/task-24.md` に記録されていること。要点は次のとおり。

- `git status --porcelain` の**全件**表で、上記「変更してよいファイル」以外が0件（ゲート1。`commit & push` の直前にもう一度実行する＝ゲート12・ゲートI）
- 出典8件を `git show 2e501ad:<path>` で開いて `file:line` と記述を突合し、`ntf-doc-24-round2.md` §2-1 の表と一致（ゲート2。一致しないものがあれば**本文を書かずに報告する**）
- `about/index.rst` に「それ以外」「すべての処理方式」が0件（ゲートA）。`:79` が名指しする処理方式が `glossary.md` §5.2 の正表記と逐語一致し、`HTTPメッセージング`・`テーブルをキューとして使ったメッセージング` が現れないこと（ゲートB）
...
**判断が要る場合**: `ntf-doc-24-round2.md` §2-1 の実測と手元の実物が食い違った場合、および §2-2 の文言が `design.md` の他の確定事項（特に `:88`・`:90`・`:92`・`:94`）と両立しない場合は、**本文を書かずに報告する。範囲を自分で広げない。**
...
- **Status**: paused
- **Date**: 2026-08-14
- **Last completed**: #22（DONE）。#24 はラウンド2 の本文を `82dbe16` で push 済み。ゲート1〜12・A〜I は全件 PASS だが、**4観点レビュー ラウンド2 は4観点とも FAIL**。判定と triage は `checks/task-24.md`「4観点レビュー ラウンド2」（未コミット）
- **Next**: **#24 のユーザー判断5件の回答を受ける。** 回答後、範囲内の是正（R2-M4・R2-M5・R2-S1〜S3）と合わせてラウンド3 を回し、4観点レビューをもう一巡する
- **Notes**: ブランチ `ntf-yaml-support`（`82dbe16` まで push 済み）。**ユーザー判断待ち5件**（2026-08-14 に提示済み・未回答。詳細は `checks/task-24.md`「4観点レビュー ラウンド2」）: (1) `about/index.rst:79`「JUnitでの自動実行はできず」に出典がない（R2-M1、4観点すべてが指摘）、(2) 同 `:79`「MOMによるメッセージングでは…自動実行する」が MOM の内側で過度一般化（R2-M2。`mapping.csv` 実測で `send_sync.rst` が MOM ページに4行、同ページ `:92` の MOM の定義と矛盾）、(3) 同 `:79` が出典の「実施可能である」を断定に強め条件節と目的語を落としている（R2-M3）、(4) 指示書 §2-1 の「`http_send_sync.rst` は実施方法をひとことも書いていない」が不正確（`:9` に記述あり。結論は不変、根拠の書き方のみ是正）、(5) `style.md` の範囲外2件（`:96-102` の「なし。」の根拠が宙に浮いた／`:189` の S-04 表が `:45-46` と食い違う）を `#24` に含めるか。**(1)〜(3) は指示書 §2-2 が逐語指定した公開本文そのものであり、回答があるまで書き換えない。** 不変条件: `mapping.csv` 595行 / 12,986 / 11,983、Docker フルビルドの既知警告は `db_double_submit.rst:108` の1件のみ。**#23 は指示書 `指示/task-23.md` の受領待ちで着手しない。**
