...
出典は次の4つ（`source` 列の値）。
  ...
  ntf-doc-terms-heading  `input/ntf-doc-terms.md` のMarkdown見出し
                         （`#`〜`####`）のうち、H1（文書題）を除いた
                         H2〜H4。H1はこのファイル自体の題であって
                         用語候補ではないため除く。実測: H1=1 / H2=10 /
                         H3=27 / H4=8（H1を除く45件）。発注元の指示は
                         「35件」だが、どのレベルの組み合わせが35なのか
                         指示に明記がなく、35に合わせるための恣意的な
                         レベル選定はしない。本ツールは「H1（文書題）を
                         除く全レベル」という単純な規則を採り、結果の45件を
                         そのまま報告する。
