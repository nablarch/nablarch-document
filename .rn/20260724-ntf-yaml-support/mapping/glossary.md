...
**`.rn/` 内の自分たちの文書を指す `S:` の参照は行番号を書かない。** 参照は原則 `PREFIX:path:line` の形で書くが、これらの文書は本作業の進行中に書き換わり続けるため、行番号を書くと参照した内容が動いていないのに参照だけが壊れる（`S:design.md` は `#pre-last` 時点で13件が壊れていた）。行番号を書かない `PREFIX:path` の形も認め、`verify_glossary.py` は「引用または表記がそのファイルのどこかに実在すること」を検査する。生きたファイルに対して内容の実在を検査し続けるので、行番号をピン留めするより強い（`#pre-last` 確定）。この免除は `design.md` に限らない。`steering.md` の `# Rules` が 2026-08-18 に「`.rn/` 内の文書どうしの相互参照は、行番号ではなく節見出し（`ファイル名` §番号「見出し」）で指す。`ja/` や他リポジトリの実物を出典として示す `file:line` は対象外で、そのまま使う。」と定め、区別の基準を「指す先が `.rn/` 内の自分たちの文書か、実物か」としているため、本用語集もその決定に従う（本書 §5.4 がこの形を採っている）。`input/` 配下の出典資料と、受領後に書き換えていない作業指示は実物の側であり、`file:line` のまま指す。
