...
| `稼動環境` | 第1部の下位セクション。動作要件を示す枠 | 揺れなし（`稼働環境` は未出現） | なし | `稼動環境` はFW解説書のページ題（FW3件、`FW:nablarch/platform.rst:3`）で、`S:design.md` も第1部のセクション名として使う。FWには「稼働」も生5件あるが動詞や別複合語（「稼働サーバ」）で、`稼動環境` とは競合しない |
