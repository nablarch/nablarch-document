...
- 各ページは、目次（`.. contents::`）の直後に「**機能概要**」のL2見出しを置き、その配下の冒頭で、
  そのページで何ができるようになるか、何を説明するページかを述べる。
  - **「機能概要」は読者にとっての価値を書く。見出しの列挙・言い換えにしない**（`#64` で user 指摘・確定。2026-09-02
    「今の概要、読んで何が嬉しい？機械的に概要を書いたらダメ」）。書くのは3点。①その設定・機能は何のためにあるか
    ②読者はいつ要るか（必須か、特定の場合だけか）③代表例を1〜2件。「テストの実装方法は〜を参照」の導線は残す。
    第2部の10ページ（`setup/common.rst`・`class_unit_test.rst`・`request_unit_test/` 5ページ・`deal_unit_test/` 3ページ）を
    この型で書き直した。第1部・第3部・第4部は既にこの型で書けていたため変更していない。
  - **目次と最初のL2見出しの間に地の文を置かない**（`#37` で user 確定。2026-08-26）。見出しの無い
    リード文を置く旧規約を改めたもので、リード文が担っていた内容は「機能概要」の冒頭に置く。
    見出しがあれば目次から辿れ、ページごとに置き場所が割れることもない。
  ...
  - 他ページへ送るだけの導線ページ（`setup/request_unit_test/db_queue.rst`・
    ...
    適用しない。いずれも「ラベル → タイトル → 導線の1文」の6行で、L2セクションを持たない
    （S-09の適用外と同じ3ページ。実測、2026-08-16）。
  - 文頭を「ここでは、」で始めない。「機能概要」の冒頭はページ全体を指すため「ここ」の指す範囲が
    曖昧になる。対象を主語に立てて言い切る（下記「根拠（リード文の位置と書き出し）」参照）。
- 第2部のページは「機能概要 → 使用方法 → 拡張例」の順に並べる。「機能概要」「使用方法」は必須、
  「拡張例」は出典が無い場合は見出し自体を置かない。
- 第3部のページは「機能概要 → 使用方法」の順に並べる（拡張例の見出しは置かない）。どちらも必須である。
...
**この根拠のうち、位置についてはFW解説書に揃えない**（`#37` で user 確定。2026-08-26）。FW解説書は
見出しの無いリード文を目次と最初のL2見出しの間に置くが、本解説書はその内容を「機能概要」の冒頭へ
移し、目次と最初のL2見出しの間に地の文を置かない。**書き出し（「ここでは、」で始めず対象を主語に
立てて言い切る）は引き続きFW解説書に揃える。** 上の実測記録は、揃えた点と違えた点のどちらについても
判断の材料になるため、そのまま残す。
...
- 旧条文「用例が薄いページでのみ使う」は撤回する。実測はこれと逆で、L4が多いのは
  ...
  `implementation/request_unit_test/web.rst`（16本）という用例が厚いページである。また「用例が薄い」は
...
- 掃き出し前（`e57a0d3`）の不一致は96件で、**すべて「49とすべき箇所を50にしている」**ものだった。
  ...
  この掃き出しはHTML出力を1バイトも変えない（`#last` でビルド前後のHTML全486ページを比較して確認）
...
- **未リリースのモジュールのクラスは、上記のいずれでも コードリテラル にする。**
  Javadocが公開されておらずリンク切れになるためである（`reviews/page-testdata_converter.md` §「クラス名の表記」）。
...
- 直後が仮名・漢字等の全角文字: 1,342件すべて `\ ` 付き。付いていないものは0件。
- 直前が全角の約物: 730件すべて `\ ` 付き。付いていないものは0件。
...
**上の実測が2,255件から2,263件に増えているのは、`#last` の区切り文字ディレクティブの是正（`checks/task-last.md`
§5-8）が本文にコードリテラルを8件足したためで、取りこぼしではない。** 内訳は
`implementation/testdata_examples.rst:1435` に3件（`` ``record-separator`` ``・`` ``"\r\n"`` ``・`` ``CRLF`` ``）、
`implementation/testdata_notation.rst:923` に4件（`` ``NONE`` ``・`` ``CR`` ``・`` ``LF`` ``・`` ``CRLF`` ``）、
同 `:950` に1件（`` ``\t`` ``）である。同じ是正で「直後が全角文字」が5件・「直前が全角の約物」が1件増えた。
**違反は増加後も0件である。**

全角の約物を全角の文字から除く案も採り得たが、直後側が掃き出し前から383件対1件で `\ ` を置く側に
そろっており、直前だけを別扱いにする理由が無いため、`\ ` を置く側に統一した。
