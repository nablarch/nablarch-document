...
観点は次の8つに限定する。文の長さ・改行位置・図の配置・括弧の全角半角・英数字と日本語の間の空白・
...
**規約**: ページは「機能概要 → モジュール一覧 → 使用方法 → 拡張例」の順に並べる。
`design.md` の第2部ページアウトライン（機能概要・使用方法・拡張例）および
第3部ページアウトライン（機能概要・使用方法。拡張例は置かない）と整合させる。

拡張できる内容が無い機能では「拡張例」を次のいずれかの方法で扱う。
...
4階層目（L3のさらに下）に対応する記号（例: `^`）の使用は、確認した範囲（`tag.rst` を含む）では見つからなかった。
...
本書は8観点に限定して作成した。文の長さ、改行位置、図の配置、括弧の全角半角、
英数字と日本語の間の空白、送り仮名・漢字/かなの揺れなど、他の規則性に気づいた場合でも
本書には記載しない（`design.md`「7. トンマナ」および本タスクの作業指示による）。
