...
**規約3（Excel形式/YAML形式の太字ラベル）**: 1つのセクション内でExcel形式の
イラスト（表・コードブロック）とYAML形式のイラストがそれぞれ独立したブロックとして
隣接して現れ、どちらの説明か視覚的に紛れるおそれがある場合、各イラストの直前の
地の文で `**Excel\ 形式**` ／ `**YAML\ 形式**` のように太字にする。

- **適用しない場合**: Excel/YAML比較表1つ＋YAMLコード例1つのような、
  隣接するイラストブロックが1つしかなく紛れが生じない構成（`testdata_notation.rst`
  の大半のセクションがこの形式）には適用しない。
