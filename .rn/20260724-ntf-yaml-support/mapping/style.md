...
観点は次の10個に限定する（S-01〜S-08は`#4`確定時点の8観点、S-09・S-10は
2026-08-06の`/rn:gm`フィードバックを受けて追加）。文の長さ・改行位置・図の配置・括弧の全角半角・英数字と日本語の間の空白・
...
### S-10 Excel/YAML形式を対比するイラストの書き方

NTF解説書はExcel形式とYAML形式を対比しながら記法を説明する構成であり
（`design.md`§4、`testdata_notation.rst`が代表例）、FW解説書には存在しない
構成のため本節を追加した。3つの規約からなる。

**規約1（識別子行は表の外に出す）**: Excelのセル内容を格子状に示すイラストで、
...
**規約2（識別子行は普通の文字で書く）**: 規約1で表から出した識別子行は、
コード書式（`` ` ``）を使わず、他の例示セル（カラム名・値など）と同じ普通の
文字で書く。読みやすさのため、必要なら和文引用符「」で囲んでよい
（例: 「SETUP_TABLE=テーブル名」）。

**規約3（Excel形式/YAML形式の区切り。太字ラベルとL4見出しを使い分ける）**:
1つのL3セクション内でExcel形式の説明とYAML形式の説明が分かれる場合、
どちらの説明かが視覚的に紛れないよう区切りを入れる。区切りの強さは
分量で使い分ける。

- **短い一対（表1つまたはコードブロック1つ程度ずつ）**: 各イラストの直前の
  地の文で `**Excel\ 形式**` ／ `**YAML\ 形式**` のように太字にする
  （例: `testdata_notation.rst` の「テーブルのデータを記述する」節、
  `SETUP_TABLE` の記述例）。
- **長い一対（表・コードブロック・アドモニションが複数個連なる場合）**:
  太字の地の文では見出しが本文に埋もれて見失われる（スクロールすると
  太字行自体が画面外に流れるため）。`S-04`のL4見出し（`^`の下線）で
  「Excel形式の場合」「YAML形式の場合」という小見出しを立てる
  （例: `testdata_notation.rst` の「ファイルのデータを記述する」節、
  `SETUP_FIXED` の記述例。用語定義表や概念図など両形式に共通する説明は
  見出しの外＝両形式の前に置く）。L4見出しは`S-09`で追加したページ先頭の
  目次（`.. contents::`）にも反映されるため、長いセクションほど見出し化の
  効果が大きい。
- **適用しない場合**: Excel/YAML比較表1つ（各行がExcel/YAMLの説明）＋
  YAMLコード例1つのような、比較表自体がExcel/YAMLを既に明示している構成
  （`testdata_notation.rst`の大半のセクションがこの形式）には、
  どちらも適用しない。
...
**由来**: 2026-08-06、`testdata_notation.rst`のSETUP_TABLE例・SETUP_FIXED例
に対するユーザーの`/rn:gm`フィードバック3点（セル結合の検討・識別子行の書式・
Excel/YAML説明ブロックの視認性）を受けて追加。同日、ユーザーから追加指摘
（「ファイル構成と記述時の注意点を確認する」節にExcel/YAMLの区切りがなく
分かりにくい）を受け、規約3を「太字のみ」から「分量に応じて太字/L4見出しを
使い分ける」に改訂した。太字ラベルは1つのセクション内の狭い範囲でしか
目に入らず、複数ブロックにまたがる長い説明では区切りとして機能しなかった
ことによる。

## 3. 検証していない事項

本書は10観点（S-09は2026-08-06追加、S-10は2026-08-06追加）に限定して作成した。文の長さ、改行位置、図の配置、
