...
# ラウンド4（指示書 `ntf-doc-24.md` §7。ラウンド3 の成果は承認済み、判断待ち3件への回答を反映）

レビュー役が `4961a86` を自分でクローンして追試し、**ラウンド3 の成果を承認した**（追試の全件は指示書 §7-1）。報告1（`design.md` の申し送りの前提が事実に反していた件）も CC の指摘が正しいと認められ、是正の内容も正しいと判定された。ラウンド4 の作業範囲は下表の3件のみである。

| # | 判断待ち（ラウンド3） | 回答 | ラウンド4 での反映 |
|---|---|---|---|
| 1 | 公開本文の文言の改善案5件 | **2件だけ採る**（目的語の補い／「アプリケーション」3回の解消）。残り3件は採らない | `about/index.rst:77` の**第2文のみ**を差し替え。第1文・第3文は変更していない |
| 2 | `:81` の「このうち」の先行詞 | **直す。指摘は正しい。** 許可行を `:81` まで広げる | `3種類のテストのうち、` に差し替え |
| 3 | 申し送りに回した3件 | **`style.md:5` の「7. トンマナ」→「8. トンマナ」だけ `#24` で直す。** 残り2件は `#pre-last` へ送る | `style.md:5` を1語置換。残り2件は本節末尾に申し送りとして記録 |

## 変更の全件（3行）

| ファイル | 行 | 変更前 | 変更後 |
|---|---|---|---|
| `ja/.../about/index.rst` | `:77` 第2文 | ウェブアプリケーションでは、テスト対象のアプリケーションをアプリケーションサーバにデプロイし、手動で操作してテストを行う。 | ウェブアプリケーションでは、テスト対象をアプリケーションサーバにデプロイし、手動で操作して取引単体テストを行う。 |
| `ja/.../about/index.rst` | `:81` | このうちリクエスト単体テストは、対象とする処理方式によって、次の6つに分かれる。 | 3種類のテストのうち、リクエスト単体テストは、対象とする処理方式によって、次の6つに分かれる。 |
| `.rn/.../mapping/style.md` | `:5` | 本書は、`design.md`「7. トンマナ」が定めた8観点について、… | 本書は、`design.md`「8. トンマナ」が定めた8観点について、… |

`style.md:5` の是正根拠は実測である。`grep -n '^## ' design.md` の結果、`:390` が `## 7. 用語`、`:411` が `## 8. トンマナ` であり、`style.md:569` は既に「8. トンマナ」と正しく書いている。`:5` だけが食い違っていた。「8観点」は変更していない（`style.md:12` が「S-01〜S-08は`#4`確定時点の8観点」と定義しており、そのままで正しい）。

## ゲート O〜U（ラウンド4 で追加）

| # | ゲート | 結果 |
|---|---|---|
| O | `git diff 4961a86..HEAD -- ja/` の変更行が `about/index.rst` の2行だけ | **PASS**。`+2 / -2` の4カウント（2行の置換）。他ファイルの差分0 |
| P | 差し替えた段落に `アプリケーション` が2回だけ | **条件付き PASS。指示との食い違いを本節下部に報告する。** 第2文単位では2回（`ウェブアプリケーション`・`アプリケーションサーバ`）で指示どおり。**段落単位では3回**で、3件目は第3文の `Nablarchバッチアプリケーション` である |
| Q | `about/index.rst` に `このうち` が0件 | **PASS**。`grep -c 'このうち'` = 0 |
| R | `.. _testing_framework_about-test_type_names:` が `:79` にあり、直後の段落が `3種類のテストのうち、` で始まる | **PASS**。ラベル行は `:79`、`:81` は `3種類のテストのうち、リクエ…` |
| S | `design.md`・`mapping.csv`・`glossary.md`・`steering.md` の差分が空 | **PASS**。`git diff 4961a86 -- <4ファイル>` の出力0行 |
| T | `style.md` の差分が `:5` の1行だけ | **PASS**。`1 insertion(+), 1 deletion(-)` |
| U | `style.md` が見出しで指す `design.md` の節が全件実在し一意 | **PASS**。下表のとおり全5件 |

### ゲートU の全件

`grep -n 'design\.md.\{0,2\}「' style.md` のヒットは5件（母集団をホワイトリストで切り出していない）。

| style.md | 指す見出し | design.md での実在 | 一意か |
|---|---|---|---|
| `:5` | 「8. トンマナ」 | `:411` `## 8. トンマナ` | 一意（1件） |
| `:76` | 「3. 第2部 導入と設定」＞「ページのアウトライン」 | `:186` `## 3. 第2部 導入と設定` ／ 配下 `:212` `### ページのアウトライン` | 親節つきで一意 |
| `:78` | 「4. 第3部 テストの実装方法」＞「ページのアウトライン」 | `:252` `## 4. 第3部 テストの実装方法` ／ 配下 `:281` `### ページのアウトライン` | 親節つきで一意 |
| `:82` | 「モジュール一覧は第1部に置かない。…」 | `:157` `### モジュール一覧は第1部に置かない。…` | 一意（1件） |
| `:569` | 「8. トンマナ」 | `:411` | 一意（1件） |

`### ページのアウトライン` は `design.md` に3件ある（`:212`・`:281`・`:342`）が、`style.md:76`・`:78` はいずれも親節を添えて指しているため一意に定まる。`:212` は §3（`:186`〜`:251`）、`:281` は §4（`:252`〜`:329`）、`:342` は §5（`:330`〜`:363`）の配下である。

**指示書 §4 S-9 が「`design.md:118` の節題」と書いていた箇所は、実物では `:157` である。** ラウンド3 で `design.md` に段落を足したことによる行番号のずれで、指す内容は変わっていない。`style.md` は行番号ではなく見出しで指しているため影響は無い（これが `S-7` で節見出し参照に切り替えた狙いそのものである）。

## ラウンド3 ゲート1〜12・A〜N の再実行

| # | ゲート | 結果 |
|---|---|---|
| 1 / I / 12 | `git status --porcelain` の全件が許可ファイルのみ | **PASS**。2件（`mapping/style.md`／`about/index.rst`）のみ。`checks/task-24.md`・`reviews/page-about_index.md` はこの後の追記で3〜4件になるが、いずれも許可ファイル。Docker ビルド後も `locales/ja/LC_MESSAGES/sphinx.mo` は現れていない |
| 2 / L | 出典8件の突合と走査コマンドの再現 | **PASS**。母集団22件・`.rst` 8件。走査1 のヒットは `batch.rst:5`・`:6`／`index.rst:8`／`real.rst:10`／`rest.rst:7`／`send_sync.rst:151`・`:176`・`:207` の8件で `delayed_send`・`delayed_receive`・`http_send_sync` は0件。走査2 は `send_sync.rst:163` の1件のみ。`design.md:92-96` の記載と全件一致 |
| 3〜8 | 用語・段落内改行・表記 | **PASS**。`実施方法` は `about/index.rst` に0件。差し替えた2行はいずれも1段落1行で段落内改行なし |
| 9 | `verify_mapping.py` | **PASS**。`Loaded 595 rows` / `lines total (all rows): 12986` / `lines total (excluding DROP): 11983` / `OK: no errors` / exit 0 |
| 10 | 変更してはならないファイルの差分 | **PASS**。ゲートS と同一の実行 |
| 11 / H | Docker フルビルド（`-a`） | **PASS**。`build succeeded, 1 warning.`。警告は `db_double_submit.rst:108: WARNING: undefined label: how_to_set_token_in_request_unit_test` の既知1件のみ、新規0件。実行直後に `git checkout -- locales/ja/LC_MESSAGES/sphinx.mo` を実行し `git status --porcelain` に現れないことを確認。`_build` はコンテナ内（root 所有）のためコンテナ内で `rm -rf _build` してから `sphinx-build -a` を実行した |
| A | 閉じた表現 | **PASS**。`grep -n 'それ以外\|すべての処理方式' about/index.rst` = 0件 |
| B | 名指しが3処理方式だけ | **PASS**。`:77` に `ウェブアプリケーション` 1・`RESTfulウェブサービス` 1・`Nablarchバッチアプリケーション` 1、`MOMによるメッセージング`・`HTTPメッセージング`・`テーブルをキューとして使ったメッセージング` は各0件 |
| C | `design.md:84` と `about:73`・`:75` の逐語一致 | **PASS**。Python の部分文字列比較で実行方法セル・備考セルとも `True`。`APサーバ` は両ファイルとも0件。ラウンド4 では `:73`・`:75` を変更していない |
| D | `実施方法` 0件 | **PASS**。`about/index.rst` 0件。`design.md` の唯一のヒット `:640` は `mapping.csv` の `heading_path` の引用（出典の引用箇所）である |
| E | `style.md` の `design.md:NN` 行番号参照 | **PASS**。`grep -c 'design\.md:[0-9]' style.md` = 0 |
| F | `glossary.md` の差分 | **PASS**。`git diff 4961a86 -- mapping/glossary.md` の出力0行（ラウンド4 では変更していない） |
| G | `reviews/page-about_index.md` の変更行が `## #24` 節の内側 | **PASS**。`## \`#24\`` は `:322`。ラウンド4 の追記は `:335` 以降のみ |
| J | 差し替えた段落が地の文 | **PASS**。`:77`・`:81` とも字下げ0。`about/index.rst` のアドモニションは `:46` tip・`:94` important・`:98` important の3件で、important はJakarta Batch・マルチスレッドの2件だけ |
| K | 不可能性の語 | **PASS**。`:77` に `できず`・`できない`・`不可` が0件 |
| M | `style.md` の2件 | **PASS**。S-04 表の L2 行（`:191`）の用途欄は「ページの大セクション。構成は `S-02` による」で `モジュール一覧` は現れない。`:200` の `モジュール一覧` は「根拠」欄が挙げる FW解説書側の事実であり、指示書 §4-2 が残してよいとしたもの。`なし。` のヒットは `:102` の「なお…本解説書では採らない」の1件のみで、「根拠」の箇条書きには残っていない |
| N | `design.md` と `reviews/` の逐語重複 | **PASS**。`design.md:86-125`（`#24` で追記した範囲）を句点・改行で分割した25字以上の文57件のうち、`reviews/page-about_index.md` の `#24` 節に逐語で現れるものは**0件**。20-gram 一致率 8.8%（残る一致は公開本文の逐語・走査コマンド・`file:line` の識別子） |

## 指示との食い違い1件（ゲートP。本文は変更していない）

**ゲートP は「差し替えた段落に `アプリケーション` が2回（`ウェブアプリケーション`・`アプリケーションサーバ`）だけ現れること」と書かれているが、段落単位では満たせない。** 実測は次のとおり。

| 単位 | `アプリケーション` の出現数 | 内訳 |
|---|---|---|
| 第1文 | 0 | — |
| **第2文（ラウンド4 の変更対象）** | **2** | `ウェブアプリケーション`・`アプリケーションサーバ` |
| 第3文 | 1 | `Nablarchバッチアプリケーション` |
| 段落全体 | 3 | 上記の合計 |

第3文の `Nablarchバッチアプリケーション` は `glossary.md` §5.2 の処理方式の正表記であり、指示書 §7-2 が「第1文・第3文は変えない」と明記している。**指示書 §7-2 の本文が問題としているのは「1文に『アプリケーション』が3回」であり、単位は第2文である**（挙げている3つが `ウェブアプリケーション`／`テスト対象のアプリケーション`／`アプリケーションサーバ` で、いずれも第2文の語である）。ゲートP の「段落」は「第2文」の書き誤りと判断し、第2文単位で PASS とした。**本文は変更していない。**

## `#pre-last` へ送った申し送り2件

指示書 §7-4 の裁定に従い、次の2件は `#24` では扱わず `#pre-last` へ送った。いずれも `style.md` の適用範囲そのものを広げる話であり、第1部1ページの都合で全ページの規約を決めることになるためである。

1. **段落内改行の禁止が `style.md` に規約として無い。** `style.md:13`・`:567` が「改行位置」を明示的に対象外としており、漏れではなくそう決めた結果である
2. **`:widths:` の算出基準が `style.md` に無い。** `style.md:258` は `:widths:` を付けよとだけ定め、数値の決め方を定めていない。`#24` の `9,26,16,49` の算出根拠は本ファイル（ラウンド2 S-4）にしかなく、`checks/` は作業の記録であって規約ではないため、次に `list-table` を書く人は同じ基準に辿り着けない

## 4観点レビュー

指示書 §7-7 により**回していない**。ラウンド3 が `must` を是正しきって収束しており、ラウンド4 の3件はいずれも局所的な語句の差し替えであるため、ゲート O〜U とフルビルドで判定した。
