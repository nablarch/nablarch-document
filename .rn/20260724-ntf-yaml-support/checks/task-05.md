...
## 暫定扱い一覧（#5d STEP1、2026-07-28）

`mapping.csv`の`note`が「暫定」で始まる行、全27行を機械抽出し一覧化した（実測: `grep`ではなく`note.startswith('暫定')`で27件）。すべて`vocabulary.md`「暫定（`#6`で正式な語彙に置き換わる）」区分の`dest_page`を使用しており、design.md第2部・第3部の処理方式別ページ分割が`#6`（未確定事項の確定）で確定するまでの暫定割当である（`HTMLチェックツール`8行を除く。後述）。

### グループ1: 第3部 取引単体テスト（ウェブアプリケーション）— 4行

design.md自身がページ分割を未確定としている4カテゴリの1つ（`vocabulary.md`89行）。`#6`で正式ページ名（プレフィックスなし）に一括置換される。

| mapping_id | dest_section | 暫定の理由（要旨） |
|---|---|---|
| current-0142 | 機能概要 | ページ冒頭の総論。処理方式非依存の共通総論として扱うか個別処理方式ページに割り当てるかがdesign.md §10未確定事項#2依存 |
| current-0143 | 使用方法 | dest_pageの判断根拠はcurrent-0142に従属。テスト準備の3項目（DB準備／デプロイ／サーバ起動） |
| current-0144 | 使用方法 | 同上。テスト実施手順とDBダンプ取得タイミングのtip |
| current-0145 | 使用方法 | 同上。取得エビデンスの箇条書きとテスト補助ツール検討中のtip |

### グループ2: 第3部 取引単体テスト（MOMによるメッセージング）— 4行

同上（`vocabulary.md`119行）。

| mapping_id | dest_section | 暫定の理由（要旨） |
|---|---|---|
| current-0154 | 機能概要 | ページ冒頭の機能概要。モックアップクラス使用時の処理フロー・提供3機能の説明 |
| current-0155 | 使用方法 | 「モックアップクラスを使用した取引単体テストの実施方法」章の導入文（L2直下） |
| current-0156-b | 使用方法 | `split-plan.md`承認済み分割の2つ目（Excelファイルの再読み込み挙動） |
| current-0157 | 使用方法 | 要求電文のログ出力機能（Map/CSV形式、log.properties設定例） |

### グループ3: 第2部 取引単体テストの設定（MOMによるメッセージング）— 1行

同上（`vocabulary.md`107・128-131行、`split-plan.md`current-0158で「割当先ページは未確定」とされた行の暫定値）。

| mapping_id | dest_section | 暫定の理由（要旨） |
|---|---|---|
| current-0158 | 使用方法 | モックアップクラス設定・Excel配置場所設定・テストデータ解析クラス設定・pom.xml依存追加の4小見出し。第2部「取引単体テストの設定」ページがdesign.mdに未定義 |

### グループ4: 第3部 リクエスト単体テスト（MOMによるメッセージング）— 9行

4カテゴリのうち唯一design.md上ページ名は確定済みだが、`dest_page`単体でテスト種別を判別できるようプレフィックスを暫定表記として付与（`vocabulary.md`89-93・109-113行）。`#6`ではプレフィックスを外すだけでよい。

| mapping_id | dest_section | 暫定の理由（要旨） |
|---|---|---|
| current-0321 | 機能概要 | 導入文＋概要ページへの参照tip（L2直下） |
| current-0322 | 機能概要 | 全体像図と親クラス継承要件のtip |
| current-0323 | 機能概要 | 主なクラスとリソース表（7行） |
| current-0324 | 使用方法 | StandaloneTestSupportTemplateの説明（テストクラス作成手順） |
| current-0325 | 使用方法 | AbstractHttpRequestTestTemplateの説明（同上、対をなすスーパクラス） |
| current-0326 | 使用方法 | RequestTestingMessagingProviderの機能説明・対応表 |
| current-0327 | 使用方法 | MessageSenderの4機能の箇条書き |
| current-0329 | 使用方法 | 「同期応答メッセージ送信処理固有のテストデータ」導入文（L2直下） |
| current-0330 | 使用方法 | 記法参照文のみ（REFERENCE、`current-0049`前例に倣う） |

### グループ5: 第2部 リクエスト単体テストの設定（MOMによるメッセージング）— 1行

グループ4と同じ暫定プレフィックスページ（`vocabulary.md`101行）。

| mapping_id | dest_section | 暫定の理由（要旨） |
|---|---|---|
| current-0328 | 拡張例 | TestDataConvertorインタフェースの拡張点説明（アーキテクトが実装する拡張クラス） |

### グループ6: 第4部 ツール > HTMLチェックツール — 8行（受け皿問題は解消済み、`note`文言のみ暫定表記が残存）

作成時点（batch-22/23相当の後続作業）ではdesign.mdの第2部・第3部いずれのページツリーにもHTMLチェックツールに対応するページが無く、`note`は「design.md未記載のため要確認」と記していた。その後の2026-07-28差し戻し対応でdesign.md第4部「ツール」が新設され、`HTMLチェックツール`は`vocabulary.md`の確定ページ一覧（第4部、49行）に格上げ済み（`vocabulary.md`57-58行「旧・第2部の暫定ページから第4部の確定ページへ格上げされた（下記「暫定」区分から削除）」）。**受け皿問題自体は解決済みであり、`#6`では`note`冒頭の「暫定。」を除去する文言更新のみで済む（`dest_page`/`dest_section`の変更は不要）。**

| mapping_id | dest_section | 暫定の理由（要旨、note作成当時） |
|---|---|---|
| current-0367 | 機能概要 | L1直下の導入文（目的・仕様・使用方法を扱う旨） |
| current-0368 | 機能概要 | ツールの存在理由（終了タグ忘れ防止・禁止タグ使用防止） |
| current-0369 | 機能概要 | チェック内容の仕様（HTML4.01準拠構文チェック等） |
| current-0370 | 機能概要 | ボディが空のタグを許容する仕様差分 |
| current-0371 | 機能概要 | 前提条件（リクエスト単体テストを実行可能であること） |
| current-0372 | 使用方法 | htmlCheckerConfigプロパティの設定例・設定ファイル記法 |
| current-0373 | 使用方法 | checkHtmlプロパティのXML設定例 |
| current-0375 | 使用方法 | チェック失敗時のJUnitコンソール出力と対応手順 |

### 集計

- 27行 = グループ1(4) + グループ2(4) + グループ3(1) + グループ4(9) + グループ5(1) + グループ6(8)
- グループ1〜5（19行）: design.md §10未確定事項#2（第2部・第3部の処理方式別ページ分割）に依存。`#6`で構成確定後、暫定`dest_page`を正式名称へ一括置換する
- グループ6（8行）: 受け皿は解決済み。`#6`では`note`文言の「暫定。」除去のみ
