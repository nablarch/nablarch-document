...
`NTF-MOD-01-1` の行は、依頼書 `ntf-mod-01-nablarch-testing-converter.md` §2（事象1: XLS → YAML → XLS → YAML の往復で内容が変わる）に記録した3事象 (a)(b)(c) の判定がすべて返ったため、`#31` で TODO を削除して本表から外した。この行がもう1つ持っていた出典 `checks/task-28.md` §7-3 は、同じ `#31` で当該行を外したため現在の版には無く、`git show 65a1756:.rn/20260724-ntf-yaml-support/checks/task-28.md` で `65a1756` 時点の記録（同 `:452`）として辿れる。判定は **確定（user 引用による。出どころである `nablarch-testing-converter` の課題 `XLS-05`「対応不要」・`XLS-27`「要対応」の記録は本作業ディレクトリからは参照できないため、user が作業指示に引用した文面による）** で、(a) は課題 `XLS-05` として対応不要、(b) は (a) と同じ明文による挙動（課題番号への帰属は示されていない）、(c) は課題 `XLS-27` として要対応かつ修正済みと確定した（2026-08-20、user 確定）。あるべき姿「往復しても内容が保たれる」（`ntf-doc-28-decide-disposition.md` §7-2 の表）とは、(a)(b) で逐語の内容そのものは変わるものの、`tools/testdata_converter.rst`「意味を変えずに往復できる」が保つと述べている「テスティングフレームワークの仕様上の意味」の水準では食い違わない。この判定に対する本文の書き直しは不要だが、別件として同ページの表の「意図のある情報」の行1行を `#31` で是正した。3事象それぞれの帰結と根拠、「内容」と「仕様上の意味」の水準の違い、是正した1行の位置づけ、`XLS-27` に残る制約を `NTF-MOD-01-3` が引き継いでいることは `checks/task-31.md`「判定の内訳（3事象）」にある。
