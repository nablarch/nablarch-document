...
`NTF-MOD-01-3` の行は、解除条件が満たされたことを実測で確認したため `#36` で TODO を削除して本表から外した。**実測（2026-08-26）**: `nablarch-testing-converter` `60d9a2d` に対し、`setup_tables` と `expected_tables` の両方に `rows: []` を書いた YAML を `TestDataConverter.convert` で Excel へ変換し（converted=1）、さらに YAML へ戻したところ、`rows: []` が保たれて元の内容に戻った。**TODO 本文の「0件テーブルを含む YAML は Excel へ変換できない」は事実ではない。** 変換経路は `TableData#loadData()` を呼ばない（converter の `src/main/java` に `loadData` の参照は0件）ため、この結果は `nablarch-testing` の #23 の適用有無に依存しない。解除条件に書かれていた「`XLS-27` の2段目（識別子行だけを書く）へ切り替わったら」は**満たされないまま満たされ得ない**。`XLS-27` は2段目ではなくマーカーカラム方式（`[EMPTY]` 列1つ）で決着しており（`XlsFormatWriter.java:213`・`:252`・`:543`）、制約はこの別経路で解消した。また #23 は `nablarch-testing` の `origin/main`・`origin/develop` に入っていない（`TableData.java:341` の early return が残る）。入っているのは `convert-testdata-excel-to-text` ブランチとローカル `~/.m2` の `6-NEXT-SNAPSHOT`（2026-08-21 18:28 install）だけである。行の指示にあった「`rows: []` を無条件の記法として教えている `implementation/testdata_examples.rst`「0件のテーブルデータを記述する」と `implementation/testdata_notation.rst`「0件のデータを記述する」もあわせて見直す」は、制約が消えた結果いずれも現状のままで正しいため、変更していない。行の現物は本ファイルの `#36` 以前の版で辿れる。

`NTF-MOD-02-4` の行は、解除条件が満たされたことを実測で確認したため `#37` で TODO を削除して本表から外した。**実測（2026-08-26）**: `nablarch-testing` の PR ブランチ `convert-testdata-excel-to-text` `3c4bd2a` の `src/main/java/nablarch/test/core/db/MasterDataSetUpper.java:185-204` が、マスタデータファイルの拡張子で分岐し、Excel 形式（`.xls`／`.xlsx`）以外は `parser.getSetupTableData(dir, <拡張子を除いたファイル名>)` を1回呼ぶ（`:190-194`）。すなわち #22（YAML 形式のマスタデータファイルへの対応）は PR ブランチで**実装済み**である。参照点を PR ブランチとする根拠は本 `steering.md` の Rules（2026-08-26 user 指示）。

これを受けて本文を書き直した。書き直しの根拠として次を実測した（プローブは `PoiXlsReader`／`YamlLoader` を直接呼ぶ独立のもので、両モジュールの既存テストは使っていない）。

- YAML 形式のマスタデータファイルの置き場所は `<masterdata.dir>/<ファイル名>.yaml` である。自動テストのテストデータと違い、テストクラスに対応するディレクトリは作らない。`MasterDataSetUpper.java:190-194`（`3c4bd2a`）が拡張子を除いたファイル名だけを渡し、`nablarch-testing-yaml` `0db2221` の `src/main/java/nablarch/test/core/reader/yaml/YamlLoader.java:81-86` が `basePath + "/" + resourceName + ".yaml"` を組み立てるため。実測: `YamlLoader.isResourceExisting(dir, "MASTER_DATA")` = `true`、`YamlLoader.isResourceExisting(dir, "MASTER_DATA/USERS")` = `false`
- YAML 形式では1ファイルが1つの読み込み単位であり、シートに相当する区切りは無い。`MasterDataSetUpper.java:196-203`（`3c4bd2a`）のシート走査は Excel 形式の分岐にしかない
- 拡張子は `.yaml` でなければならない。`.yml` は無言で0件になる。実測: `MASTER_DATA2.yml` を置いた状態で `YamlLoader.isResourceExisting(dir, "MASTER_DATA2")` = `false` → `YamlTestDataParser.getSetupTableData` は `Collections.emptyList()` を返す（`0db2221:src/main/java/nablarch/test/core/reader/YamlTestDataParser.java:108-111`）。そのため本文では `masterdata.file` の YAML 形式での値を `MASTER_DATA*.yaml` と明示した
- `masterdata.file` の対象は**ファイル**であってディレクトリではない。配布物 `master-data-setup-tool.zip` の `master_data-build.xml:65-67` が Ant の `<fileset dir="${masterdata.dir}"><include name="${masterdata.file}"/></fileset>` として使っている
- `.. important::`（Excel 形式のマスタデータファイルに YAML 形式用のパーサを設定すると無言で0件になる）は**そのまま残した**。逆向き（YAML 形式のファイル＋Excel 形式用のパーサ）は、実測で `IllegalArgumentException: invalid data name. [MASTER_DATA]` になり無言ではないため、記載しない。`PoiXlsReader.isDataExisting` は resourceName を `/` で2分割できることを前提としており（`3c4bd2a:src/main/java/nablarch/test/core/reader/PoiXlsReader.java:255-261`）、マスタデータ投入ツールが渡す `/` を含まない名前では例外になる。「利用者が正しく書こうとしても踏むもの」だけを書く基準（2026-08-25 user 確定）による

`NTF-MOD-01-1` の行の出典は `ntf-mod-01-nablarch-testing-converter.md` §2（事象1: XLS → YAML → XLS → YAML の往復で内容が変わる）・`checks/task-28.md` §7-3 の2つであった。§2 に記録した3事象 (a)(b)(c) の判定がすべて返ったため、`#31` で TODO を削除して本表から外した。§7-3 の節は残っているが、その表からは同じ `#31` で `NTF-MOD-01-1` の行を外した。行の現物は `git show 65a1756:.rn/20260724-ntf-yaml-support/checks/task-28.md` の `:452` で辿れる。判定は **確定（user 引用による。出どころである `nablarch-testing-converter` の課題 `XLS-05`「対応不要」・`XLS-27`「要対応」の記録は本作業ディレクトリからは参照できないため、user が作業指示に引用した文面による）** である（2026-08-20、user 確定）。(a)(b) では、あるべき姿「往復しても内容が保たれる」（`ntf-doc-28-decide-disposition.md` §7-2 の表）の逐語の「内容」そのものは変わる。それでもあるべき姿と食い違わないのは、`tools/testdata_converter.rst`「意味を変えずに往復できる」が保つと述べている「テスティングフレームワークの仕様上の意味」の水準で見ているためである。本文への注意書きの追加は不要で、(a)(b) が仕様と確定したことにより明文と矛盾していた同ページの表の「意図のある情報」の1行だけを、`ntf-doc-28-decide-disposition.md` §7 の指定どおり仕様に合わせて書き直した。3事象それぞれの帰結と根拠、「内容」と「仕様上の意味」の水準の違い、書き直した1行の位置づけ、`XLS-27` に残る制約を `NTF-MOD-01-3` が引き継いでいることは `checks/task-31.md`「判定の内訳（3事象）」にある。
