...
- 2-1・2-3 には「着手前に検証すること」、2-2 には「着手前に特定すること」があります。実装に入る前に、その結果だけを
...
**現行**: この記載例（同 `:2237`-`:2260`。**データ行は2件**）の Excel 形式を往復させると、
**全フィールドが空文字のレコードが消える**。ディレクターの実測（データ行3件のブック。記載例そのものではない）では、
テスティングフレームワークが読むレコードが原本3件・XLS→XLS 後2件・XLS→YAML→XLS 後2件だった。

**訂正（2026-08-28）**: 初版は記載例のレコードを「3件」と書いていたが誤り。`5783b35` の list-table は
識別行・ディレクティブ行・名前行・型行・データ行2件の6行である（CC 実測・ディレクター確認）。
...
**テーブルと `LIST_MAP` も対象である（2026-08-27 訂正）。**

**訂正前は「全要素が空のエントリを記述する記法が無いため対象外」と書いていた。これは誤りである。**
Excel 形式では**各セルに `""` と書けば表せる**。本体は空エントリの判定を**解釈前の生セル**で行うため
（`3c4bd2a` の `PoiXlsReader.java:92`・`TestDataParsingTemplate.java:180`。どちらも `interpret` の前）、
`""`（2文字）は空セルに当たらず、エントリは読み飛ばされない。

**実測**（2026-08-27。ディレクターが POI で組んだ実 `.xlsx`。`SETUP_TABLE=T`／カラム行 `[no]`,`id`,`name`／
データ行3件 = `1,U0001,yamada` ／ `(空),"",""` ／ `3,(空),(空)`）:

| 読み手 | データ行の件数 |
|---|---|
| 本体 `PoiXlsReader#readLine` | **3件**（3行とも返る） |
| `XlsFormatReader#read` | **1件**（`U0001,yamada` だけ） |

2件目が消えるのは `XlsFormatReader.java:664` の `isEmptyCell` が `""`・`””` を空セル扱いするためである。
3件目が消えるのはマーカーカラム除外後に判定しているため（XLS-08。**こちらは現状のままでよい**。
`tools/testdata_converter.rst:63` のとおり往復で消えることが解説書に明記されている）。

**やること**

1. **`isEmptyCell` が `""`・`””` を空セル扱いするのをやめる。** 本体と同じく、生セルが `null` か
   空文字のときだけ空とする。テーブル・`LIST_MAP` の両経路に効く
2. **書き出しで、全要素が空文字になるエントリの各セルへ `""` と書く**（読みで残るようになった値を
   Excel 形式で表す）

**XLS→YAML 方向は表せない。** YAML 形式では「`rows:` 内の要素が……すべての値が空文字の場合にスキップされる」
（`notation.rst:1500`）ため、この形は YAML に書けない。**完了条件3 で `@Ignore` ＋ 印つきの理由に記録する。**

**着手前に特定すること**（実装より前に出す。`nablarch/CLAUDE.md` 2-2）: `isEmptyCell` の変更で
期待値が変わる既存テストの全件。`""` を含むデータ行を持つテストが対象になる
（`XlsFormatReaderCellTypeTest` は検証対象セルが空になる行が消えるのを避けるため `KEY` 列を置いている。
この回避が不要になる可能性がある）。**変える／変えないの判断と件数を報告に書く。**
...
**本項で「YML-04」と呼ぶのは、先頭の `{}` が後続行を巻き込む形だけである**（2026-08-28 追記）。
converter の `coverage/issues.md` YML-04 が併せて挙げる「カラムは先頭行のキー集合で決まり、後続行にしかない
カラムは読まれない」は、`5783b35` の `implementation/testdata_notation.rst:818`「後続の行に最初の行のキーに
ないものを追加しても、そのキーは読み込まれない」が定める**仕様**であり、欠陥ではない。
...
2. **2-1・2-3 の「着手前に検証すること」と 2-2 の「着手前に特定すること」の結果が、実装に入る前に報告されている**（2-1・2-3 は 2026-08-27 に判定済み。残るのは 2-2）
...
6. **既存テストの期待値を変えた箇所が全件挙がっている。** 2-1・2-2・2-3 は既存テストの期待値に触れる。
...
1. **2-1・2-3 の「着手前に検証すること」と 2-2 の「着手前に特定すること」の結果**（実装前に一度報告する）
