...
`.rst` に加えてよい変更は、`.. image::` の挿入・差し替え・削除と、次の (a)〜(i) の逐語だけである。それ以外の本文は1文字も変えない。
...
- **(i) `implementation/testdata_notation.rst:46`・`:78`**（`a6da1f6` の `:44`・`:83`）: 各行の末尾の一文「ディレクトリ構成の対応は、以下のとおりである。」を削除する。前の文はそのまま。(e) の `code-block` 削除で指す先を失った文であり、落とすだけで前後が成立する（図19が `:30` で両形式の対比を示している）
