...
**進め方**: フェーズ0（着手前検証。`.rst` は1文字も変更しない）→ 報告して停止 → ディレクター OK → フェーズ1（実装）→ 4観点レビュー（ラウンド1）→ 差分限定レビュー（ラウンド2以降・上限3）→ 報告して停止。
...
**このタスクは 4観点レビューを回す**（`nablarch/CLAUDE.md` 3-1: 公開本文に新しい記述が入る。§8）。
...
12. §8 のレビューが実施・記録済みで `must` が残っていない

### §8 レビュー

- **ラウンド1: 4観点**（A 充足／B 整合／C 規約／D 検証の妥当性）を**それぞれ別のサブエージェント**で回す。各プロンプトに必ず入れる: **実測で裏付ける／付属の検証スクリプトを正解として使わず独立に組む／敵対的に見る**。加えて観点B・D には「**本指示書とコミットメッセージが挙げる `file:line`・逐語を1件ずつピンで開き、主張を支えているか確かめる**」、全観点に「**指示された案そのものに反例がないか**」（`nablarch/CLAUDE.md` 3-4）を入れる。
  - A: 65ブロック・tip 6件・移設3件・波及7件に抜けがないか（0-2 の全件表と突合）
  - B: 書き換え後の例・記述が `c06ebe8`・`9ef4096`・`e21bf67` の実物および他ページと矛盾しないか
  - C: `style.md` S-01〜S-13・`glossary.md` 正表記・見出し形式
  - D: 完了条件の検証手段（コンパイル・走査・突合）は欠陥を実際に検知できるか（例: 走査の母集合固定・語境界）
- **ラウンド2以降**: 差分限定の2観点（是正が範囲に収まっているか／新しい欠陥を生んでいないか）。上限3。各ラウンドの指摘件数と観点を `reviews/` に記録し、既決事項（本指示書冒頭の user 確定・却下済み論点）に反する指摘は却下して理由を記録する。
- レビュー記録は `reviews/page-standard_usage.md`（新設。junit4・about・implementation の指摘もここに集約し、対象ファイルを列に持つ）。
