...
6. **出典が「〜を指定する」と制約を述べている箇所は、実装がその制約を検査しているかで採否を決めない。** 検査していなくても、その制約に実質的な理由（値の加工・変換・切り詰めなどの挙動）があることが多い。**実装で挙動を確かめ、制約は残したうえで理由を書き添える。** 制約だけを書くと読者が守る意味を判断できず、実装が検査しないことを理由に緩めると、実際には壊れる値を許容してしまう。`#14` の `charValue`（固定長時にカラム長分繰り返される）・`numberValue`（カラム長で切り詰められる）がこの型である。ラウンド1の `R1-X4` は「実装が検査していないから緩める」対「推奨値として維持する」の対立で処理したが、**どちらも実装が何をするかを示していない点で不十分だった**（`#14` の締めで説明セルに挙動を追記して解消。`ntf-doc-14-close.md`）。
...
| | 内容 |
|---|---|
| 移動前（`使用方法` 直下、`:12`） | ここでは、エンティティ単体テストで使用する設定項目と、データベースを使用するクラスのテストでカラムの記述を省略したときのデフォルト値について説明する。いずれもテスト用のコンポーネント設定ファイルに記述する。 |
| 移動後（目次の直後、`:10`） | クラス単体テストでは、エンティティ単体テストで使用する設定項目と、データベースを使用するクラスのテストでカラムの記述を省略したときのデフォルト値を設定できる。いずれもテスト用のコンポーネント設定ファイルに記述する。 |
