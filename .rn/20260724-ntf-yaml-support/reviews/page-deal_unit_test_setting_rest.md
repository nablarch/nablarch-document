...
行番号は本ラウンド適用後の `rest.rst` のもの。実装の参照先は §2 と同じく `/home/tie303177/work/nablarch/nablarch-testing-rest` の `origin/main`（`b7729dfb980076a36ee80e88cf8ce4b038a7721d`）で、すべて `git show origin/main:<path>` で読んだ。出典は `git show c241906:ja/development_tools/testing_framework/guide/development_guide/05_UnitTestGuide/03_DealUnitTest/rest.rst`。

### R2-1（must）. リード文 `:10` の「自動で引き継げる」を「実装クラスをコンポーネント設定ファイルに登録することで引き継げる」に改めた

**是正前**: 「RESTfulウェブサービスの取引単体テストでは、サーバが発行したセッションIDやCSRFトークンを、後続のリクエストへ自動で引き継げる。」

**是正後**: 「RESTfulウェブサービスの取引単体テストでは、実装クラスをコンポーネント設定ファイルに登録することで、サーバが発行したセッションIDやCSRFトークンを後続のリクエストへ引き継げる。」

**根拠**: 引き継ぎは登録しなければ起きない。`SimpleRestTestSupport.java:96-103` の `setupDefaultProcessor()` は `SystemRepository.get(DEFAULT_PROCESSOR_KEY)`（`:97`。キーは `:47` で `"defaultProcessor"`）を引き、`null` のときは `NOP_PROCESSOR`（`:61-76`。`processRequest` は引数の `HttpRequest` を、`processResponse` は引数の `HttpResponse` をそのまま返す）を `defaultProcessor` に設定する（`:101`）。この `defaultProcessor` がリクエスト送信時に渡る（`:187`・`:211`）。したがって未登録時は何も引き継がれない。出典 `:40-95` にも「自動」に当たる記述は無い。第2文（提供実装から選べる／独自クラスに差し替えられる）とは内容が重ならない（第1文は登録という手順、第2文は登録する実装クラスの選択肢）。

### R2-2（should）. `:17` の「直前のレスポンス」を「先行するリクエストのレスポンス」に改めた

**是正前**: 「1つの取引を複数のリクエストで構成する場合、直前のレスポンスに現れた値を、次に送るリクエストへ持ち越したいことがある。」

**是正後**: 「1つの取引を複数のリクエストで構成する場合、先行するリクエストのレスポンスに現れた値を、次に送るリクエストへ持ち越したいことがある。」

**根拠**: 出典 `:40-41` は「セッションIDやCSRFトークンなど、先行するリクエストのレスポンスとしてサーバから受け取った値」と書いており、直前に限定していない。実装も直前に限定しない。`RequestResponseCookieManager.java:40-57` の `processResponse` は、レスポンスのクッキーに `cookieName` と一致するものがあるときだけ `cookieValue` を更新して返し（`:47-52`）、無ければログを出して `cookieValue` を変えずにレスポンスを返す（`:55-56`）。`cookieValue` が `null` に戻るのは `reset()`（`:59-62`）だけで、これは各テストメソッドの開始時に呼ばれる（`SimpleRestTestSupport.java:84-86` の `@Before setUp()` → `:103`）。つまり保持している値は直前のレスポンス由来とは限らない。

### R2-3（should）. `:21` 第2文に主語「この実装クラスは、」を補い、`:33` と書き方を揃えた

**是正前**: 「レスポンスの ``Set-Cookie`` ヘッダに現れるクッキーのうち、``cookieName`` プロパティに指定した名前のものを取り出し、次のリクエストの ``Cookie`` ヘッダに設定する。」

**是正後**: 「この実装クラスは、レスポンスの ``Set-Cookie`` ヘッダに現れるクッキーのうち、``cookieName`` プロパティに指定した名前のものを取り出し、次のリクエストの ``Cookie`` ヘッダに設定する。」

**根拠**: 並列の位置にある `rest.rst:33` の第2文が「この実装クラスは、」で始まっており、同じ形に揃えた。`:33` は変更していない。文の内容そのものは変えておらず、裏付けは §2 の事実表 #3（`RequestResponseCookieManager.java:45`・`:47-52`・`:25-37`・`:69-71`）のまま。

### 本ラウンドで変えていないこと

差分は上記3行のみ（`git diff --stat` が `3 insertions(+), 3 deletions(-)`）。見出し・並び順・XML記述例・`important` ディレクティブ・他の段落には差分が無い。文あたりの可視文字数（role 記法を表示テキストへ展開して計測）の最大は 94 のままで、既存ページの範囲（87〜112）の内側にある。
