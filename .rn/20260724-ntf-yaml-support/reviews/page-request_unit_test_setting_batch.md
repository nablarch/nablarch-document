...
**回答**（2026-08-13、`/rn:ty`）: **是正する。** 推奨どおり `testdata_notation.rst:967` を型記号に改めた。是正する理由は、`#19` の申し送り4（`:1244`）と違って読者が誤った設定を書く経路があること、加えて `#20` のページ `:82` 自身が「記述方法は :ref:`ファイルのデータを記述する <testdata_notation-file_data>` を参照」と書いてその経路を作っていることである。承認済みページに触れる不利益より、正面から食い違ったまま公開する不利益が大きい。第3文の「同名の基底型より優先して使用される」も「元の型に代えてその型が使用される」に改めた（`TEST_X9` は `X9` と同名ではないため。`DataFileFragment.java:238-244`）。末尾に第2部への `:ref:` を追加し、申し送り7の一部を解消した。実行結果は `checks/task-20.md` ゲート16。
...
**回答**（2026-08-13、`/rn:ty`）: **第3部「テストデータの書き方」へ移す。** 置き場所も推奨どおり `testdata_notation.rst:967` の段落とした。**ただし採るべき理由は「`decide` 1 と同じ段落なので1回で済む」ではない。** `design.md:273-274` が2ページの役割を「テストデータの書き方＝記法の仕様。どう書けばどう解釈されるか」「テストデータの記載例＝用途別の実例。Excel と YAML の対比で示す」と定めている。「フォーマット定義がこうなら `12345` は `0000012345` と書く」は**どう書けばどう解釈されるかの規則そのもの**であり、かつ Excel 形式・YAML 形式で完全に同一であるため「対比で示す」記載例ページの構成に載らない。同ページ `:863-865` の tip（「``1AB`` と記載した場合、3バイトのバイト配列に変換される」）に先例がある。形式別 L4 には割っていない（`style.md` S-10 規約1）。

あわせて `mapping.csv` を実態に追随させた。`current-0037-b` を `263`〜`274`（`current-0037-b2`・12行・第3部）と `275`〜`316`（`current-0037-b`・42行・第2部）に分割し、594→595行とした（`#6` の `current-0128` と同じ手順）。`volume.md` の集計は「テストデータの書き方」3,391→3,403、「リクエスト単体テストの設定（Nablarchバッチアプリケーション）」129→117、合計は不変。実行結果は `checks/task-20.md` ゲート19〜21。
...
**回答**（2026-08-13、`/rn:ty`）: **どちらにも触れない（現状維持）。** 推奨どおり。レビュー役が jar を展開して前提を全件確認した（`process-service_test.xml` を `import` するファイルは jar 内0件、`fixed-length-convertor-setting_test.xml` を `import` するのは `nablarch/override_test.xml` のみで、その `override_test.xml` を `import` するファイルは0件）。

**記録に1件追加する。上の表に抜けていた。** `fixed-length-convertor-setting_test.xml` は自身の `:10` で `nablarch/batch/resume-point-manager_test.xml` を `import` している。**`override_test.xml` を経由せず当該ファイルだけを直接 `import` した場合でも、レジュームポイント管理の設定が抱き合わせで入る。** 「案内しない」という結論はこの事実によってさらに強くなる。`#last` までに `design.md` §8 の記録として残すかどうかは `#last` で判断する。**`#20` では記録に留め、別タスク化はしない。**
...
3. **見出し下線の実測則は階層ごとに違う。** NTF全ページの実測で、L1（`=`）は `max(50, 表示幅)`、L2（`-`）は50固定、L3（`~`/`^`）は `max(49, 表示幅)` である。ただし `implementation/testdata_examples.rst` だけは L3 の下限が50で、コーパス自体が割れている。`setup/` 配下は5ページすべて49であり、新ページは49側に揃えた
   - **裏付けの範囲を明示する**（2026-08-13、`/rn:ty` の `should` 3）。`setup/` 配下には表示幅が49を超える L3 見出しが `#20` のページ以外に無く、`max(下限, 表示幅)` の**表示幅側**をこの範囲だけでは確かめられない。裏付けは `implementation/testdata_examples.rst` の7箇所である（`:16` 幅63・`:196` 幅50・`:399` 幅52・`:500` 幅60・`:637` 幅56・`:864` 幅59・`:1703` 幅50。いずれも下線＝表示幅）。**うち下限50を超えて discriminating なのは5箇所**（`:16`・`:399`・`:500`・`:637`・`:864`）で、`:196`・`:1703` は幅がちょうど50のため同ファイルの下限50と区別できない。**下限が49であることの裏付けは `setup/` 配下の5ページ側にあり、両者を合わせて `max(49, 表示幅)` となる**（`unicodedata.east_asian_width` で全 L3 見出しを実測。同ファイルの他21箇所はすべて幅44以下・下線50）
...
5. **`glossary.md:223` の `ディレクティブ` の意味列（「フォーマット定義を指定する**設定行**」）が、採用根拠より狭い。** 承認済み `testdata_notation.rst:871` は「キー名と値の2要素で記述するもの」と定義し、本ページの `<map>`/`<entry>` による登録は「行」ではない。`#19` の `decide` 1（`モックアップクラス`）と同型の欠陥である
   - **`#20` の中で処理済み**（2026-08-13、`/rn:ty` の `should` 1）。意味列を「ファイル・電文のフォーマットに関する属性を、キー名と値の2要素で指定するもの」に改めた。正表記・揺れ・採用根拠の各列は不変で、既存ページへの波及なし。`checks/task-20.md` ゲート18
