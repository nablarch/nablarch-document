...
| 空エントリの保持 | 同上 | どちらの欄にも書けないため表から外した。読み飛ばしそのものは `implementation/testdata_notation.rst:1534` が説明している | 実装は経路で割れる。Excel は NTF 本体が読み飛ばす（`nablarch-testing@e21bf67:src/main/java/nablarch/test/core/reader/PoiXlsReader.java:140-147` の `private boolean isBlankLine(List<String> line)` が全要素空のとき `true` を返し、同 `:93` `if (isBlankLine(list)) {` の直後の `continue;` で行が読み飛ばされる）。ただし converter 側にも判断がある。`nablarch-testing-converter@e977824` の `src/main/java/nablarch/test/tool/converter/xls/XlsFormatReader.java:566` `private static List<List<String>> dropEmptyEntries(List<List<String>> rows)` が、テーブル系（同 `:162`）と `LIST_MAP`（同 `:193`）に対して動く。同 `:550-554` の Javadoc は、本体が「除外前の生の行で空エントリを判定する」ため「マーカーカラムだけを持つ行」は本体の判定をすり抜けると述べており、converter が落とすのはこの残差である（一般の空エントリは本体が落とす）。YAML は `YamlFormatReader` に処理が無い（`nablarch-testing-converter@e977824` で `git grep -ln dropEmptyEntries` と `git grep -ln isEmptyEntry` を `src/main/java/` に対して実行したヒットが、どちらも `xls/XlsFormatReader.java` のみ）。いずれも `git show` で開いて照合した。無損失が両形式で成り立たない（`#32` の是正2、2026-08-21） |
| 行末の空セルの除去 | `input/testdata-converter-design.md:32`「無意味な情報は持たない（コメント、完全な空行、行末の空セルを除去）」 | 「意味を持たない情報」の行から外し、前提事項へ1段落として移した。段落は適用範囲をファイルとメッセージのテストデータに限定している | 実装は Excel 経路のみ（`src/main/java/nablarch/test/core/reader/TestCoreReaderAdapter.java:254`「{@link NablarchTestUtils#trimTailCopy(List)} で行末の空セルを除去済みである。」・同 `:410`「{@link NablarchTestUtils#trimTailCopy(List)}で行末の空セルを除去して返す。」。`src/main/java/nablarch/test/tool/converter/yaml/YamlFormatReader.java` には無く、`grep -n 'trimTail'` のヒットが0件。いずれも `nablarch-testing-converter@e977824` を `git show` で開いて照合した）。ページが適用範囲をファイルとメッセージに限ったのは、`implementation/testdata_notation.rst:1544-1545` の表行が対象を「ファイル・メッセージ」としており、同 `:1545` が「\ Excel\ 形式のみ。\ YAML\ 形式では ``rows:``\ の各要素をそのまま読み込む」と明記しているためである。あわせて因果を「Excel形式から読み込むときに取り除かれ、取り除かれたあとの状態が中間モデルに入る」に直した。なお NTF 本体側では、行末の空セルの除去はファイルとメッセージに閉じない。`nablarch-testing@e21bf67` の `src/main/java/nablarch/test/core/reader/HeaderLine.java:33` `List<String> keys = trimTailCopy(headerLine);` が、テーブル系（同 `TableDataParser.java:93`）と `LIST_MAP`（同 `ListMapParser.java:64`）のカラム名行でも走る。実測と、ページの記述を変えないと判断した経緯は `checks/task-32.md` §「4観点レビューの判定（是正3。調整役が記入。2026-08-21）」にある。（`#32` の是正2・是正3、2026-08-21） |

上表の末尾3件（マーカーカラム・空エントリ・行末の空セル）は `#32` の是正2 で追記した。逐語は 2026-08-21 に `nablarch-testing-converter@e977824` の `src/main/java/nablarch/test/core/reader/TestCoreReaderAdapter.java:129`・`:254`・`:410` と `src/main/java/nablarch/test/tool/converter/yaml/YamlFormatReader.java:491`、および `nablarch-testing@e21bf67` の `src/main/java/nablarch/test/core/reader/PoiXlsReader.java:93`・`:140-147` を `git show` で開いて照合し、行番号と文面が一致することを確認した。章構成設計 `.rn/20260724-ntf-yaml-support/design.md` §8「トンマナ」の「出典と実装が食い違う場合」が求める、確認した実装のファイル名・行番号・参照したコミットの記録にあたる。`#32` の是正3（2026-08-21）では、これに加えて `nablarch-testing-converter@e977824` の `src/main/java/nablarch/test/tool/converter/xls/XlsFormatReader.java:550-554`・`:566` と `src/main/java/nablarch/test/core/reader/TestCoreReaderAdapter.java:264`・`:464` を `git show` で開いて照合した。行末の空セルの除去は `readBlockBodyLines`（`TestCoreReaderAdapter.java:264`。実処理は同 `:413` の `BodyLineCollector` にあり、行末の空セルを落としているのは同 `:464` `bodyLines.add(NablarchTestUtils.trimTailCopy(line));` である）で行われ、`XlsFormatReader` からの呼び出しはファイル（`:212`）・メッセージ（`:240`）・同期応答電文（`:274`）の3か所である（`git grep -n readBlockBodyLines e977824 -- src/main/` で確認）。ただし同メソッドは `TestCoreReaderAdapter.java:124`（`readListMapColumnNames`）からも呼ばれるため、行末の空セルの除去がファイルとメッセージだけで起きるとは書いていない。
...
`#35` §1 の変更後の文は、第1文「行末の空セルは往復すると消える。」と第2文「……中間モデルに入らない。」がいずれも上記 (b) の反例に反する。作業指示 §1 の「1系統でも落ちない経路が見つかったら、この段落は書かずに報告すること」に従い、**本ラウンドでは `:71` を変更しなかった。** この文面は採らず、user が是正1・是正2 で `:71` の文面を確定し直した。

**現在の `:71` は、是正2（`ntf-doc-35-fix2.md` §1）が確定した次の1段落である。**
