...
初めてテスティングフレームワークを使う人になったつもりで刷新版を読み、次を判断してください。実装との整合は作成側で確認済みなので、レビューで確かめる必要はありません。

1. アーキテクトが第2部だけで導入と設定を終えられるか。設定漏れが起きそうな箇所はないか
...
6. 「間違えたときにどうなるか」の注意を落としました。落として困るものはないか
