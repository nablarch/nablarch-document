...
- リクエスト単体テスト・取引単体テストの実装ページ 12 本の記述（主張 346 件）を、実装を実際に動かして検証し、誤り 10 件・曖昧 28 件を直した
  - 実装（ピンのコミット）の上に解説書のコード例とテストデータをそのまま置いて実行し、期待値をずらすと落ちることまで確かめた。Example アプリケーションに題材の無い 5 件は実装ソースの確認にとどまる
...
- HTML リソースのコピー先と、コピー抑止の判定に使うディレクトリを実装どおりに直した
  - コピー先はテストクラスごとのダンプディレクトリで、「../htmlResources に作られる」は誤りだった
...
- カラム省略の制約を「`LIST_MAP` は Map 完全一致のため全カラム必須／登録系は推奨」に分けた
  - v6 は一律に省略不可としていた
- 実装で動かして確かめた結果、3 箇所を直した
  - 共通の準備データ（setUpDb）の投入は「テストメソッドごと」ではなく「execute の呼び出しごと」
  - テストショット番号は連番に限らない（`1-1` の形も使える）
  - グループID併用時に「両方が有効」になるのはテーブルのデータブロックだけで、ファイルは指定したグループだけ
- YAML スキーマが入力を拒否する制約 77 箇所を、同じ入力を Excel で書いたときのテスティングフレームワークの挙動と突き合わせ、3 箇所を直した
  - ファイルデータの構造説明「データ（1件以上）」は誤りで、カラム名の行だけのブロック（0 行のレコードレイアウト）は Excel の正規の書き方。期待要求電文では「電文が 1 通も送信されない」の検証になる。YAML では `rows: []` と書く
  - ディレクティブの値は整数・真偽値でも文字列でも書ける（Excel のセルは常に文字列で、フレームワークが変換する）
  - 数字だけの構造値（`record_type: 1` など）と `fw_header` の数字だけの値はクォートしないと読めない。v6 に無い規則なので書き足した
...
- アップロードファイルの具体例2組を落とし、2方法の説明と記法ページへの参照だけにした
  - 記載例ページに画像ファイルの例はある
- 実装で動かして確かめた結果、誤り 3 件・曖昧 9 件を直した
  - 「実行後の HttpRequest からリクエストパラメータの書き換えを確認する」節は削除した。取り出せるのは送信前に組み立てたオブジェクトで、内蔵サーバ側の書き換えは反映されない（v6 由来）
  - 自動確認の項目から `responseMessage`（応答電文の指定であって確認項目ではない）を外し、`expectedMessageByClient` を足した
  - `assertApplicationMessageId` は期待 ID が空なら例外が無くても成功する
  - トークン設定の効果、`context` カラムのユーザ ID の入り先、Advice を取る `execute` の 4 オーバーロード、`setUpDb` の投入単位などを補った
...
- 実装で動かして確かめた結果、誤り 2 件を直した
  - `setBody` に文字列を渡しても既定の設定では JSON 文字列リテラルにクォートされる。「そのままボディになる」は刷新で新規に書いた誤りだった
  - 委譲していないメソッドの一覧に 3 件が漏れていた
...
- 「送信キュー・受信キューを通信先と読み替える」の適用範囲を同期応答メッセージ送信の説明に限定した
  - 受信側は実際にキューを使うため
- 実装で動かして確かめた結果、2 件を直した
  - 読み替え表にテストショット一覧のカラム名（`expectedMessageByClient`・`responseMessageByClient`）を足した。MOM 用のカラムで書くと失敗する
  - HTTP メッセージ送信では要求電文のフレームワーク制御ヘッダの値が照合されないことを書いた（実装でアサートがコメントアウトされている）
...
- 実装で動かして確かめた結果、誤りは無く、2 件を補った
  - `execute` の第2引数（テストショットごとの setUpDb 再投入）
  - 同期応答メッセージ送信を行うテストでは `expectedMessage`・`responseMessage` を空欄にできない
...
- 実装で動かして確かめた結果、誤り 2 件・曖昧 3 件を直した
  - `MQSupport` はテストデータから電文を組み立てるだけで、キューへの投入と取り出しはサポートクラス自身が行う。構成図もシーケンス図に合わせた
  - `TestDataConverter` の参照先を〈テストデータの設定〉の該当節に直した
  - 受信テストでテスト対象が同期応答メッセージ送信も行う場合は要求電文の確認も行われる
...
- 「確認内容が分かれる理由」の参照先を節に絞り、参照先に因果（両方で動作するため）を書いた
...
**変えたことと理由**
- なし（横断的な変更のみ）
- `MESSAGING` ロガーへの電文出力を MOM との差として書いていたのを直した。MOM のモックも同じログを出す。差は Map／CSV 形式の有無だけ
...
**変えたことと理由**
- なし（横断的な変更のみ）
- 実装で動かして確かめた結果、誤り 1 件・曖昧 5 件を直した
  - 例1・例2 が取引の途中のテストショットにも `setUpTable=default` を書いていた。SETUP_TABLE は全件 DELETE→INSERT なので前のテストショットの結果が消える（実測で削除したはずのレコードが復活した）
  - `execute` を複数回呼ぶと呼び出しごとに setUpDb が再投入されることを書いた
  - YAML のファイル名は「テストメソッド名」ではなく「データセクション名」
...
- 実装で動かして確かめた結果、誤り 2 件・曖昧 5 件を直した
  - 応答不要メッセージ受信のテストクラス（`MessagingReceiveTest`・`MessagingReceiveTestSupport`）が書かれていなかった
  - `log.properties` の例をそのまま足すとロガー初期化に失敗する。`writerNames`・`availableLoggersNamesOrder` の追記を足した（v6 からの欠落）
  - モックのログには `MESSAGING` ロガーへの電文出力もあること、Map／CSV の項目の並びが定義順とは限らないこと、同じデータで繰り返す場合もタイムスタンプ更新が要ることを書いた
...
- 前提から「開発環境構築ガイドに従って構築済み」を落とした
  - 参照先のページが存在しない
- Eclipse に起動用スクリプトを登録する手順と、Eclipse から右クリックで起動する手順（画面 4 枚）を落とし、コマンドプロンプトから `httpDump.bat` に HTML ダンプのパスを渡す起動に書き換えた
  - IDE 固有の操作であり、ツール自体は Eclipse に依存しないため。リクエスト単体テストの設定（ウェブアプリケーション）で Eclipse の手順を落としたのと同じ理由
...
- Eclipse の Ant ビューにビルドファイルを登録して実行する手順（画面 4 枚）を落とし、コマンドラインの `ant -f master_data-build.xml` での実行に書き換えた。前提を「Eclipse と Maven」から「Ant と Maven」に変えた
  - IDE 固有の操作であり、ビルドファイルは素の Ant で動くため。Eclipse は Ant を同梱しているので、Eclipse を使わない場合は Ant を別途インストールすることになる
