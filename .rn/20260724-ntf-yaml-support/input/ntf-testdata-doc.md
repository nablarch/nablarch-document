...
- **FK 付き数値カラムも省略しない**。FK が設定された数値カラムを省略すると `"0"` が INSERT され、参照先に ID=0 の行が存在しない場合は FK 違反になる。NULL を意図する場合は省略せず明示的に `null` を記述すること（省略 ≠ NULL）。
...
FK のある親テーブルをセットアップ（= clear）するテストでは、**子テーブルも `SETUP_TABLE` に列挙して一緒に clear する**こと。NTF は親子関係に従い子→親の順で削除する。子テーブルのデータを書き換えないテストであっても、親を clear するなら子の clear 指定が必要。
...
- **期待行と DB 行の対応付けは DB の主キーで行われる**。主キーカラムは期待側でも省略しないこと
- **対象テーブルに存在する DB 行は全件を列挙する**。期待側に存在しない DB 行があると「余分な行がある」旨のエラーになる（部分検証はできない）

> **注意**: 主キーが自動採番（IDENTITY / シーケンス等）の場合、テスト実行時に払い出される値が不定になるため期待側に主キー値を書けず、複数行の検証が成立しない。テスト時に主キー値を既知にできる設計（業務キーの複合主キー等）にしておく必要がある。
