...
| MISSING / DUPLICATED / MODIFIED の全件が gate2-findings.md に列挙され対処案がある | OK | MISSING=486件、DUPLICATED=80件（DUP-001）、MODIFIED=3件（MOD-001〜003）が記録済み |
...
- MOVED=29（前回44→コード一意照合適用後29）: input ファイル由来の code 種別29件。2〜3行一意照合で真陽性のみ残存。
- MISSING=486: before 由来 281件 + input 由来 205件。
  ...
  - `setup_files:` 等、同一パターンが移送先に複数箇所存在し一意特定不可なものは MISSING（note に理由記録）。
