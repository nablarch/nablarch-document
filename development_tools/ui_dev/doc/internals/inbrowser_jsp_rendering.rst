...
.. _localrendering-list_format:

4. listFormat属性の動作がローカルレンダリングとサーバサイドレンダリングで異なる

  listFormat属性に ``sp`` を指定した場合、 サーバサイドでは ``&nbsp;`` となるが、ローカルレンダリングでは ``半角スペース`` となる。
  ブラウザで表示した際に差異が出ることは基本的にはないが、 ``span`` で代替可能であるため ``span`` の使用を検討すること。
