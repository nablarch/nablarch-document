...
# Sphinx 4以降の sphinx.util.nodes.split_explicit_title は明示タイトルの
# 正規表現が <([^<]*?)>$ に変更され、ターゲットに '<' を含む記法
# (例: `Title <pkg.Class.<init>(args)>`) を解釈できない。
# 本ドキュメントはコンストラクタ参照でこの記法を使用しているため、
# 旧来の解釈 (<(.*?)>$ 相当) を本ロール内に限定して実装する。
# Sphinx本体の正規表現には手を入れず、:ref: 等の他ロールへは影響しない。
_explicit_title_re = re.compile(r'^(.+?)\s*(?<!\x00)<(.*?)>$', re.DOTALL)


def split_explicit_title(text):
    """split role content into title and target, if given"""
    match = _explicit_title_re.match(text)
    if match:
