...
この２つの表現方法をアプリケーションが意識して実装すると思わぬ不具合を生み出す可能性がある。
...
また、カスタムタグやフォーマットして出力するなどの外部へ ``null`` を出力するときは出力するデータ形式に応じて未入力を意味する値に変換する。
変換はNablarchのハンドラ・ライブラリが行うため、アプリケーションで未入力値を変換する必要はない。

アプリケーションはこの方針に従い、未入力値を ``null`` に統一して開発するべきである。
また、未入力値を ``null`` に変換する :ref:`normalize_handler` をハンドラキューに追加する必要がある。
