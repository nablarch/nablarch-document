...
Nablarchが発行するSQLを変更する場合はコンポーネントを差し替える
----------------------------------------------------------------------------
Nablarchが発行するSQLをデフォルトから変更したい場合、
コンポーネントをプロジェクト側で作成したものに差し替えることで対応する。
これにより、アプリケーションの要件に応じて柔軟に発行するSQLを変更することができる。

例えば、Nablarchが使用するテーブルのカラムの追加や削除を行いたい場合がこれに該当する。

なお、テーブル名やカラム名をデフォルトから変更したい場合であれば、
新しくコンポーネントを作成する必要はなく、設定ファイルの修正のみでよい。
