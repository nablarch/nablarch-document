...
:ref:`http_character_encoding_handler` より後ろに配置すること
  本ハンドラはログにリクエストパラメータを出力することができる仕様となっているため、
  ウェブアプリケーション上でHTTP文字エンコード制御ハンドラよりも前に本ハンドラを配置してしまうと、
  規定の文字エンコーディングの設定が有効とならず、文字化けしたリクエストパラメータを出力してしまう可能性がある。
  そのため、ウェブアプリケーション上で本ハンドラを使用する場合は、HTTP文字エンコード制御ハンドラよりも後ろに本ハンドラを配置すること。

:ref:`status_code_convert_handler` より後ろに配置すること
  ステータスコード→プロセス終了コード変換ハンドラは共通起動ランチャの直後に配置される必要があるため、
  共通起動ランチャを使用するNablarchバッチアプリケーションとMOMメッセージングアプリケーションでは、
  ステータスコード→プロセス終了コード変換ハンドラよりも後ろに本ハンドラを配置すること。

上記以外のアプリケーションで本ハンドラを使用する場合は、特別な理由がない限りハンドラキューの先頭に配置すること。
