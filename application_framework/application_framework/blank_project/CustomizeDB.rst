...
-------------------------------------------------------------------------------------------------------------------------------------------------------------------
(DB2を使用してJSR352に準拠したバッチプロジェクトの疎通確認を行う場合のみ)sample-etl.xml  (src/main/resources/META-INF/batch-jobs)
-------------------------------------------------------------------------------------------------------------------------------------------------------------------
DB2を使用してJSR352に準拠したバッチプロジェクトの疎通確認を行う際、初期設定ではETL機能を使用するアプリケーションの起動時にエラーが発生する。
よって、エラーが発生するクラスを、DB2用のクラスに差し替える。

src/main/resources/META-INF/batch-jobs/sample-etl.xmlを以下のように書き換える。
...
.. code-block:: xml
      ...
      <job id="sample-etl" xmlns="http://xmlns.jcp.org/xml/ns/javaee" version="1.0">
          ...
          <step id="truncate" next="extract">
              ...
              <!-- ========書き換え箇所。TableCleaningBatchletForDb2クラスを指定する。======== -->
              <batchlet ref="tableCleaningBatchletForDb2"/>
          </step>
          <!-- 一時テーブルへのロード処理 -->
          ...
          <step id="validation" next="load">
              ...
              <!-- ========書き換え箇所。ValidationBatchletForDb2を指定する。======== -->
              <batchlet ref="validationBatchletForDb2"/>
          </step>
          <step id="load">
              <listeners>
                  <listener ref="nablarchStepListenerExecutor" />
...
.. important::
     DB2用に修正したクラスを使用する場合、指定するエンティティクラスには@Tableを付け、@Tableのname属性にテーブル名を記述すること。@Tableのないクラス、もしくはテーブル名の記述が無いクラスは処理しない。
