...
.. important::
  ...
  * 値を移送するときに :java:extdoc:`BeanUtil <nablarch.core.beans.BeanUtil>` を使用していないため、他の機能との整合性が取れていないことに注意する必要がある。
  * 出力対象の :java:extdoc:`Map <java.util.Map>` をどう扱うかの仕様が明確でないため、実際の挙動に注意して実装しなければならない。
  ...
  * JSONのデータタイプはマーカークラスであり、値をそのまま入出力するため、アプリケーション側で値が不正でないかをチェックしなければならない。(数値型や真偽値型を指定しても "data" などの値がそのまま読み書きされる。)  
  * XML, JSONでチェックなしのダウンキャストを行っているため、実装時に適した型を渡していることを確認する必要がある。(適さない型が指定されると実行時に例外が送出される。)
