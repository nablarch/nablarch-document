...
.. important::
  ...
  * 値を移送するときに :java:extdoc:`BeanUtil <nablarch.core.beans.BeanUtil>` を使用していないため、他の機能との整合性が取れていない。
  * 出力対象の :java:extdoc:`Map <java.util.Map>` をどう扱うかの仕様が明確でない（項目のキーがない場合と、キーが存在し値がnullの場合の扱いの違いなど）。
  * JSONのデータタイプが型変換せず、値をそのまま入出力する（数値型や真偽値型でも "data" などが読み書きされる）。
  * XML, JSONでチェックなしのダウンキャストを行っているため、適さない型が指定されると :java:extdoc:`ClassCastException <java.lang.ClassCastException>` が発生する。
