...
\c) ユーティリティクラス
  ...
  SystemTimeUtil                  システム日時の取得に使用するユーティリティ。
...
.. tip::
  ...
  実装例に入っているSystemAccountは仮のため、プロジェクトでGSPプラグインで自動生成したエンティティクラスを使うようにすること。
...
システムアカウントテーブルには、アカウント情報を格納する。
  ...
  ユーザIDロック            USER_ID_LOCKED            boolean                ロックしていない場合はfalse、ロックしている場合はtrue
