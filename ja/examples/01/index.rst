...
.. tip::
  ...
  Nablarch導入プロジェクトでは、gsp-dba-maven-plugin(DBA作業支援ツール)を使用した、データベースのテーブルに対応したEntityクラスの自動生成を推奨しています。
  本サンプルには、説明に最低限必要なシステムアカウントテーブルから自動生成したEntityクラスを含めています。
  実際のプロジェクトでは、本サンプルのEntityクラスを使用せず、データベースから自動生成したEntityクラスに置き換える必要があります。
