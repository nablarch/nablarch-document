...
**ルールベース変換による知識ファイル作成**
  約340ファイルをルールベース変換。構造化された公式知識のみを使用する。
...
本プラグインの利用には、Claude Code または GitHub Copilot が必要である。
...
上記の対応バージョン表の「インストール先」リンクから、ご利用のNablarchバージョンの手順を確認すること。
...
上記の対応バージョン表の「インストール先」列から、該当するバージョンのリンクをクリックし、セットアップ手順を確認すること。
