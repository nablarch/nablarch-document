...
* マーカーカラムの値。消えるのは値だけで、エントリ自体は残る（\ :ref:`コメント・マーカーカラム・空エントリを扱う <testdata_notation-comment_and_marker>`\ 参照）。例外として、カラム名の行をマーカーカラムだけで構成したデータブロックは、エントリの数と並びが意味を持つため、マーカーカラムとその値を保ったまま変換する
* テーブルと\ ``LIST_MAP``\ のカラム名の行にある、行末の空セル。テスティングフレームワーク自身が\ Excel\ 形式から読み込む時点で取り除いている

**そのまま保たれるもの**\ （消えると誤解しやすいもの）

* テスティングフレームワーク独自の特殊記法（\ ``${systemTime}``\ ・\ ``${binaryFile:...}``\ など）。本ツールは解釈せず、記法のまま変換先の形式へ書き出す。これらの記法は、テストの実行時にテスティングフレームワークが解釈する
* データ行の空セル。記法が意味を定めており（すべての要素が空のエントリの読み飛ばしなど。\ :ref:`テストデータの書き方 <testdata_notation>`\ 参照）、本ツールはその意味を保って変換する

**本ツールの決め**

* カラム名を1つも持たないテーブルデータを\ Excel\ 形式へ書き出すときは、カラム名の行に置くマーカーカラムの名前を\ ``[EMPTY]``\ とする。マーカーカラムを置くこと自体はテストデータの記法が定めるところで（\ :ref:`テーブルのデータを記述する <testdata_notation-table_data>`\ 参照）、名前が本ツールの決めである
* 収集方式が「グループ」のデータタイプ（テーブル・ファイル・グループID付きの電文）で、同じデータタイプと同じグループIDのデータブロックの間に別のデータブロックを挟んだシートは、\ Excel\ 形式ではテスティングフレームワークが収集を途中で終え、後ろのデータブロックを読まない（\ :ref:`グループIDによる使い分け <testdata_notation-group_id>`\ 参照）。\ YAML\ 形式にはこの中断に相当する記法が無いため、本ツールは読まれなかったデータブロックを出力せずに変換を続け、警告を出す。出力しないのは、テスティングフレームワークが読んでいなかったデータブロックだけである
