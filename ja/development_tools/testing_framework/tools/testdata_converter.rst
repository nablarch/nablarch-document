...
往復で再現されないもの
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
往復で再現されない情報が、形式ごとにいくつかある。いずれもテスティングフレームワークの仕様上の意味を持たない情報であり、再現されなくてもテストの内容は変わらない。本節では、何が再現されないかと、それでもテストの内容が変わらない理由を挙げる。

\ Excel\ 形式のセルの色・書式・結合セルと、\ YAML\ 形式のコメントは、再現されない。いずれも見た目と注記であり、テスティングフレームワークは読まない。\ Excel\ 形式へ書き出す際は、元の色や書式ではなく、\ :ref:`Excel形式の出力を整形する <testdata_converter-xls_format>`\ の設定に従った整形が新たに付与される（設定しない場合はデフォルト値が適用される）。
...
マーカーカラムは往復すると消える。テスティングフレームワークが読み込み対象から除外するため、中間モデルに入らない。マーカーカラムだけに値があるエントリは、テスティングフレームワークが読み飛ばさない（\ :ref:`コメント・マーカーカラム・空エントリを扱う <testdata_notation-comment_and_marker>`\ 参照）ため、本ツールもマーカーカラムの値だけを除いたエントリとして中間モデルに残す。消えるのはマーカーカラムの値だけである。ただし、カラム名の行をマーカーカラムだけで構成したデータブロックは例外として、マーカーカラムとその値を保ったまま変換する。このブロックの各エントリはフィールドを持たないが、テストショット一覧と行の順序で対応付ける用途では、エントリの数と並びが意味を持つためである。
...
テーブルと\ ``LIST_MAP``\ のカラム名の行にある行末の空セルは、往復すると消える。この空セルはテスティングフレームワーク自身が\ Excel\ 形式から読み込む時点で取り除いており、あってもなくてもテストの内容は変わらない。なお、データ行の空セルはこれと異なり、記法が意味を定めている（すべての要素が空のエントリの読み飛ばしなど）。本ツールはその意味を保って変換する。記法の詳細は\ :ref:`テストデータの書き方 <testdata_notation>`\ を参照。
