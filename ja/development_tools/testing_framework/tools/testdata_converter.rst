...
変換した\ YAML\ 形式のテストデータをテスティングフレームワークに読ませるには、\ :ref:`テストデータの設定（形式・配置・記述の省略） <testdata_setting>`\ の\ :ref:`テストデータの形式をYAMLに変更する <testdata_setting-yaml>`\ を行う。
...
.. tip::
  ...
  YAML\ 形式へ書き出す値は、値なしを除いてすべてダブルクォートで囲む。\ YAML\ の仕様では、囲まない\ ``123``\ は数値と解釈され、また前後の空白が脱落する。数値や空白を文字列として保つには、囲む必要がある。値なしだけは、文字列の\ ``"null"``\ と区別するためクォートなしの\ ``null``\ で表す。読み込み時は\ YAML\ のライブラリが囲みのダブルクォートを取り除くため、囲みが値に残ることはない。

.. _testdata_converter-round_trip:
...
往復で再現されないもの
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
本ツールは、どの形式の間で変換しても、テスティングフレームワークの仕様上の意味を保つ（\ :ref:`意味を変えずに往復できる <testdata_converter-round_trip>`\ 参照）。形式ごとの表現は変わっても、テストの内容は変わらない。そのうえで、往復で再現されないものと、本ツールの決めをここに挙げる。再現されないのは、テスティングフレームワークが読まない情報だけである。

**再現されないもの**\ 。いずれもテスティングフレームワークが読まない。

* \ Excel\ 形式のセルの色・書式・結合セルと、\ YAML\ 形式のコメント。\ Excel\ 形式へ書き出す際の整形は、\ :ref:`Excel形式の出力を整形する <testdata_converter-xls_format>`\ の設定に従って新たに付与される。設定しない場合はデフォルト値が適用される
* マーカーカラムの値。消えるのは値だけで、エントリ自体は残る（\ :ref:`コメント・マーカーカラム・空エントリを扱う <testdata_notation-comment_and_marker>`\ 参照）。例外として、カラム名の行をマーカーカラムだけで構成したデータブロックは、エントリの数と並びが意味を持つため、マーカーカラムとその値を保ったまま変換する（\ :ref:`記載例 <testdata_examples-comment_and_marker>`\ ）
* テーブルと\ ``LIST_MAP``\ のカラム名の行にある、行末の空セル。テスティングフレームワーク自身が\ Excel\ 形式から読み込む時点で取り除いている

**そのまま保たれるもの**\ （消えると誤解しやすいもの）

* テスティングフレームワーク独自の特殊記法（\ ``${systemTime}``\ ・\ ``${binaryFile:...}``\ など）。本ツールは解釈せず、記法のまま変換先の形式へ書き出す。これらの記法は、テストの実行時にテスティングフレームワークが解釈する（\ :ref:`記載例 <testdata_examples-special_notation>`\ ）
* データ行の空セル。記法が意味を定めており（すべての要素が空のエントリの読み飛ばしなど。\ :ref:`テストデータの書き方 <testdata_notation>`\ 参照）、本ツールはその意味を保って変換する（\ :ref:`記載例 <testdata_examples-comment_and_marker>`\ ）

**本ツールの決め**

* カラム名を1つも持たないテーブルデータを\ Excel\ 形式へ書き出すときは、カラム名の行に置くマーカーカラムの名前を\ ``[EMPTY]``\ とする。マーカーカラムを置くこと自体はテストデータの記法が定めるところで（\ :ref:`テーブルのデータを記述する <testdata_notation-table_data>`\ 参照）、名前が本ツールの決めである（\ :ref:`記載例 <testdata_examples-empty_table>`\ ）
* 収集方式が「グループ」のデータタイプ（テーブル・ファイル・グループID付きの電文）で、同じデータタイプと同じグループIDのデータブロックの間に別のデータブロックを挟んだシートは、\ Excel\ 形式ではテスティングフレームワークが収集を途中で終え、後ろのデータブロックを読まない（\ :ref:`グループIDによる使い分け <testdata_notation-group_id>`\ 参照）。\ YAML\ 形式にはこの中断に相当する記法が無いため、本ツールは読まれなかったデータブロックを出力せずに変換を続け、警告を出す。出力しないのは、テスティングフレームワークが読んでいなかったデータブロックだけである（グループIDの\ :ref:`記載例 <testdata_examples-group_id>`\ ）
...
Mavenプラグインで一括変換する場合も、\ Java\ のコードから変換を呼び出す場合も、入力ディレクトリ配下を再帰的に探索し、入力ディレクトリからの相対パスを保ったまま出力ディレクトリへ書き出す。変換元が\ Excel\ 形式の場合は、拡張子が\ ``.xls``\ のファイルと\ ``.xlsx``\ のファイルをどちらも対象とする。\ Excel\ 形式と\ YAML\ 形式ではデータセクションのまとめ方が異なるため、出力の構造は次のように読み替わる。
...
.. list-table::
  ...
  * - YAML\ 形式から\ Excel\ 形式へ
    - ディレクトリ\ ``foo/bar/``
    - ブック\ ``foo/bar.xlsx``\ と、その中のデータセクションごとのシート
...
Javaのコードから変換を呼び出す
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
変換を呼び出すクラスは\ ``nablarch.test.tool.converter.TestDataConverter``\ である。テストデータの形式変換を拡張する\ :java:extdoc:`nablarch.test.core.file.TestDataConverter <nablarch.test.core.file.TestDataConverter>`\ とは別のクラスである。変換元・変換先の形式と入出力ディレクトリだけを指定する場合は、4引数の\ ``convert``\ メソッドを呼び出す。戻り値は、変換したコンテナの件数である。コンテナは、\ Excel\ 形式ではブック、\ YAML\ 形式ではディレクトリであり、テストクラス1つ分のテストデータに相当する。
