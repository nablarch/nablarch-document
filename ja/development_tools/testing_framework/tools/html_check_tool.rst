...
使用を禁止するタグ・属性を変更する
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
使用を禁止するタグ・属性は、CSV形式の設定ファイルに記述する。設定ファイルは、ブランクプロジェクトに\ ``src/test/resources/nablarch/test/http-request-test/html-check-config.csv``\ として含まれており、デフォルト設定はこのパスを参照する。1行にカンマ区切りでタグ名と属性名を記述する。1つのタグに複数の属性を指定する場合は、複数行に分けて記述する。タグ名・属性名の大文字・小文字は区別せず、前後の空白は除去される。
