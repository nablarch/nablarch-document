...
.. _ntf_examples_table_data:

----------------------------
テーブルデータの記述例
----------------------------

以下に SETUP_TABLE・EXPECTED_TABLE・LIST_MAP の記述例を示す。
...
.. _ntf_examples_file_data:

----------------------------
ファイルデータの記述例
----------------------------

以下に固定長・可変長ファイルデータの記述例を示す。
...
.. _ntf_examples_messaging:

---------------------------------
メッセージングデータの記述例
---------------------------------

以下にメッセージング系データブロックの記述例を示す。
...
.. _ntf_examples_values:

---------------------------------------
特殊値・ディレクティブ・ヘッダの記述例
---------------------------------------

以下に特殊値・ディレクティブ・ヘッダの記述例を示す。
...
テーブルアサート（順序不問・主キー突合・全件列挙）
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

``EXPECTED_TABLE`` の行照合の動作は以下の通り。

- 期待行と DB の行は **DB の主キー** で対応付けられる。記述順と DB の格納順が違っても正しく突合される。主キーカラムは省略しないこと。
- 対象テーブルに存在する DB 行は **全件を列挙する** 必要がある。期待側に存在しない DB 行があると「余分な行がある」旨のエラーになる（部分検証はできない）。

以下の例では USER テーブルに 2 行存在することを前提として、全件を期待側に列挙している。
