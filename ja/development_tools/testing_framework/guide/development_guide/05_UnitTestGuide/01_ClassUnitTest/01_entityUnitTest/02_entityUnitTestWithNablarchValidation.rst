...
+--------------------+----------------------------------------------+
|underLimitMessageId |文字列長不足時のメッセージID \ [#]_\          |
+--------------------+----------------------------------------------+
...
.. [#]
   :java:extdoc:`@Length <nablarch.core.validation.validator.Length>` において、 ``max > min`` である場合、
   文字列長が超過・不足のいずれの場合であっても、出力されるメッセージは同じである。
   そのため、underLimitMessageIdはmaxAndMinMessageIdと同じ値を設定する必要がある。
