...
+--------------+--------------+----------------+-------------------------+
| max欄の記載  | min欄の記載  | maxとminの比較 | 省略時に使用される設定  |
+==============+==============+================+=========================+
| あり         | なし         | (該当なし)     | maxMessageId            |
+--------------+--------------+----------------+-------------------------+
| あり         | あり         | max > min      | maxAndMinMessageId      |
+--------------+--------------+----------------+-------------------------+
...
コンストラクタに対するテストケースフォーマット・テストメソッドは用意していない。
必要に応じて、個別にコンストラクタのテストを実装すること。
...
以下の手順では、項目間精査で使用するプロパティ等一部のプロパティは取得できない。
これらのプロパティが必要になる場合は、個別に記載すること。
...
+-------------------------+--------------------------------------------------------------------------+
|underLimitMessageId      |最長最小文字列長範囲外のメッセージのデフォルト値(可変長、不足時)  \ [#]_\ |
+-------------------------+--------------------------------------------------------------------------+
...
.. [#]
   :java:extdoc:`@Length <nablarch.core.validation.ee.Length>` において、 ``max > min`` である場合、
   文字列長が超過・不足のいずれの場合であっても、出力されるメッセージは同じである。
   そのため、underLimitMessageIdとmaxAndMinMessageIdは同じ値を設定する必要がある。
