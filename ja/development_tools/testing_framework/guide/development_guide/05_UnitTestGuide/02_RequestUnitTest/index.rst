...
テストデータを記載したテストデータファイルは、クラス単体テストと同様に\
...
テストデータを記載したテストデータファイルに、\ **setUpDb**\ という名前でシートを用意し、
...
ダウンロードファイルをテストする場合は、\
:ref:`batch_request_test` と同じ方法でファイルの期待値をテストデータファイルに記載してテストする。\
...
**期待するファイルの定義例**
 ...
 ダンプ出力結果が格納されるディレクトリの詳細は、 :ref:`html_dump_dir` を参照すること。
  ...
  .. code-block:: bash
   ...
   ダンプファイルの命名規則：
     テストデータファイルのシート名＋"_"＋テストケース名＋"_"＋ダウンロードされたファイル名
