...
.. tip::
  ...
  HTMLリソースのコピー先は、テストクラスごとのダンプディレクトリである。コピーを抑止するかどうかの判定には、ダンプディレクトリ直下の\ ``htmlResources``\ ディレクトリ（\ ``htmlResourcesRoot``\ で変更できる）の有無が使われる。このディレクトリはテスティングフレームワークが作成するものではなく、存在しない場合は、このシステムプロパティの指定の有無にかかわらずHTMLリソースがコピーされる。
