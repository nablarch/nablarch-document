...
Nablarchバッチアプリケーションのリクエスト単体テストでは、テスト対象のハンドラ構成にリクエストスレッド内ループ制御ハンドラが含まれる場合に、これをテスト用のハンドラに置き換える。応答不要メッセージ送信のテストでは、メッセージングプロバイダもテスト用のものに差し替える。ディレクティブのデフォルト値と、固定長ファイルの数値フィールドで使用するテスト用のデータ型も、コンポーネント設定ファイルに設定できる。後の2つはNablarchバッチアプリケーションに固有の設定ではなく、ファイルデータや電文のテストデータを扱うテストで使用する。テストの実装方法は\ :ref:`リクエスト単体テスト（Nablarchバッチアプリケーション） <request_unit_test_batch>`\ を参照。
