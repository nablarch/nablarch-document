...
機能概要
--------------------------------------------------
Nablarchバッチアプリケーションのリクエスト単体テストは、本番用のハンドラ構成のままでは終了しないことがある。リクエストスレッド内ループ制御ハンドラが要求データを探し続けるためで、このハンドラを含む構成ではテスト用のハンドラに置き換える。応答不要メッセージ送信のテストでは、キューに送る代わりに要求電文を保持して期待値と照合するメッセージングプロバイダに差し替える。ファイルや電文のテストデータを扱うテストでは、ディレクティブのデフォルト値を登録すると同じ記述の繰り返しを省け、固定長ファイルの数値フィールドにはテスト用のデータ型を登録する。テストの実装方法は\ :ref:`リクエスト単体テスト（Nablarchバッチアプリケーション） <request_unit_test_batch>`\ を参照。
