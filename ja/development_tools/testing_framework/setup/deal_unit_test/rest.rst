...
機能概要
--------------------------------------------------
RESTfulウェブサービスの取引単体テストでは、先行するリクエストのレスポンスに現れたセッションIDやCSRFトークンを、次のリクエストへ引き継ぐことが多い。この引き継ぎをテストコードに毎回書かずに済むよう、引き継ぐ処理をコンポーネントとして登録する。テスティングフレームワークが提供する実装から選べるほか、複数を組み合わせたり、独自の実装に差し替えたりできる。テストの実装方法は\ :ref:`取引単体テスト（RESTfulウェブサービス） <deal_unit_test_rest>`\ を参照。
