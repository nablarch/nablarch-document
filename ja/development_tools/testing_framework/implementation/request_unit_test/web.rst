...
.. image:: images/web/request_unit_test_structure.png
...
.. list-table::
  ...
  * - ``DbAccessTestSupport``
    ...
    - －
  * - ``HttpServer``
    - 内蔵サーバ。サーブレットコンテナとして動作し、\ HTTP\ レスポンスをファイルへ出力する機能を持つ
    - －
  * - ``HttpRequestTestSupport``
    - 内蔵サーバの起動や、リクエスト単体テストで必要となるアサートの機能を提供する
    - －
...
これらのクラスは、内蔵サーバも含めてすべて同一の\ JVM\ 上で動作する。このため、リクエストやセッションなどサーバ側のオブジェクトをテストコードから加工できる。
