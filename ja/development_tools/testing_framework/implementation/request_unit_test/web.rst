...
サポートクラスは、テストデータに記述したテストショットを次の手順で1件ずつ実行する。テストクラス全体で共通する準備データ（\ ``setUpDb``\ ）は、\ ``execute``\ の呼び出しごとに、この繰り返しに入る前に1回投入される。
...
* \ ``ThreadContext``\ への値の設定。ハンドラが設定するため不要である。ログインユーザの\ ID\ は、テストショットの\ ``context``\ カラムで指定する。指定した値はセッションの\ ``user.id``\ （設定ページの\ ``userIdSessionKey``\ ）に格納される。アプリケーションが別の方法で認証情報を保持している場合は、後述の\ ``Advice``\ の\ ``beforeExecute``\ でその情報をセッションに設定する。
...
テストメソッドを作成する
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
テストメソッドは、データセクション（\ :ref:`テストクラスとテストデータの対応 <testdata_notation-file_structure>`\ 参照）と1対1で対応させて作成する。ここでは、テストメソッドの分割方針、テストを実行するメソッドの呼び出し、実行前後への固有処理の挿し込み、二重サブミット防止機能のトークンの設定、リクエストと実行コンテキストの組み立ての5つを説明する。
...
* 画面表示の検証項目は、正常系と異常系のいずれかのテストメソッドに含められるかを検討する。1つのデータセクションに正常系・異常系・画面表示の検証を混在させるとテストデータの記述が煩雑になる場合は、画面表示の検証用のテストメソッドを別に作成する。そうでない場合は、正常系または異常系のテストメソッドに含める。

リクエスト\ ID\ は、アプリケーションのルーティング定義で決まる。例えば ``routes.xml`` に ``<match path="/action/:controller/:action"/>`` と定義されていれば、\ ``:action``\ に当たる\ Action\ のメソッド名（\ ``update``\ など）がそのままリクエスト\ ID\ になる。

正常系・異常系・画面表示の検証用に分割した場合の例を次に示す（リクエスト\ ID\ とメソッド名を別の名前にする命名規約を採るプロジェクトの例）。データセクション名は、既定ではテストメソッド名と同じにする（後述）。
...
.. tip::
  ...
  テストメソッドを分割するのは、1つのデータセクションが煩雑になり可読性が下がることを避けるためである。この例以外でも、1つのデータセクションにさまざまなテストショットを詰め込むと可読性が下がる場合は、データセクションを分割する。
...
引数のない\ ``execute``\ は、テストメソッド名と同じ名前のデータセクションを読み込む。データセクション名をテストメソッド名と変えたい場合は、データセクション名を引数に取るオーバーロードメソッドを呼び出す。
...
固有の準備処理や結果確認処理が必要な場合は、\ :java:extdoc:`Advice <nablarch.test.core.http.Advice>`\ を引数に取る次のオーバーロードメソッドを呼び出し、リクエストの送信前後に処理を挿し込む。
...
\ :java:extdoc:`BasicAdvice <nablarch.test.core.http.BasicAdvice>`\ には次の2つのメソッドが用意されており、それぞれリクエストの送信前と送信後に呼び出される。
...
.. list-table::
  ...
  * - メソッド
    - 取得できる値
  * - ``getTestCaseName()``
    - テストショットを識別する名前（\ ``データセクション名_Shot番号_説明``\ ）。アサート失敗時のメッセージと\ HTML\ ダンプのファイル名に使用する
  * - ``getSheetName()``
    - データセクション名
  * - ``getTestCaseNo()``
    - テストショット番号
...
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
二重サブミット防止機能（\ :ref:`二重サブミットを防ぐ <tag-double_submission>`\ ）は、サーバサイドとクライアントサイドの両方で動作するため、リクエスト単体テストではサーバサイドの動作を、取引単体テストではクライアントサイドの動作を確認する。
...
サポートクラスが生成するリクエストを使わず、テストコードからトークンを設定する場合は、次のメソッドを呼び出す。トークンの発行と、リクエストパラメータおよびセッションへの格納が行われる。

.. code-block:: java

  void setValidToken(HttpRequest request, ExecutionContext context)

トークンを設定するかどうかをテストデータで制御したい場合は、次のメソッドを使用する。第3引数が\ ``true``\ の場合は\ ``setValidToken``\ と同じ動作になり、\ ``false``\ の場合はセッションのトークンが無効になる。リクエストパラメータのトークンは取り除かれない。テストクラスに分岐処理を書かずに済む。
...
\ ``createExecutionContext``\ の引数にはユーザ\ ID\ を指定する。指定したユーザ\ ID\ はセッションの\ ``user.id``\ （設定ページの\ ``userIdSessionKey``\ ）に格納される。
...
.. tip::
  ...
  テストショット一覧に記述するリクエストパラメータは、出力された\ HTML\ ダンプから\ :ref:`リクエスト単体データ作成ツール <request_data_tool>`\ を使って得られる。パラメータ名を人手で書き写す必要がなくなる。得られるのは\ Excel\ 形式のテストデータである。
  ...
  void testUsers00303Normal() {
      support.execute(new BasicAdvice() {
          ...
          public void afterExecute(TestCaseInfo testCaseInfo, ExecutionContext context) {
              // 期待値をテストデータから取得する（データセクションはテストメソッド名と同じ）
...
オブジェクトのプロパティを確認する
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
リクエストスコープなどから取り出したオブジェクトのプロパティを確認する場合は、次のメソッドを使用する。エンティティや\ Form\ を1件だけ確認する場合は前述の\ ``assertEntity``\ でもよいが、配列やリストを確認する場合や、アサート失敗時のメッセージを指定したい場合はこちらを使用する。第1引数にはアサート失敗時のメッセージ、第2引数にはデータセクション名、第3引数には期待値の\ ID\ 、第4引数には確認対象を指定する。
...
ダウンロードファイルを確認する
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
ダウンロードされたファイルは、\ HTML\ ダンプと同じディレクトリへ出力される。ファイル名は\ ``データセクション名_Shot番号_説明_ダウンロードされたファイル名``\ である。
...
ダンプディレクトリの配下にはテストクラスごとのディレクトリが作られ、その配下に\ ``データセクション名_Shot番号_説明.html``\ という名前で\ HTML\ ダンプが出力される。\ HTML\ ダンプが参照する\ HTML\ リソース（スタイルシートや画像など）も同じディレクトリへ出力されるため、このディレクトリを保存しておけば、どの環境でも同じように表示を確認できる。
