...
テストデータを作成する
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
テストデータの格納場所と記述方法は\ :ref:`テストデータの書き方 <testdata_notation>`\ に従う。ただし、RESTfulウェブサービスのリクエスト単体テストで自動的に読み込まれるのは、次の2つのデータセクションだけである。

* テストクラス全体で共通する準備データ（\ :ref:`共通の準備データをまとめる <testdata_notation-setupdb>`\ 参照）
* テストメソッドごとの準備データ（データセクション名をテストメソッド名と同じにする。\ :ref:`テストクラスとテストデータの対応 <testdata_notation-file_structure>`\ 参照）
