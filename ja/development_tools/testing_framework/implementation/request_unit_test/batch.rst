...
テストメソッドでは、サポートクラスの\ ``execute``\ を呼び出す。引数には、読み込むデータセクション名を渡す。共通の準備データ（\ :ref:`setUpDb データセクション <testdata_notation-setupdb>`\ ）は\ ``execute``\ の呼び出しごとに1回投入されるが、第2引数に\ ``true``\ を渡すとテストショットごとに投入し直される。
...
* 要求電文の内容の確認。テストショット一覧の\ ``expectedMessage``\ に記述したグループIDの期待値と、送信された電文を照合する。ただし同期応答メッセージ送信を行うテストでは、\ ``expectedMessage``\ と\ ``responseMessage``\ を空欄にできない。空欄のまま送信が行われるとテストは失敗する。
