...
テストメソッドを作成する
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
1つの取引は、1つのテストメソッドの中で実行する。テストメソッドから\ ``execute``\ を呼ぶと、引数に渡したデータセクションに記述したテストショットが上から順に実行される。データセクション名は、テストメソッド名と同じにする。
...
データセクションを複数に分ける場合は、それぞれの名前を引数に渡した\ ``execute``\ を、データセクションの数だけ呼ぶ。\ ``execute``\ を呼ぶたびに、共通の準備データ（\ :ref:`setUpDb データセクション <testdata_notation-setupdb>`\ ）が投入し直される。取引の途中結果を受け渡すテーブルは\ ``setUpDb``\ に記述しない。
...
テストデータを作成する
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
1つのデータセクションにつき1つのテストを記述することを原則とする。1つの取引を構成する複数のバッチ処理は、そのデータセクションのテストショット一覧に上から順に並べる。テストショット一覧に記述できるカラムの仕様は\ :ref:`テストショット一覧（testShots）を記述する <testdata_notation-test_shots>`\ を、処理方式ごとの一般的な記述例は\ :ref:`テストデータの記載例 <testdata_examples>`\ を参照。ここでは、取引単体テストに固有の書き方だけを示す。

原則に対する例外は2つある。テストデータが大量である場合や、1つの取引に含まれる処理が多い場合は、すべてのテストデータを1つのデータセクションに詰め込むと読みにくくなる。このときは、1つのテストを複数のデータセクションに分割してよい。逆に、非常に簡単なテストでデータ量が少ない場合は、1つのデータセクションに複数のテストを含めてよい。

以降では、次の3つの書き方を\ Excel\ 形式・\ YAML\ 形式のそれぞれについて示す。

* **1つのデータセクションにまとめる**

  * 原則どおりの書き方である。ファイル入力・ユーザ削除・ファイル出力の3つの処理で構成される取引（取引\ ID\ は\ ``B21AC01``\ ）を、3件のテストショットとして1つのデータセクションに記述する。テストメソッドは\ ``execute``\ を1回呼ぶだけであり、データセクション名はテストメソッド名と同じ\ ``testSuccess``\ にする

* **複数のデータセクションに分割する**

  * 1つ目の例外である。同じ構成の取引（取引\ ID\ は\ ``B21AA01``\ ）を、処理ごとに\ ``testSuccess_fileInput``\ ・\ ``testSuccess_userDelete``\ ・\ ``testSuccess_fileOutput``\ の3つのデータセクションに分ける。テストメソッドからは、それぞれの名前を引数に渡して\ ``execute``\ を呼ぶ

* **1つのデータセクションに複数のテストを含める**

  * 2つ目の例外である。通常の入力と入力データが0件の場合という2つのテストを、1つのデータセクションに記述する。テストショット番号は\ ``1-1``\ ・\ ``1-2``\ ・\ ``2-1``\ ・\ ``2-2``\ のように「テストの番号 - 取引内での順序」の形で付ける
...
シート名は\ ``testSuccess``\ である。カラム数が多いため、前半・後半に分けて示す。後半の表の\ ``no``\ 列は前半と対応付けるための再掲であり、実際のシートでは\ ``no``\ を1列だけ置いて1行に続けて記述する。
...
テストクラスと同じ名前のディレクトリに置いた\ ``testSuccess.yaml``\ に記述する。ファイル名はデータセクション名と同じである。この例ではデータセクション名をテストメソッド名と同じにしているため、\ ``testSuccess.yaml``\ になる。
