...
.. list-table::
  ...
  * - 名称
    ...
    - 作成単位
  * - テストクラス
    - データセクションを指定して、サポートクラスのメソッドを呼び出す。
    - テスト対象クラスにつき1つ作成する。
  * - テストデータ
    - 入力値・期待するバリデーションメッセージ・期待値を記述する。
    - 確認する対象につき1つのデータセクションを作成する。
  * - テスト対象クラス
...
テストメソッドを作成する
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
確認する対象ごとに、テストデータに用意するカラムと、呼び出すサポートクラスのメソッドを示す。テストメソッドには、データセクション名とデータブロックの\ ID\ を指定してサポートクラスのメソッドを呼び出すコードだけを書く。両方のバリデーション方式に共通する3つを先に示し、続いて方式ごとに固有の3つを示す。

文字種と文字列長をテストする
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
単項目バリデーションは、入力される文字種と文字列長に関するものがほとんどである。例えば、最大文字列長50文字・必須入力・全角カタカナのみを許容するプロパティであれば、次のテストショットが必要になる。

* 全角カタカナ50文字を入力し、バリデーションが成功する

  最大文字列長と文字種の確認。

* 全角カタカナ51文字を入力し、バリデーションが失敗する

  最大文字列長の確認。

* 全角カタカナ1文字を入力し、バリデーションが成功する

  最小文字列長と文字種の確認。

* 空文字を入力し、バリデーションが失敗する

  必須入力の確認。

* 許容しない文字種を入力し、バリデーションが失敗する

  文字種の確認。半角カタカナ・半角英字・全角ひらがな・漢字など、許容しない文字種ごとにテストショットが必要になる。

このように、プロパティ1つあたりのテストショット数が多くなる。
