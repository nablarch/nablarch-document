...
デフォルトのトランザクション以外も使用する場合は、テスト用のコンポーネント設定ファイルと環境設定ファイルへの設定が必要である（\ :ref:`コンポーネント単体テストでデフォルト以外のトランザクションを使用する <class_unit_test_setting-db_transaction>`\ ）。
...
第1引数にはアサート失敗時のメッセージ、第2引数には読み込み単位の名前、第3引数には期待値の\ ID\ 、第4引数には検索結果を指定する。期待値は、第3引数の\ ID\ を持つ ``LIST_MAP``\ として、\ SELECT\ 文で指定したカラム名（別名を付けた場合は別名）をカラム名にして記述する（\ :ref:`LIST_MAPのデータを記述する <testdata_notation-list_map>`\ ）。
