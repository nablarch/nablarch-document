...
--------------------------------------------------
テストショット一覧（``testShots``\ ）の記述例を、処理方式ごとに示す。これは\ ``LIST_MAP``\ というデータタイプの利用例の1つであり、``LIST_MAP``\ 自体の記述例は後述の\ :ref:`LIST_MAPのデータを記述する <testdata_examples-list_map>`\ に示す。使用できるカラムの仕様は\ :ref:`テストショット一覧（testShots）を記述する <testdata_notation-test_shots>`\ を参照。
