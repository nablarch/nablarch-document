...
--------------------------------------------------
テストショット一覧（``testShots``\ ）の記述例を、処理方式ごとに示す。テストショット一覧は\ ``LIST_MAP``\ というデータタイプの用途の1つであり、``LIST_MAP``\ の他の用途は後述の\ :ref:`LIST_MAPのデータを記述する <testdata_examples-list_map>`\ に示す。使用できるカラムの仕様は\ :ref:`テストショット一覧（testShots）を記述する <testdata_notation-test_shots>`\ を参照。
...
LIST_MAPのデータを記述する
--------------------------------------------------
キーバリュー形式の汎用データ（``LIST_MAP``\ ）の記述例を、用途ごとに示す。用途の1つであるテストショット一覧（``testShots``\ ）は、前述の\ :ref:`テストショット一覧（testShots）を記述する <testdata_examples-test_shots>`\ で扱っている。記法の仕様は\ :ref:`LIST_MAPのデータを記述する <testdata_notation-list_map>`\ を参照。
