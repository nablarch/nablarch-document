...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
注文明細テーブル（\ ``ORDER_DETAIL``\ ）に対して、明細1件の正常注文（テストショット1）と明細2件の大量注文（テストショット2）という2つのテストショットを実行する例である。テストショット一覧の ``setUpTable``\ ・\ ``expectedTable``\ カラムに記述した値がグループIDになり、そのグループIDを持つデータブロックだけが対象になる。\ Excel\ 形式・\ YAML\ 形式のそれぞれについて示す。
...
.. important::
  ...
  同じグループIDのデータブロックは、シート内で連続させて記述する。異なるグループIDのデータブロックを挟むと、そこで収集が終了し、以降のデータが読み込まれないままテストが成功してしまう（\ :ref:`グループIDによる使い分け <testdata_notation-group_id>`\ 参照）。
...
--------------------------------------------------
テストショット一覧（``testShots``\ ）の記述例を、処理方式ごとに示す。テストショット一覧は\ ``LIST_MAP``\ というデータタイプの利用例の1つであり、``LIST_MAP``\ の他の利用例は後述の\ :ref:`LIST_MAPのデータを記述する <testdata_examples-list_map>`\ に示す。使用できるカラムの仕様は\ :ref:`テストショット一覧（testShots）を記述する <testdata_notation-test_shots>`\ を参照。
...
LIST_MAPのデータを記述する
--------------------------------------------------
キーバリュー形式の汎用データ（``LIST_MAP``\ ）の記述例を、用途ごとに示す。利用例の1つであるテストショット一覧（``testShots``\ ）の記述例は、前述の\ :ref:`テストショット一覧（testShots）を記述する <testdata_examples-test_shots>`\ に示している。記法の仕様は\ :ref:`LIST_MAPのデータを記述する <testdata_notation-list_map>`\ を参照。
...
.. important::
  ...
  1つの固定長ファイルのデータブロック内では、全てのレコードレイアウトのレコード長を一致させる。一致していない場合はエラーになる。この例では、フィールド長の合計が40バイトになるよう、ヘッダレコードに\ FILLER\ を置いて調整している。
...
.. important::
  ...
  ``quoting-delimiter``\ を指定しない場合、ダブルクォートで囲んだ範囲のカンマも区切り文字として解釈されるため、フィールド数が定義と一致せず ``number of input fields was invalid.``\ で始まるエラーになる。
...
グループIDでファイルを使い分ける
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
テストショットごとに異なる入力ファイルを使い分ける例である。グループIDなしのデータブロックが1件処理のテストショットに、``case2``\ のデータブロックが複数件処理のテストショットに対応する。\ Excel\ 形式・\ YAML\ 形式のそれぞれについて示す。
...
メッセージングのデータを記述する
--------------------------------------------------
要求電文・応答電文のテストデータの記述例を、用途ごとに示す。記法の仕様は\ :ref:`メッセージングのデータを記述する <testdata_notation-messaging_data>`\ を参照。なお、同期応答メッセージ送信で使う4つのデータタイプでは、レコード種別の扱いが\ Excel\ 形式と\ YAML\ 形式で異なるため、同じ内容を記述しても生成されるレコード種別は一致しない。
...
``NAME``\ カラムは半角スペース1文字、``MEMO``\ カラムはダブルクォート1文字として読み込まれる。
...
空エントリを記述する
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
テストデータの見やすさのために空行を挟む例である。全要素が\ null\ または空文字のエントリは読み飛ばされるため、データの区切りとして空行を入れても結果は変わらない。\ Excel\ 形式・\ YAML\ 形式のそれぞれについて示す。
