...
.. list-table::
  ...
  * - リクエスト単体テスト
    ...
    - 本番に近い構成でテストできる
  * - 取引単体テスト
    - ウェブアプリケーションは手動操作、それ以外の処理方式はJUnitで自動実行
    - 複数リクエストにまたがる業務の流れ
    - 手動操作の場合は、アプリケーションサーバへのデプロイが必要。エビデンスは画面ハードコピー・DBダンプ

.. important::

  取引単体テストの実施方法は、対象とする処理方式によって異なる。ウェブアプリケーションでは、テスト対象のアプリケーションをアプリケーションサーバにデプロイし、手動で操作してテストを行う。それ以外の処理方式では、リクエスト単体テストを連続実行することにより、取引単体テストをJUnitで自動実行できる。
