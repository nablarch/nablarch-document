...
.. important::
   PostgreSQLで遅延ロードを使用して大量データを処理する際は、処理中にトランザクション制御が実行されないよう注意すること。

   PostgreSQLでは、カーソルオープン中にトランザクション制御が行われると、カーソルがクローズされる。
   このため、遅延ロードを使用した大量データの処理中にトランザクション制御を行った場合、遅延ロードがクローズ済みのカーソルを参照し、エラーとなる。
   特に :ref:`バッチ処理でコミット間隔を指定する <loop_handler-commit_interval>` 場合、
   処理対象データが存在している状態でコミット間隔に到達すると、コミットに伴いカーソルがクローズされ、以降のデータが処理できず異常終了する。
   
   トランザクション制御を含む大量データ処理を実施したい場合は、遅延ロードではなく :ref:`ページング<universal_dao-paging>` を使用すること。
