...
本認可チェック(簡易版)と :doc:`もう1つの認可チェック <permission_check>` を使い分ける基準について説明する。
...
このようなシステムでは、 :doc:`もう1つの認可チェック <permission_check>` を用いて権限の組み合わせをデータで管理することを推奨する。
...
:doc:`もう1つの認可チェック <permission_check>` では、JSPのカスタムタグで認可チェックを行い自動的にボタンの表示・非表示を切り替えるような仕組みが提供されている。
