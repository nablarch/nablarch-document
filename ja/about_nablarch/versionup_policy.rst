...
ただし、アダプタのAPIについてはPublishedアノテーションが付与されていても後方互換性ポリシーの対象外となります。
アダプタ利用者が使用することを想定したAPIにはPublishedアノテーションを付与していますが、アダプタは外部ライブラリのAPIに依存するため、外部ライブラリをバージョンアップする際にどうしても後方互換性を維持できない場合があるためです。
後方互換性を維持できるように最大限努めますが、後方互換性が維持できない場合があることはご承知おきください。
また、Publishedアノテーションを付与していないAPIについては非公開APIと同様に使用しないでください。
